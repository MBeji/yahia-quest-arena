      ...
      <div className="relative flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-gold/30 bg-gold/10 px-4 py-2">
          ...
          <div>
            ...
            <div className="text-2xs uppercase tracking-wider text-gold">{heroClass}</div>
          </div>
        </div>
      </div>
      ...
      <div className="relative mt-5">
        <div className="mb-1 flex items-center justify-between text-xs text-muted-foreground">
          <span className="flex items-center gap-1">
            <Sparkles className="h-3.5 w-3.5 text-gold" /> {p.intoLevel}/{p.perLevel} XP
          </span>
          <span>{p.isMax ? t.parcours.maxLevel : `${p.toNext} ${t.parcours.xpToNext}`}</span>
        </div>
        <GoldProgress value={p.pct} aria-label={t.parcours.level} />
      </div>
    </div>
  );
}
