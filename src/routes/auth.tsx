...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Rejoindre l'académie · YahiaAcademy" },
      { name: "description", content: "Crée ton héros et démarre ta préparation au concours de 9ème année." },
    ],
  }),
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isSignup) {
        ...
        toast.success("Héros créé ! Bienvenue à l'académie.");
        navigate({ to: "/dashboard" });
      } else {
        const { error } = await supabase.auth.signInWithPassword({ email, password });
        if (error) throw error;
        toast.success("Bon retour, guerrier.");
        navigate({ to: "/dashboard" });
      }
    } catch (err) {
      ...
      <div className="relative mx-auto flex min-h-screen max-w-md flex-col items-center justify-center px-6 py-12">
        ...
        >
          <h1 className="font-display text-2xl font-bold">
            {isSignup ? "Forge ton héros" : "Bon retour, guerrier"}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {isSignup ? "Quelques secondes pour entrer dans l'arène." : "Reprends ta quête là où tu l'as laissée."}
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            >
              ...
              {isSignup ? "Forger mon héros" : "Entrer dans l'arène"}
            </button>
          </form>

          <p className="mt-5 text-center text-xs text-muted-foreground">
            {isSignup ? "Déjà un héros ? " : "Pas encore d'héros ? "}
            <Link
              to="/auth"
