import type { TranslationKeys } from "./types";
import { frPublic } from "./fr-public";

export const fr: TranslationKeys = {
  meta: {
    title: "Na9ra Nal3ab — tous les cours de l'école tunisienne, gratuitement",
    description:
      "Cours, résumés et exercices corrigés de toute l'école tunisienne, de la 1re année au Baccalauréat. Accès libre, gratuit, sans inscription — pour toute la famille.",
    ogTitle: "Na9ra Nal3ab — l'école tunisienne, gratuite et en accès libre",
    ogDescription:
      "Tout le programme officiel, du Primaire au Bac : cours, résumés et exercices corrigés, gratuitement.",
  },
  common: {
    backToHall: "Retour au hall",
    back: "Retour",
    difficulty: "Difficulté {n} sur {max}",
    loading: "Chargement…",
    retry: "Réessayer",
    home: "Accueil",
    continue: "Continuer",
    completed: "Terminé",
    leaderboard: "Classement",
    signIn: "Connexion",
    signUp: "S'inscrire",
  },
  errors: {
    notFoundTitle: "Cette quête n'existe pas",
    notFoundDesc: "Le parchemin est introuvable, jeune aspirant.",
    notFoundAction: "Retour à l'Académie",
    errorTitle: "Le parchemin s'est déchiré",
    errorFallback: "Une erreur est survenue.",
    subjectLoadFailed: "Impossible de charger cette matière.",
    programmeLoadFailed: "Impossible de charger le programme.",
    parcoursLoadFailed: "Impossible de charger ce niveau.",
    parcoursNotFound: "Ce niveau est introuvable.",
    chapterLoadFailed: "Impossible de charger ce cours.",
    extrasLoadFailed: "Impossible de charger les extras.",
    sessionStartFailed: "Impossible de démarrer la session.",
  },
  auth: {
    titleSignup: "Crée ton compte",
    titleLogin: "Ravi de te revoir",
    subtitleSignup: "Quelques secondes, et c'est 100 % gratuit.",
    subtitleLogin: "Reprends là où tu t'étais arrêté.",
    googleCta: "Continuer avec Google",
    sessionRemembered:
      "Session mémorisée sur ce navigateur : pas besoin de ressaisir tes identifiants à chaque fois.",
    or: "ou",
    roleLabel: "Je suis…",
    roleStudent: "Élève",
    roleParent: "Parent",
    heroNameLabel: "Prénom ou pseudo",
    allianceCodeLabel: "Code Alliance élève (optionnel)",
    allianceCodeHint: "Entre le code de ton enfant pour lier les comptes maintenant.",
    emailLabel: "Email",
    passwordLabel: "Mot de passe (min. 8 caractères)",
    submitSignup: "Créer mon compte",
    submitLogin: "Se connecter",
    hasAccountPrompt: "Déjà un compte ? ",
    noAccountPrompt: "Pas encore de compte ? ",
    switchToSignup: "Créer un compte",
    toastSignupSuccess: "Compte créé ! Bienvenue 🎉",
    toastLoginSuccess: "Content de te revoir !",
    toastLinked: "Compte lié avec {name}.",
    googleNotConfigured: "Le provider Google n'est pas configuré dans Supabase Auth.",
    googleFailed: "Échec de la connexion Google : {message}",
    passwordTooShort: "Le mot de passe doit contenir au moins 8 caractères.",
    errorInvalidLogin: "Email ou mot de passe incorrect.",
    errorEmailNotConfirmed: "Vérifie ta boîte mail pour confirmer ton compte.",
    errorAccountExists: "Ce compte existe déjà. Connecte-toi.",
    errorAccountExistsSignup:
      "Cette adresse a déjà un compte — aucun email n'a été envoyé. Connecte-toi, ou utilise « Mot de passe oublié ? ».",
    errorMailerDown:
      "Ton compte est créé, mais l'email de confirmation n'a pas pu partir. Réessaie dans quelques minutes.",
    errorRateLimit: "Trop de tentatives. Réessaie dans quelques minutes.",
    errorSignupDisabled: "L'inscription est temporairement désactivée.",
    errorGeneric: "Erreur d'authentification. Réessaie.",
    oauthErrorPrefix: "Google : {msg}",
    googleCallbackIncomplete:
      "La connexion Google n'a pas pu être finalisée. Réessaie — et vérifie que ce compte n'est pas déjà inscrit par email/mot de passe.",
    emailSentTitle: "Confirme ton email",
    emailSentBody1: "Un lien d'activation a été envoyé à",
    emailSentBody2: "Clique sur le lien pour activer ton compte et accéder à ton espace.",
    emailSentSpam: "📬 Vérifie aussi tes spams si tu ne vois pas l'email dans ta boîte principale.",
    emailSentEdit: "← Modifier l'adresse email",
    emailSentResend: "Renvoyer l'email",
    emailSentResendWait: "Renvoyer dans {s} s",
    emailSentResendDone: "Email renvoyé — vérifie ta boîte (et tes spams).",
    forgotCta: "Mot de passe oublié ?",
    forgotTitle: "Réinitialiser le mot de passe",
    forgotSubtitle: "Saisis ton email : on t'envoie un lien pour en choisir un nouveau.",
    forgotSubmit: "Envoyer le lien",
    forgotBack: "← Retour à la connexion",
    forgotSentTitle: "Lien envoyé",
    forgotSentBody:
      "Si un compte existe pour {email}, tu recevras un lien pour réinitialiser ton mot de passe.",
    forgotSentSpam: "📬 Pense à vérifier tes spams.",
    resetTitle: "Choisis un nouveau mot de passe",
    resetSubtitle: "Saisis ton nouveau mot de passe ci-dessous.",
    resetPasswordLabel: "Nouveau mot de passe (min. 8 caractères)",
    resetSubmit: "Enregistrer",
    resetSuccess: "Mot de passe mis à jour. Te voilà connecté !",
    resetInvalidTitle: "Lien invalide ou expiré",
    resetInvalidBody: "Ce lien de réinitialisation n'est plus valable. Demande-en un nouveau.",
    resetRequestNew: "Demander un nouveau lien",
    resetError: "Impossible de mettre à jour le mot de passe. Réessaie.",
    resetChecking: "Vérification du lien…",
  },
  dashboard: {
    title: "Hall des Héros · Na9ra Nal3ab",
    heroClass: "",
    days: "jours",
    day: "jour",
    longestStreak: "Plus longue série",
    levelLabel: "Niveau",
    noQuestTarget: "Aucune quête disponible pour l'instant. Termine d'abord une quête de matière.",
    xpProgress: "Progression XP",
    dailyGoalLabel: "Objectif du jour",
    dailyGoalReached: "✨ Objectif atteint !",
    dailyGoalRemaining: "XP restants aujourd'hui",
    consecutiveDays: "jours consécutifs",
    quoteLabel: "Parole du jour",
    streakLostTitle: "Série perdue !",
    streakLostDesc: "Récupère ta série pour 15 pièces (tu avais {n} jours)",
    streakRecover: "🔥 Récupérer",
    retryTitle: "Ton dernier exercice",
    // Priorité 1 de R-31 : une révision espacée est échue. Elle passe avant tout le reste.
    reviewOverline: "À réviser",
    // Étude 22 R-4 : la bannière de rentrée. « Je reste » ferme pour la saison, sans rien
    // écrire côté serveur — un refus d'une année ne doit pas peser sur la suivante.
    backToSchool: {
      title: "C'est la rentrée !",
      body: "Tu prépares toujours la {current} ? Si tu passes en {next}, ton XP, tes badges et ta série te suivent — et tu retrouveras la {current} quand tu voudras réviser.",
      cta: "Passer en {next}",
      ctaSection: "Choisir ma section de {next}",
      stay: "Je reste en {current}",
    },
    reviseGateway: "Réviser la {name}",
    reviseGatewayHint: "Tes acquis y sont intacts.",
    changeClass: "Changer de classe",
    changeClassHint: "Tu es en {name}. Tes XP, pièces et badges te suivent.",
    reviewTitle: "Ta révision du jour",
    // Étude 04 A1.1 : être à jour est une VICTOIRE, pas un écran vide.
    dailyPlanEmpty: "Rien à réviser aujourd'hui — ta mémoire tient bon. Continue !",
    dailyPlanReason: "Tu n'as pas revu {chapter} depuis {n} jours",
    dailyPlanReasonOneDay: "Tu n'as pas revu {chapter} depuis 1 jour",
    dailyPlanReasonToday: "{chapter} est à revoir aujourd'hui",
    dailyPlanWeak: "Un point faible repéré ici",
    dailyPlanCta: "Réviser",
    // Carte de compétences (étude 07, lot 4). L'état vide invite, il ne reproche pas.
    weakSpotsTitle: "Tes points faibles",
    weakSpotsSubtitle: "Les erreurs qui reviennent le plus — celles-là, on les corrige.",
    weakSpotOccurrences: "{n} fois",
    weakSpotTrendImproving: "ça s'améliore",
    weakSpotTrendWorsening: "ça revient",
    weakSpotTrendStable: "stable",
    weakSpotCourseCta: "Revoir le cours",
    competencyTitle: "Ta carte de compétences",
    competencyEmpty: "Joue quelques exercices et ta carte de compétences s'allumera ici.",
    competencyEvaluating: "en cours d'évaluation",
    competencyBlockersTitle: "Ce qui te bloque en {competency}",
    competencyBlockersTitleGeneric: "Ce qui te bloque",
    competencyTrainCta: "S'entraîner",
    continueLabel: "Continuer",
    dungeonDesc: "Toutes matières mélangées",
    dailyQuests: "Quêtes Quotidiennes",
    weeklyQuests: "Quêtes Hebdomadaires",
    objectiveTypes: {
      "3_exercises": "Complète 3 exercices",
      "15_min_study": "15 minutes d'étude",
      perfect_score: "Réussis un sans-faute",
    },
    questTypes: {
      // `beat_2_bosses` est le SEUL type de quête hebdo que la base crée réellement
      // (20260604200000) ; sans ce libellé l'élève lisait le repli brut « beat 2 bosses »,
      // en anglais, dans les trois langues. Les autres clés couvrent des codes hérités.
      beat_2_bosses: "Bats 2 boss",
      "5_day_streak": "Tiens une série de 5 jours",
      "2_boss_exercises": "Bats 2 boss",
      "10_exercises": "Complète 10 exercices",
      all_subjects: "Entraîne-toi dans toutes les matières",
    },
    dailyEmpty: "Complète des exercices pour débloquer les objectifs quotidiens.",
    weeklyEmpty: "Les quêtes hebdomadaires apparaissent avec ta progression.",
    zoneToday: "Aujourd'hui",
    zoneProgress: "Ta progression",
    zoneExplore: "Explorer",
    pathsTitle: "Voies à conquérir",
    boutiqueTitle: "Boutique & récompenses",
    boutiqueSubtitle: "Dépense tes pièces, équipe tes objets et admire les badges gagnés.",
    boutiqueCard: "Boutique & badges",
    boutiqueCardDesc: "Objets, potions, skins et ta collection de badges.",
    discoverTitle: "Découvrir d'autres thèmes",
    discoverDesc: "Culture générale, langues, logique… au-delà de ton parcours.",
    notAttempted: "Pas encore tenté",
    failedLoad: "Impossible de charger le dashboard",
    failedLoadDesc: "Quelque chose s'est mal passé. Réessaie.",
    badgesTitle: "Badges du Héros",
    badgeTag: "Badge",
    badgeDefaultReason: "Récompense débloquée.",
    badgeEarnedOn: "Obtenu",
    badgesEmpty:
      "Aucun badge débloqué pour l'instant. Continue tes quêtes pour remplir ta collection.",
    shopTitle: "Boutique de l'Académie",
    shopDefaultDesc: "Objet de l'Académie.",
    shopEquipped: "Équipé",
    shopOwned: "Possédé",
    shopInStock: "En stock x{n}",
    shopBuy: "Acheter",
    shopEquip: "Équiper",
    shopActivate: "Activer",
    armedPassive: "Actif · protège ta série",
    armedQuest: "Actif · prochaine quête",
    itemTypes: { skin: "Skin", potion: "Potion", booster: "Booster", shield: "Bouclier" },
    rarities: { common: "Commun", rare: "Rare", epic: "Épique", legendary: "Légendaire" },
    radarTitle: "Radar de Réussite",
    radarCaption: "Tes scores moyens par attribut.",
    inventoryTitle: "Inventaire",
    inventoryEmpty: "Ton inventaire est encore vide.",
    purchaseSuccess: "{name} ajouté à l'inventaire.",
    purchaseFailed: "Achat impossible",
    equipSuccess: "{name} équipé.",
    equipFailed: "Équipement impossible",
    itemArmedPassive: "{name} activé · protège ta série.",
    itemArmedQuest: "{name} activé · prochaine quête.",
    activationFailed: "Activation impossible",
    streakRecovered: "🔥 Série récupérée ! Tu as maintenant {n} {unit}.",
    recoveryFailed: "Récupération impossible",
    familyGoalTitle: "Objectif famille",
    familyGoalProgress: "{done}/{target} missions cette semaine",
    familyGoalReached: "Objectif atteint, bravo ! 🎉",
    familyGoalHint: "Fixé par ton parent — montre-lui ce que tu sais faire !",
    welcomeBack: "Bon retour,",
    firstRunWelcome: "Bienvenue dans l'aventure,",
    resumeSubtitle: "Reprends là où tu t'es arrêté",
  },
  quest: {
    title: "Quête · Na9ra Nal3ab",
    preparing: "Préparation de l'arène…",
    leaveQuest: "Quitter la quête",
    bossFight: "⚔️ Combat de Boss",
    bossHp: "HP du Boss",
    bossStrike: "Prends le temps qu'il te faut : le chrono décide des dégâts, pas de la fin !",
    bossRankCritical: "☄️ Coup critique — le Boss est à terre !",
    bossRankFast: "⚔️ Frappe solide",
    bossRankSteady: "🛡️ Combat mené jusqu'au bout",
    bossResultHp: "Boss à {hp} % de HP",
    bossSpeedBonus: "⚡ Prime de rapidité : +{pct} % d'XP",
    bossRankHint: "Le chronomètre décide des dégâts, jamais de la correction.",
    questionOf: "Question {current} / {total}",
    feedbackMsg:
      "Choisis ta réponse — tu peux en changer — puis clique sur Valider. La correction détaillée apparaît à la fin de la quête.",
    selectedAnswer: "Réponse sélectionnée",
    nextQuestion: "Valider",
    finishQuest: "Terminer la quête",
    bossAttack: "⚔️ Attaque !",
    bossFinalBlow: "⚔️ Coup final !",
    keyboardHint: "Raccourcis : {keys1} ou {keys2} pour répondre",
    victoryTitle: "Victoire !",
    niceTriTitle: "Bien essayé, guerrier",
    xpLabel: "XP",
    coinsLabel: "Pièces",
    levelLabel: "Niveau",
    streakLabel: "Série",
    badgesUnlocked: "Badges débloqués",
    retryShieldUsed: "Bouclier utilisé — réessaie sans pénalité !",
    revealHint: "Indice ({count})",
    hintRevealed: "Indice",
    noHintsLeft: "Aucun indice disponible",
    hintUnavailable: "Pas d'indice disponible pour cette question.",
    hintUseInQuest: "À utiliser en quête",
    replayQuest: "Rejouer la quête",
    nextQuest: "Quête suivante",
    backToSubject: "Retour à la matière",
    questReview: "Révision de la quête",
    passed: "Réussi",
    needsWork: "À revoir",
    yourAnswer: "Ta réponse",
    correctAnswer: "Bonne réponse",
    reviewMistakeTitle: "Ce qui t'a fait trébucher",
    reviewCourseCta: "Revoir le cours",
    reviewTrainCta: "S'entraîner",
    resultScore: "{correct} / {total} bonnes réponses · {pct}%",
    serverValidatedTime: "Temps validé côté serveur · {n}s",
    timeSpent: "Temps · {n}s",
    quizContract: "Réussis ≥ {pct} % pour déverrouiller les missions du chapitre.",
    questionN: "Question {n}",
    potionXpApplied: "Potion XP ×{x} appliquée !",
    potionCoinsApplied: "Potion Pièces ×{x} appliquée !",
    recallBadge: "Rappel",
    recallUnlockedTitle: "Mode Rappel débloqué !",
    recallUnlockedBody:
      "Rejoue cette mission sans les options — tape tes réponses, XP ×1,5 à la clé.",
    recallPlay: "Jouer en Rappel",
    recallChip: "🧠 Rappel",
    recallLockedHint: "Termine d'abord la mission à 100 %",
    recallLockedHintAnon: "Connecte-toi et finis la mission à 100 %",
  },
  levelUp: {
    title: "Niveau supérieur !",
    tapContinue: "Touche pour continuer",
  },
  exam: {
    title: "Examens blancs",
    subtitle:
      "L'épreuve en conditions réelles : plusieurs matières, un chrono, aucune aide — la correction vient après.",
    none: "Aucun examen blanc n'est ouvert pour l'instant.",
    loading: "Préparation de l'épreuve…",
    notFound: "Cet examen est introuvable.",
    back: "Retour aux examens",
    duration: "{min} min",
    papers: "{n} épreuves",
    maxPoints: "{n} points",
    startRanked: "Passer l'examen",
    startPractice: "Refaire en entraînement",
    resume: "Reprendre ma copie",
    done: "Rendu",
    noHelp: "Aucune correction pendant l'épreuve.",
    byPaper: "Par épreuve",
    coefficient: "coef. {n}",
    questionOf: "Question {i} sur {n}",
    previous: "Épreuve précédente",
    next: "Épreuve suivante",
    answered: "{n} / {total} réponses enregistrées",
    saving: "Enregistrement…",
    saveFailed: "Sauvegarde en attente",
    submit: "Rendre ma copie",
    resultTitle: "Copie corrigée",
    scoreOn20: "Note / 20",
    scorePoints: "{score} / {max} points",
    rewards: "+{xp} XP · +{coins} pièces",
    practiceNoRank: "Entraînement : ni classement ni XP.",
    percentile: "Tu fais mieux que {pct} % des candidats.",
    percentilePending: "Classement affiché à partir de {n} candidats.",
    review: "Corrigé",
    yourAnswer: "Ta réponse",
    correctAnswer: "Réponse attendue",
    noAnswer: "Sans réponse",
    retryPractice: "Refaire en entraînement",
  },
  dungeon: {
    heroesHall: "Hall des Héros",
    title: "Le Donjon Infini",
    desc: "Descends étage par étage. Les questions viennent de ta classe, de plus en plus difficiles. Une mauvaise réponse et c'est fini. Jusqu'où peux-tu aller ?",
    poolGrade: "Questions de ta classe",
    poolCycle: "Questions de ton cycle",
    poolAll: "Questions de tout le catalogue",
    xpPerFloor: "XP / étage",
    coinsPerFloors: "Pièces / 5 étages",
    life: "Vie",
    enterDungeon: "Entrer dans le Donjon",
    collapsed: "Donjon effondré",
    fellAt: "Tu es tombé à l'étage {n}.",
    fatalQuestion: "Question fatale",
    yourAnswer: "Ta réponse",
    correctAnswer: "Bonne réponse",
    floors: "Étages",
    xp: "XP",
    coins: "Pièces",
    correct: "Correct",
    backToHall: "Retour au hall",
    retryDungeon: "Rejouer le donjon",
    descending: "Descente vers l'étage {n}…",
    leaveDungeon: "Quitter le donjon",
    floor: "Étage {n}",
    oneHp: "1 PV",
    warning: "Une mauvaise réponse met fin à la course. Choisis bien.",
    validate: "⚔️ Valider",
    correctMsg: "Correct ! Descente en cours…",
    wrongMsg: "Faux ! Le donjon s'effondre…",
    depth: "Profondeur",
    errorSavingRun: "Erreur en sauvegardant la course",
    noMoreQuestions: "Plus aucune question disponible dans le donjon. Finalisation de ta course…",
    failedLoadQuestions: "Impossible de charger les questions du donjon",
    failedStartRun: "Impossible de démarrer la course",
    failedValidateAnswer: "Impossible de valider la réponse",
    loadAccessError: "Impossible de vérifier l'accès au Donjon. Vérifie ta connexion et réessaie.",
    enterDungeonAria: "Entrer dans le mode donjon infini",
    locked: "Donjon verrouillé",
    dailyLimitReached: "Limite quotidienne atteinte ({max}/jour). Reviens demain.",
    levelLocked: "Monte de niveau pour débloquer le Donjon.",
    prereqLocked: "Le Donjon exige d'avoir déjà progressé dans plusieurs matières et chapitres.",
    subjectsStarted: "Matières entamées : {done}/{required}",
    chaptersStarted: "Chapitres entamés : {done}/{required}",
    keepTraining: "Continuer à m'entraîner",
    runsToday: "Courses aujourd'hui : {n}/{max}",
  },
  duel: {
    title: "Duels",
    subtitle: "Défie un adversaire de ton parcours sur les mêmes questions.",
    errorSearch: "Recherche impossible pour le moment. Réessaie.",
    findOpponent: "Trouver un adversaire",
    searching: "Recherche d'un adversaire…",
    cancel: "Annuler",
    activeDuels: "Duels en cours",
    history: "Historique",
    noHistory: "Aucun duel pour l'instant. Lance-toi !",
    rulesFirstDone: "Mêmes questions pour les deux : le premier à finir juste l'emporte.",
    rewardWin: "Victoire",
    rewardDraw: "Nul",
    rewardLoss: "Participation",
    rewardsHint: "Tu gagnes des XP même en perdant — participer paie toujours.",
    play: "Jouer",
    resume: "Reprendre",
    forfeit: "Abandonner",
    forfeitDone: "Duel abandonné.",
    opponent: "Adversaire",
    statusActive: "En cours",
    statusFinished: "Terminé",
    statusExpired: "Expiré",
    statusPending: "En attente",
    questionOf: "Question {n}/{total}",
    validate: "Valider",
    tooFast: "Trop rapide — réponse non comptée.",
    waitingOpponent: "En attente de l'adversaire…",
    opponentAnswered: "Adversaire : {n}/{total}",
    opponentDone: "L'adversaire a terminé.",
    online: "En ligne",
    offline: "Hors ligne",
    resultWin: "Victoire !",
    resultLoss: "Défaite",
    resultDraw: "Égalité",
    resultPending: "En attente du résultat",
    finalScore: "Ton score : {score}/{total}",
    reviewTitle: "Correction",
    correctAnswer: "Bonne réponse : {answer}",
    backToHub: "Retour aux duels",
    loading: "Chargement…",
    notFound: "Duel introuvable.",
    emptyHubHint: "Le premier arrivé attend un adversaire du même parcours.",
    leagueTitle: "Ligue de la semaine",
    leagueSubtitle: "Classement des duellistes · victoire 3 pts, nul 1 pt.",
    leagueEmpty: "Aucun duel terminé cette semaine. Sois le premier !",
    leaguePts: "{n} pts",
    lastAward: "Semaine dernière : {tier} · +{coins} 🪙",
    tierDiamond: "Diamant",
    tierPlatinum: "Platine",
    tierGold: "Or",
    tierSilver: "Argent",
    tierBronze: "Bronze",
  },
  // Tuteur déterministe (étude 30 lot 3). Le serveur rend un identifiant, ces chaînes le
  // mettent en langue — et aucune n'affiche une probabilité de croyance (D-1).
  adaptive: {
    state: {
      maitrisee: "Maîtrisée",
      "en-cours": "En cours",
      fragile: "Fragile",
      lacune: "Lacune",
      inconnue: "Pas encore vue",
    },
    stateHint: {
      maitrisee: "Tu l'as prouvée plusieurs fois, et de plusieurs façons.",
      "en-cours": "Ça vient. Encore une ou deux preuves et c'est acquis.",
      fragile: "Tu l'as vue, mais ça ne tient pas encore.",
      lacune: "Celle-là bloque la suite. On la reprend depuis le début.",
      inconnue: "On ne l'a pas encore croisée ensemble.",
    },
    zone: {
      interieur: "Acquis",
      frontiere: "Prêt à apprendre",
      "hors-portee": "Il manque une base",
    },
    mapTitle: "Où tu en es",
    mapSubtitle: "Ce que tu maîtrises, ce que tu peux attaquer, ce qui te bloque.",
    mapEmpty: "Joue quelques exercices : ta carte s'allumera au fur et à mesure.",
    provenBy: "prouvé {n} fois, sous {m} formes",
    inferredBadge: "Déduit",
    inferredExplain:
      "Tu as réussi {source} : je considère celle-ci acquise. Dis-moi si je me trompe.",
    disputeCta: "Je ne suis pas d'accord",
    disputePending: "On note…",
    disputeDone: "C'est noté — on te la reposera pour en avoir le cœur net.",
    suspectBadge: "À revoir",
    frontierTitle: "Prêt à apprendre",
    frontierSubtitle: "Tu as tout ce qu'il faut pour attaquer ces compétences maintenant.",
    frontierEmpty: "Rien de neuf à attaquer pour l'instant — continue ce que tu as commencé.",
    frontierUnlocks: "ouvre {n} suites",
    frontierUnlocksNone: "à ton rythme",
    frontierStartCta: "Commencer",
    blockedNotice:
      "Il te manque une base pour celle-ci. Tu peux quand même essayer — mais on te conseille de reprendre en amont d'abord.",
  },
  tutor: {
    ask: "Demander au Prof",
    panelTitle: "El Ostedh",
    thinking: "El Ostedh réfléchit…",
    again: "Explique autrement",
    againExhausted: "On a fait le tour — regarde le cours, puis reviens me voir",
    gotIt: "J'ai compris",
    helpful: "Ça m'a aidé",
    notHelpful: "Pas clair",
    rated: "Merci, c'est noté",
    close: "Fermer",
    fromCache: "Réponse déjà préparée",
    offTitle: "El Ostedh n'est pas là",
    offBody: "Le mode IA n'est pas activé sur ce compte. Demande à tes parents.",
    noEnergyTitle: "El Ostedh revient demain",
    noEnergyBody: "Tu as utilisé toute ton énergie du jour. Le cours, lui, est toujours là !",
    pausedTitle: "El Ostedh est en pause",
    pausedBody: "Il ne répond pas pour l'instant. Réessaie dans un moment.",
    lockedSession: "Termine d'abord ta mission — après, on en parle",
    lockedDungeon: "Pas pendant un donjon ! On en parle à la sortie",
    lockedDuel: "Pas pendant un duel ! On en parle après",
    lockedNotAttempted: "Réponds d'abord, et je t'explique",
    coach: {
      signature: "El Ostedh",
      weak1: "Là, tu te trompes encore — dix minutes et c'est réglé.",
      weak2: "C'est ici que ça coince vraiment. On s'y met ?",
      late1: "Ça fait un moment : ta mémoire commence à lâcher là-dessus.",
      late2: "Le plus urgent du jour, c'est celui-là.",
      due1: "Un petit tour dessus, et tu le gardes.",
      due2: "C'est le bon moment pour le revoir.",
      today1: "C'est prévu pour aujourd'hui — profite que ce soit frais.",
      today2: "Cinq minutes maintenant valent une heure plus tard.",
      comeback1: "Content de te revoir ! On reprend tranquillement.",
      comeback2: "Ça faisait longtemps — on reprend là où tu t'étais arrêté.",
      streak1: "Belle série ! Ne la casse pas aujourd'hui.",
      streak2: "Tu es régulier, et ça se voit dans tes résultats.",
      clear1: "Rien à réviser aujourd'hui : tu es à jour. Bravo !",
      clear2: "Tout est à jour. Tu peux partir à l'aventure.",
      steady1: "Prêt ? On commence par le plus utile.",
      steady2: "Voilà ce que je te conseille aujourd'hui.",
    },
    planPushTitle: "Le rappel d'El Ostedh",
    planPushDesc:
      "Un seul rappel par jour, et seulement si tu as quelque chose à réviser. Tu peux l'arrêter quand tu veux.",
    chat: {
      open: "Poser une question sur ce chapitre",
      title: "En parler avec El Ostedh",
      intentExplain: "Explique-moi ce chapitre",
      intentExample: "Donne-moi un exemple",
      intentSummarize: "Résume-le en 3 points",
      placeholder: "Ta question sur ce chapitre…",
      send: "Envoyer",
      tooLong: "Ta question est un peu longue — dis-la en 300 caractères max.",
      noLinks: "Pas de lien ici : pose ta question avec tes mots.",
      rateLimited: "Doucement ! Attends un instant avant de reposer une question.",
      empty: "Choisis une question, ou écris la tienne.",
      you: "Toi",
      historyTitle: "Tes conversations",
      historyEmpty: "Tu n'as encore parlé de rien avec El Ostedh.",
      outputRejected: "Cette réponse n'était pas nette — redemande-la autrement.",
    },
    miniCheck: {
      title: "Vérifions ensemble",
      start: "Vérifions ensemble",
      loading: "El Ostedh cherche une question…",
      submit: "Je valide",
      correctTitle: "C'est ça !",
      correctBody: "Tu as compris. On peut passer à la suite.",
      wrongTitle: "Pas encore",
      wrongBody: "Ce n'est pas grave : on va s'y prendre autrement.",
      unavailable: "On verra ça une autre fois — continue ton chapitre.",
    },
    escalation: {
      reteach: "Je vais te l'expliquer d'une autre façon.",
      lesson: "Allons relire le passage du cours ensemble.",
      prerequisite: "Il manque une base juste avant. On la reprend d'abord ?",
      plan: "Je l'ajoute à ton plan du jour : on y revient demain, à tête reposée.",
      parentDigest: "J'en parle à tes parents cette semaine, pour qu'ils t'aident aussi.",
      cta: "D'accord",
    },
    practice: {
      cta: "Entraîne-moi là-dessus",
      loading: "Je cherche de quoi t'entraîner…",
      onTargetHint: "J'ai trouvé de quoi retravailler exactement cette erreur.",
      offTargetHint: "Je n'ai rien d'exactement ciblé — on s'entraîne sur des questions voisines.",
      forgingHint: "Il n'y a rien d'assez frais là-dessus : je t'en écris de nouvelles.",
      noChapter: "Je ne sais pas encore dans quel chapitre travailler cette erreur.",
      noMaterial: "Je n'ai rien à te proposer là-dessus pour l'instant.",
    },
    digest: {
      title: "Ton bilan de la semaine",
      parentTitle: "Le bilan de la semaine",
      weekLabel: "Semaine du {date}",
      notYet: "Ton bilan arrive dimanche — tout ce que tu joues cette semaine s'y retrouvera.",
      parentNotYet: "Le bilan de la semaine est écrit le dimanche soir.",
      unavailable:
        "Le bilan n'a pas pu s'afficher cette semaine. Tes progrès, eux, sont bien enregistrés.",
      notLinked:
        "Le lien avec ce compte élève n'est plus actif. Rétablissez-le pour recevoir les bilans.",
    },
    energy: {
      title: "Ton énergie du jour",
      hint: "Chaque question à El Ostedh en utilise un peu. Elle revient au complet demain.",
      bonus: "+{n} regagnés aujourd'hui avec tes indices.",
      rechargeCta: "Échanger un indice (+{n})",
      rechargeBusy: "Un instant…",
      recharged: "C'est fait : +{n} d'énergie !",
      rechargedWithItem: "Tu as échangé « {item} » : +{n} d'énergie !",
      atCap: "Tu es déjà au maximum pour aujourd'hui — ton indice n'a pas été pris.",
      noItem: "Tu n'as pas d'indice à échanger. Tu en gagnes en jouant des quêtes.",
      failed: "Ça n'a pas marché pour l'instant. Réessaie dans un moment.",
    },
    cacheStats: {
      title: "Cache mutualisé et rebut",
      hitTitle: "Explications resservies",
      hitDetail: "{hits} resservies · {misses} écrites",
      discardTitle: "Rebut de la Forge",
      discardDetail: "{discarded} rejetées · {kept} gardées",
      window: "sur les explications produites ces {n} derniers jours",
      lifetime: "depuis toujours",
      unavailable: "Mesure indisponible pour le moment.",
    },
  },
  subscription: {
    premiumTitle: "Fonctionnalité Premium",
    premiumDesc:
      "L'accès premium se débloque par parcours Concours, accordé par l'administrateur (pas d'abonnement mensuel). Contacte-le pour débloquer ton parcours.",
    contactTitle: "Pour débloquer un parcours, contacte l'administrateur",
    statusActive: "Actif",
    statusInactive: "Inactif",
    none: "—",
    perpetual: "Permanent",
    colUser: "Utilisateur",
    colStatus: "Statut",
    colParcours: "Parcours",
    colSource: "Source",
    colExpires: "Expire le",
    colActions: "Actions",
    // Entitlement sources
    purchase: "Achat",
    beta: "Bêta",
    gift: "Cadeau",
    family: "Famille",
    adminTitle: "Gestion des accès parcours",
    adminDesc: "Accorde ou révoque l'accès premium parcours par parcours.",
    accessDenied: "Accès réservé aux administrateurs.",
    grantTitle: "Accorder un accès parcours",
    grantUserId: "Identifiant utilisateur",
    grantUserIdPlaceholder: "UUID de l'utilisateur",
    grantParcours: "Parcours",
    grantSource: "Source",
    grantMonths: "Durée (mois)",
    grantMonthsPlaceholder: "Permanent si vide",
    grant: "Accorder",
    revoke: "Révoquer",
    granted: "Accès parcours accordé.",
    revoked: "Accès parcours révoqué.",
    entitlementsEmpty: "Aucun accès parcours pour l'instant.",
    updateError: "Échec de la mise à jour de l'accès parcours.",
  },
  betaAccess: {
    cta: "Demander un accès gratuit (bêta testeur)",
    title: "Accès bêta testeur",
    desc: "Teste gratuitement toutes les fonctionnalités premium en tant que bêta testeur.",
    nameLabel: "Nom",
    emailLabel: "Email",
    motivationLabel: "Motivation (optionnel)",
    motivationPlaceholder: "Pourquoi veux-tu tester ?",
    submit: "Envoyer la demande",
    submitting: "Envoi…",
    sentTitle: "Demande envoyée ✓",
    sentDesc: "Ta demande a bien été envoyée. Tu seras informé après examen.",
    statusLabel: "Statut de ta demande",
    statusPending: "En attente",
    statusApproved: "Approuvée",
    statusRejected: "Refusée",
    pendingDesc: "Ta demande est en cours d'examen par l'administrateur.",
    approvedDesc: "Félicitations ! Ton accès premium gratuit est activé.",
    rejectedDesc: "Ta demande n'a pas été retenue cette fois-ci.",
    cancel: "Annuler",
    error: "Échec de l'envoi. Réessaie.",
    adminTitle: "Demandes d'accès bêta",
    adminDesc: "Approuve ou refuse les demandes d'accès premium gratuit.",
    pendingCount: "en attente",
    empty: "Aucune demande pour l'instant.",
    colName: "Nom",
    colEmail: "Email",
    colMotivation: "Motivation",
    colDate: "Date",
    colStatus: "Statut",
    colActions: "Actions",
    approve: "Approuver",
    reject: "Refuser",
    approveOk: "Demande approuvée — accès premium activé.",
    rejectOk: "Demande refusée.",
    reviewError: "Échec du traitement de la demande.",
  },
  contentReport: {
    cta: "Signaler une erreur",
    label: "Décris l'erreur",
    placeholder: "Ex. la bonne réponse devrait être… / la question est ambiguë…",
    send: "Envoyer",
    cancel: "Annuler",
    sent: "Merci ! Signalement envoyé.",
    error: "Échec de l'envoi. Réessaie.",
    adminTitle: "Signalements d'erreurs",
    adminDesc: "Examine et traite les erreurs signalées par les élèves.",
    openCount: "à traiter",
    empty: "Aucun signalement pour l'instant.",
    unknownExercise: "Exercice inconnu",
    reportCount: "{n} signalements",
    viewContent: "Voir le contenu",
    statusOpen: "Ouvert",
    statusResolved: "Résolu",
    statusDismissed: "Rejeté",
    resolve: "Résolu",
    dismiss: "Rejeter",
    updated: "Signalement mis à jour.",
    updateError: "Échec du traitement du signalement.",
  },
  betaBanner: {
    badge: "Bêta",
    tooltip: "Plateforme en phase de test — merci de signaler tout problème rencontré.",
  },
  bugReport: {
    launcherLabel: "Signaler un bug",
    title: "Signaler un bug",
    desc: "La plateforme est en phase de test. Décris le problème rencontré, on s'en occupe.",
    label: "Décris le problème",
    placeholder: "Ex. la page reste bloquée quand… / le bouton ne répond pas…",
    send: "Envoyer",
    cancel: "Annuler",
    sent: "Merci ! Bug signalé.",
    error: "Échec de l'envoi. Réessaie.",
    adminTitle: "Bugs signalés",
    adminDesc: "Examine et traite les bugs signalés par les utilisateurs.",
    openCount: "à traiter",
    empty: "Aucun bug signalé pour l'instant.",
    unknownPage: "Page inconnue",
    statusOpen: "Ouvert",
    statusResolved: "Résolu",
    statusDismissed: "Rejeté",
    resolve: "Résolu",
    dismiss: "Rejeter",
    updated: "Bug mis à jour.",
    updateError: "Échec du traitement du bug.",
  },
  pushNotifications: {
    cardTitle: "Rappels de quête",
    cardDesc: "Reçois une alerte quand ta série est en danger pour ne jamais la casser.",
    enable: "Activer les notifications",
    enabling: "Activation…",
    enabled: "Notifications activées",
    disable: "Désactiver",
    blocked: "Notifications bloquées. Autorise-les dans les réglages de ton navigateur.",
    errorGeneric: "Impossible d'activer les notifications. Réessaie.",
  },
  layout: {
    heroesHall: "Hall des Héros",
    parcours: "Parcours",
    themes: "Découvrir",
    dungeon: "Donjon",
    arena: "Arène",
    ranking: "Classement",
    parentReport: "Suivi",
    admin: "Admin",
    subscriptions: "Abonnements",
    betaRequests: "Demandes bêta",
    contentReports: "Signalements",
    bugReports: "Bugs",
    parcoursInterest: "Intérêts",
    economy: "Économie",
    console: "Console",
    signOut: "Déconnexion",
    logoutToast: "À bientôt, guerrier.",
  },
  theme: {
    label: "Thème",
    reference: "Clair",
    dark: "Sombre",
  },
  sound: {
    label: "Sons",
    effects: "Effets sonores",
    music: "Musique d'ambiance",
    enable: "Activer les sons",
    disable: "Couper les sons",
  },
  settings: {
    title: "Paramétrage",
    subtitle: "Tout ce qui se règle, au même endroit.",
    menuAria: "Ouvrir le paramétrage",
    quickTitle: "Réglages rapides",
    allSettings: "Tous les paramètres",
    accountTitle: "Mon compte",
    accountDesc: "Ton identité, ton code d'alliance et ta déconnexion.",
    email: "Adresse e-mail",
    pseudoAction: "Changer",
    pseudoSave: "Enregistrer",
    pseudoCancel: "Annuler",
    pseudoSaved: "Te voilà renommé.",
    pseudoError: "Le changement a échoué. Réessaie dans un instant.",
    pseudoRule: "De 1 à {max} caractères, sans symboles invisibles.",
    avatar: "Avatar",
    avatarAction: "Choisir dans la Boutique",
    allianceCode: "Code Alliance",
    allianceCopy: "Copier",
    allianceCopied: "Copié",
    allianceHint: "Partage ce code avec ton parent pour activer le suivi mentor.",
    password: "Mot de passe",
    passwordAction: "Recevoir un lien de changement",
    passwordSent: "Un lien de changement vient de partir vers ton adresse.",
    passwordError: "L'envoi a échoué. Réessaie dans un instant.",
    parcoursTitle: "Mon parcours",
    parcoursDesc: "Le niveau que tu suis aujourd'hui.",
    parcoursCurrent: "Parcours actif",
    parcoursNone: "Aucun parcours actif",
    parcoursChange: "Changer de parcours",
    displayTitle: "Affichage",
    languageLabel: "Langue",
    displayDesc: "Thème et langue de l'interface.",
    soundTitle: "Sons & notifications",
    soundDesc: "Effets, musique et rappels.",
    helpTitle: "Aide & à propos",
    helpDesc: "Signaler un problème et consulter les mentions.",
    helpTerms: "Conditions d'utilisation",
    helpPrivacy: "Confidentialité",
    dangerTitle: "Zone sensible",
    dangerDesc: "Ce qu'un clic ne rattrape pas.",
    deleteAccount: "Supprimer mon compte",
    deleteRowLabel: "Effacement définitif",
    deleteDialogTitle: "Supprimer définitivement ton compte ?",
    deleteDialogWhat:
      "Tout part avec lui : ta progression, ton XP, tes séries, tes badges, tes duels, et le suivi partagé avec tes parents.",
    deleteDialogWhatParent:
      "Tout part avec lui : le lien avec le ou les élèves que tu suis, et les objectifs que tu leur as fixés. Leurs comptes à eux ne sont pas touchés.",
    deleteDialogFinal: "C'est immédiat et sans retour : rien ne pourra être restauré.",
    deleteConfirmLabel: "Pour confirmer, écris l'adresse e-mail du compte :",
    deleteCancel: "Annuler",
    deleteConfirm: "Supprimer définitivement",
    deleteBusy: "Suppression…",
    deleteDone: "Ton compte et tout ce qu'il contenait ont été supprimés.",
    deleteErrors: {
      email_mismatch: "L'adresse saisie ne correspond pas à celle du compte.",
      generic: "La suppression n'a pas abouti. Réessaie dans un instant.",
    },
  },
  adminHub: {
    title: "Console",
    subtitle: "Les consoles d'administration, regroupées.",
  },
  encouragement: {
    combo: "Combo x{n} !",
    nice: "Bien joué !",
    onFire: "En feu ! 🔥",
    unstoppable: "Imparable ! ⚡",
    legendary: "Légendaire ! 👑",
  },
  parcours: {
    worldTitle: "Carte de l'aventure",
    worldSubtitle: "Choisis ta voie, avance à ton rythme et deviens une légende.",
    level: "Niveau",
    xpToNext: "XP avant le niveau suivant",
    maxLevel: "Niveau max atteint",
    start: "Commencer",
    continueLabel: "Continuer",
    review: "Revoir",
    nodeNext: "Conseillé",
    done: "Terminé",
    current: "En cours",
    xpToEarn: "XP à gagner",
    chapters: "chapitres",
    backToMap: "Retour à la carte",
    empty: "Aucune étape pour l'instant.",
  },
  explorer: {
    heading: "Choisis ton parcours",
    subtitle: "Suis le programme scolaire tunisien ou explore librement les voies de l'Académie.",
    concoursTitle: "Programme scolaire",
    libreTitle: "Exploration libre",
    unlocked: "Débloqué",
    comingSoon: "Bientôt disponible",
    switching: "Changement de parcours…",
    switchError: "Impossible de changer de parcours. Réessaie.",
    failedLoad: "Impossible de charger les parcours.",
    empty: "Aucun parcours disponible pour le moment.",
  },
  onboarding: {
    intentTitle: "Que veux-tu faire ?",
    intentSubtitle: "Choisis ta voie, jeune aspirant.",
    concoursTitle: "Suis le programme scolaire",
    concoursDesc: "Trouve ta classe, de la 1ère année de base au Baccalauréat.",
    exploreTitle: "J'explore librement",
    exploreDesc: "Découvre les thèmes à ton rythme, sans pression.",
    parcoursTitleConcours: "Quelle classe prépares-tu cette année ?",
    parcoursTitleLibre: "Quel thème veux-tu explorer ?",
    parcoursSubtitleConcours:
      "Tu pourras réviser les classes précédentes à tout moment, et changer quand tu veux sans rien perdre. Les classes en construction arrivent bientôt — dis-nous lesquelles t'intéressent en priorité.",
    parcoursSubtitleLibre: "Lance-toi dans l'aventure de ton choix.",
    saving: "Enregistrement…",
    saveError: "Impossible d'enregistrer ton parcours. Réessaie.",
    promise: "Bienvenue ! On configure ton espace en deux clics.",
    chooseAnyway: "Choisir cette classe",
    celebrateTitle: "C'est parti !",
    celebrateDesc: "Bienvenue dans {parcours}. Ton tableau de bord t'attend.",
    celebrateSoonTitle: "Ta classe arrive bientôt",
    celebrateSoonDesc:
      "{parcours} est en construction. En attendant, explore les extras — et ton vote la fait avancer.",
    celebrateExtrasCta: "Explorer les extras",
    celebrateDashboardCta: "Voir mon tableau de bord",
  },
  cycles: {
    primaire: "Primaire",
    college: "Collège",
    secondaire: "Secondaire",
  },
  lycee: {
    year2: "2ème année",
    year3: "3ème année",
    yearBac: "Baccalauréat",
    sections: "{n} sections",
    available: "{n} disponibles",
    pickSection: "Choisis ta section",
  },
  parcoursInterest: {
    cta: "Ça m'intéresse",
    interested: "Intéressé·e ✓",
    count: "{count} intéressé·e·s",
    underConstruction: "En construction",
    toggleError: "Impossible d'enregistrer ton intérêt. Réessaie.",
    loadError: "Impossible de charger le classement des intérêts.",
    adminTitle: "Intérêt pour les programmes",
    adminDesc:
      "Classement des classes « en construction » par nombre d'intéressés — priorise la plus demandée.",
    colProgram: "Programme",
    colInterest: "Intéressés",
    empty: "Aucun intérêt enregistré pour l'instant.",
  },
  discover: {
    heading: "Découvrir nos programmes",
    subtitle: "Choisis ta voie : survole un programme, puis clique pour explorer.",
    center: "Nos programmes",
    enter: "Entrer",
    back: "Retour",
    families: {
      tunisien: "Programme officiel tunisien",
      langues: "Langues",
      culture: "Culture générale",
      cerveau: "Muscle ton cerveau",
      ib: "IB — Bac international",
    },
    langShort: { anglais: "Anglais", francais: "Français", arabe: "Arabe" },
    hintClasses: "{count} classes",
    hintLanguages: "{count} langues",
    hintSoon: "Bientôt",
    hintExplore: "Explorer",
    backToPrograms: "Retour aux programmes",
    start: "Commencer",
    comingSoonTitle: "Bientôt disponible",
    comingSoonDesc:
      "Ce programme est en cours de construction. Dis-nous s'il t'intéresse pour qu'on le priorise.",
    familyDesc: {
      tunisien:
        "Le programme scolaire officiel tunisien, de la 1ère année de base au Baccalauréat.",
      langues: "Progresse en anglais, français et arabe, niveau par niveau (CECRL).",
      culture: "Élargis ta culture générale : histoire, géographie, sciences, arts et plus encore.",
      cerveau: "Logique, mémoire, calcul mental : muscle ton cerveau au quotidien.",
      ib: "Le Baccalauréat International arrive bientôt.",
    },
  },
  arena: {
    title: "Arène",
    subtitle: "Mesure-toi aux autres héros : chrono, duels et classement national.",
    dungeonReward: "+{xp} XP par étage",
    duelReward: "Jusqu'à +{xp} XP par victoire — participer paie toujours",
    rankingDesc: "Situe-toi dans l'Académie, au global et par matière.",
    rankingReward: "Gagne des XP pour grimper",
  },
  leaderboard: {
    titleGradient: "Classement",
    titleRest: "de l'Académie",
    globalTab: "🌍 Global",
    myClassTab: "🎓 Ma classe",
    subtitleMyClass: "Les héros de ta classe",
    myClassEmptyTitle: "Sois le premier de ta classe",
    myClassEmptyDesc:
      "Personne de ta classe n'a encore marqué de points. Joue une mission et prends la tête.",
    subtitleGlobal: "Les héros les plus puissants de l'Académie",
    subtitleSubject: "Classement par XP · {subject}",
    loading: "Chargement du classement…",
    youChip: "Toi",
    lvl: "Niv.",
    coldStartTitle: "Le classement démarre !",
    coldStartDesc:
      "Gagne tes premiers XP pour apparaître ici — chaque quête réussie te fait grimper.",
    coldStartCta: "Commencer une quête",
    emptySubject: "Aucun score dans cette matière pour l'instant.",
  },
  parentReport: {
    adminTitle: "Admin — Suivi des élèves",
    title: "Rapport de suivi",
    adminSubtitle: "{n} élève(s) inscrits — sélectionnez pour voir le détail",
    subtitle: "Vue d'ensemble de l'activité de votre enfant",
    linkTitle: "Associer un élève via Alliance Code",
    codePlaceholder: "Code élève",
    relationPlaceholder: "Relation (parent)",
    linkCta: "Associer",
    linking: "Liaison…",
    linkHint: "L'élève trouve son code dans son paramétrage, section « Mon compte ».",
    rememberCode: "Se souvenir de ce code sur cet appareil",
    rememberHint: "Évitez sur un ordinateur partagé : ce code ouvre le bilan de votre enfant.",
    forgetCode: "Oublier ce code",
    linkSuccess: "Alliance réussie avec {name}.",
    linkFailed: "Échec de la liaison.",
    // Erreurs de code d'alliance (codes stables du serveur, registre parent = vouvoiement).
    codeErrors: {
      not_parent:
        "Votre compte n'est pas un compte parent. Reconnectez-vous avec un compte parent pour suivre un élève.",
      invalid_code: "Code invalide. Recopiez-le exactement depuis l'espace de l'élève.",
      self_link: "Ce code est celui de votre propre compte : entrez le code de l'élève.",
      not_student: "Ce code n'appartient pas à un compte élève.",
      not_found: "Aucun élève ne correspond à ce code : vérifiez-le, ou le compte a été supprimé.",
      generic_link: "Impossible d'associer cet élève. Vérifiez le code et réessayez.",
      generic_report: "Impossible d'afficher ce suivi. Vérifiez le code et réessayez.",
    },
    defaultStudentName: "Élève",
    adminEmptyTitle: "Aucun élève inscrit",
    adminEmptyDesc: "Aucun compte élève n'existe encore dans la plateforme.",
    linkFirstHint: "Associez votre premier élève avec son Alliance Code pour débloquer le suivi.",
    prevPage: "Préc",
    nextPage: "Suiv",
    pageLabel: "Page {page} / {total}",
    classLabel: "Classe :",
    classNone: "Non choisie",
    memberSince: "Membre depuis",
    timeTotal: "Temps total",
    exercisesLabel: "Exercices",
    avgScore: "Score moyen",
    activeDays: "Jours actifs (7j)",
    trendPrefix: "Tendance des scores :",
    trendSuffix: "par rapport aux exercices précédents",
    activityTitle: "Activité des 30 derniers jours",
    today: "Aujourd'hui",
    perSubjectTitle: "Performance par matière",
    exSuffix: "ex.",
    lastActivity: "Dernière activité :",
    noActivity: "Aucune activité",
    seriousness: "Score d'assiduité :",
    verdictExcellent: "Excellent",
    verdictExcellentDesc: "Votre enfant est très assidu et progresse rapidement !",
    verdictGood: "Bien",
    verdictGoodDesc: "Bonne régularité. Quelques efforts supplémentaires seront bénéfiques.",
    verdictAverage: "Moyen",
    verdictAverageDesc: "Activité irrégulière. Encouragez des sessions plus fréquentes.",
    verdictNeedsImprovement: "À améliorer",
    verdictNeedsImprovementDesc: "Peu d'activité récente. Un suivi rapproché est recommandé.",
    verdictInactive: "Inactif",
    verdictInactiveDesc: "Aucune activité détectée. Vérifiez que l'élève utilise la plateforme.",
    weekCompareTitle: "Cette semaine vs la précédente",
    weekMinutes: "Minutes",
    insightsTitle: "Points forts & à renforcer",
    insightsSubtitle: "Sur les 30 derniers jours · chapitres travaillés au moins 2 fois",
    strengthsTitle: "Points forts",
    weaknessesTitle: "À renforcer",
    strengthsEmpty: "Pas encore de point fort dégagé — encouragez une pratique régulière.",
    weaknessesEmpty: "Aucune lacune détectée ce mois-ci. Bravo !",
    namedErrorsTitle: "Les erreurs qui reviennent",
    namedErrorsSubtitle: "Ce qu'il ou elle croit à tort — c'est là qu'une explication change tout.",
    namedErrorsCount: "{n} fois",
    namedErrorsImproving: "en recul",
    namedErrorsWorsening: "en hausse",
    namedErrorsStable: "stable",
    tutorHelpTitle: "L'aide d'El Ostedh",
    tutorHelpSubtitle:
      "Combien de fois votre enfant a demandé de l'aide, et sur quels thèmes. Le contenu des échanges reste entre El Ostedh et lui.",
    tutorHelp7d: "{n} demande(s) cette semaine",
    tutorHelp30d: "{n} sur 30 jours",
    tutorHelpThemesTitle: "Les thèmes abordés",
    insightAttempts: "{n} mission(s)",
    adviceTitle: "Le conseil de la semaine",
    adviceReviewCta: "Revoir ce chapitre",
    adviceWeakness:
      "Faites réviser « {chapter} » en {subject} : relire la leçon, puis refaire les missions du chapitre.",
    adviceKeepUp:
      "Aucune lacune détectée ce mois-ci. Encouragez {name} à garder ce rythme — la régularité fait la différence.",
    adviceInactive:
      "Peu d'activité récente : proposez une petite session de 15 minutes aujourd'hui pour relancer la dynamique.",
    printCta: "Imprimer le bilan",
    shareCta: "Partager le bilan",
    printTitle: "Bilan famille — Na9ra Nal3ab",
    printGenerated: "Édité le {date}",
    pushTitle: "Recevez le bilan chaque dimanche",
    pushDesc:
      "Activez les notifications pour être prévenu·e quand le bilan hebdomadaire de votre enfant est prêt.",
    goalTitle: "Objectif de la semaine",
    goalHint:
      "Fixez un cap de missions pour cette semaine — votre enfant le verra comme une quête famille sur son tableau de bord.",
    goalSave: "Enregistrer",
    goalSaving: "Enregistrement…",
    goalSaved: "Objectif enregistré : {n} mission(s) cette semaine.",
    goalProgress: "{done}/{target} missions cette semaine",
    goalUnit: "missions / semaine",
    coverageShort: "{done}/{total} chap.",
  },
  parentDaily: {
    tabSummary: "Bilan",
    tabDaily: "Jour par jour",
    periodToday: "Aujourd'hui",
    periodYesterday: "Hier",
    periodLast7: "7 derniers jours",
    periodThisWeek: "Cette semaine",
    periodLast30: "30 derniers jours",
    periodThisMonth: "Ce mois",
    periodCustom: "Période personnalisée",
    periodFrom: "Du",
    periodTo: "au",
    periodMaxHint: "92 jours au maximum",
    q1Works: "Est-ce qu'il travaille ?",
    q1Yes: "{exercises} exercices · {lessons} cours",
    q1No: "Rien sur cette période",
    q2Serious: "Est-ce qu'il est régulier ?",
    q3Progress: "Est-ce qu'il progresse ?",
    q3Answer: "{delta} pts · moyenne {score} %",
    q4Efficient: "Son temps est-il efficace ?",
    unknown: "Pas assez de données",
    noComparison: "Pas de comparaison",
    none: "Aucune",
    measuredSinceNotice:
      "Le temps passé n'est mesuré que depuis le {date}. Avant cette date, les durées affichent 0 — ce qui ne veut pas dire que rien n'a été fait.",
    summaryTitleDay: "Résumé de la journée",
    summaryTitlePeriod: "Résumé de la période",
    appTime: "Temps dans l'application",
    appTimeHint: "navigation comprise",
    learningTime: "Temps d'apprentissage",
    learningTimeHint: "cours, exercices, révisions",
    activities: "Activités réalisées",
    firstActivity: "Première activité",
    lastActivity: "Dernière activité",
    sessions: "Sessions",
    activeDays: "Jours actifs",
    breakdownTitle: "Temps par type d'activité",
    breakdownSubtitle: "Où va réellement le temps passé dans l'application.",
    breakdownEmpty: "Aucun temps mesuré sur cette période.",
    typeLesson: "Cours",
    typeExercise: "Exercices",
    typeQuiz: "Quiz",
    typeRecall: "Révisions",
    typeArena: "Donjon & duels",
    typeBrowse: "Navigation",
    lessonsTitle: "Cours consultés",
    lessonsSubtitle: "{studied} cours réellement étudiés sur {opened} ouverts.",
    lessonsEmpty: "Aucun cours ouvert sur cette période.",
    lessonStudied: "Étudié",
    lessonOpened: "Ouvert",
    lessonViews: "{n} consultation(s)",
    lessonProgressAria: "{pct} % du cours consulté",
    exercisesTitle: "Exercices réalisés",
    exercisesSubtitle: "{passed} réussis sur {total}. Touchez un exercice pour voir les réponses.",
    exercisesEmpty: "Aucun exercice terminé sur cette période.",
    attemptNo: "tentative n° {n}",
    rightWrong: "{right} justes · {wrong} fausses",
    showMore: "Voir {n} de plus",
    modePractice: "Entraînement",
    modeQuiz: "Quiz",
    modeBoss: "Boss",
    modeRecall: "Révision",
    attemptDetailTitle: "Détail de l'exercice",
    attemptDetailFailed: "Impossible de charger le détail de cet exercice.",
    attemptReviewHidden:
      "C'est un quiz de compréhension : la bonne réponse n'est jamais révélée, pour qu'il ne puisse pas être repassé de mémoire. Vous voyez ce que votre enfant a répondu et si c'était juste.",
    attemptNoAnswers: "Le détail des réponses n'est pas disponible pour cet exercice.",
    questionLabel: "Question {n}",
    childAnswer: "Réponse donnée :",
    expectedAnswer: "Réponse attendue :",
    performanceTitle: "Performances",
    performanceSubtitle: "Ce que valent les activités réalisées.",
    kpiAccuracy: "Bonnes réponses",
    kpiSuccess: "Exercices réussis",
    kpiTriesToPass: "Tentatives avant réussite",
    kpiPerQuestion: "Temps par question",
    kpiPerExercise: "{value} par exercice",
    kpiLessonCompletion: "Cours menés à bout",
    kpiExerciseCompletion: "Exercices menés à bout",
    kpiRegularity: "Régularité",
    kpiStreak: "Jours d'affilée",
    progressionTitle: "Progression",
    progressionByTime:
      "Temps d'apprentissage par jour — vert si la moyenne du jour est au-dessus du seuil de réussite.",
    progressionByActivity:
      "Activités par jour — le temps n'était pas encore mesuré sur cette période.",
    dayTooltip: "{date} : {minutes}, {exercises} exercice(s), moyenne {score}",
    compareHint: "Comparaison avec la période précédente de même longueur.",
    engagementTitle: "Engagement et sérieux",
    engagementSubtitle: "Est-ce qu'il s'y met, et régulièrement ?",
    efficiencyTitle: "Efficacité de l'apprentissage",
    efficiencySubtitle: "Est-ce que le temps passé produit des résultats ?",
    indexNotEnoughData: "Pas encore assez d'activité mesurée pour calculer un indice fiable.",
    indexScoreOutOf: "{score} sur 100",
    indexDrivers: "Ce qui porte le niveau",
    indexHolders: "Ce qui le retient",
    bandExcellent: "Très bon",
    bandGood: "Bon",
    bandFair: "Moyen",
    bandWeak: "Faible",
    factorRegularity: "Régularité",
    factorLearningTime: "Temps d'apprentissage",
    factorActivityVolume: "Volume d'activité",
    factorLessonCompletion: "Cours menés à bout",
    factorPerseverance: "Persévérance",
    factorProgress: "Progression des résultats",
    factorRevision: "Révisions",
    factorAccuracy: "Justesse des réponses",
    factorTimeToResult: "Rendement du temps",
    factorRetrySuccess: "Réussite après plusieurs essais",
    factorPace: "Rythme de réponse",
    factorRevisionGain: "Gain après révision",
    unitDays: "j",
    unitPoints: "pts",
    unitTries: "essais",
    unitMinutesPerSuccess: "min / réussite",
    unitSecondsPerQuestion: "s / question",
    subjectsTitle: "Par matière",
    subjectsSubtitle: "Où va le temps, et ce qu'il rapporte, matière par matière.",
    subjectsEmpty: "Aucune matière travaillée sur cette période.",
    colSubject: "Matière",
    colCoverage: "Programme",
    scopeLabel: "Afficher :",
    scopeClass: "Sa classe",
    scopeClassHint: "(sa classe)",
    scopeAll: "Tout",
    scopeExcluded:
      "Hors de sa classe sur cette période : {time} et {exercises} exercice(s) — révisions d’autres niveaux, extras, donjon et duels. Bascule sur « Tout » pour les inclure.",
    coverageAria: "{done} chapitres terminés sur {total}",
    colTime: "Temps",
    colLessons: "Cours",
    colExercises: "Exercices",
    colSuccess: "Réussite",
    colProgress: "Progression",
    colLevel: "Niveau",
    subjectsStrong: "Matières fortes",
    subjectsFragile: "Matières fragiles",
    subjectsNeglected: "Matières délaissées",
    subjectsImproving: "Plus fortes progressions",
    alertsTitle: "Recommandations",
    alertsSubtitle: "Ce qui mérite votre attention, et ce qui mérite d'être salué.",
    alertsEmpty: "Rien à signaler sur cette période — tout est dans les clous.",
    alertNoActivity: "Aucune activité sur les {days} derniers jours.",
    alertPerformanceDrop:
      "La moyenne a baissé de {drop} points par rapport à la période précédente ({previousAvgScore} % → {avgScore} %).",
    alertTimeWithoutProgress:
      "{subjectName} : {minutes} de travail sur la période, mais le taux de réussite sur « {chapterTitle} » reste à {chapterScore} %. Une session de révision est recommandée.",
    alertTimeLowYield:
      "{minutes} de travail sur la période pour une moyenne de {avgScore} % : le temps passé ne se transforme pas encore en résultats.",
    alertChapterStruggle:
      "Difficultés répétées sur « {chapterTitle} » ({subjectName}) : {avgScore} % de réussite sur {exercises} exercices.",
    alertSubjectNeglected:
      "{subjectName} n'a pas été travaillée sur cette période, alors qu'elle l'avait été {previousExercises} fois juste avant.",
    alertAbandonedExercises:
      "{abandoned} exercices sur {started} ont été commencés sans être terminés ({pct} %).",
    alertLowRegularity:
      "Seulement {activeDays} jours actifs sur {days} : la régularité est le premier levier de progrès.",
    alertRevisionNeeded:
      "« {chapterTitle} » reste à {avgScore} % et aucune session de révision n'a été faite. Le mode Révision est fait pour ça.",
    alertImprovement: "Progression de {gain} points : la moyenne atteint {avgScore} %. Bravo !",
    alertGoalReached: "Objectif de la semaine atteint : {done} missions sur {target}.",
    alertStrongSubject: "{subjectName} est solide : {avgScore} % de réussite.",
  },
  explain: {
    xp: "Tes points d'expérience. Réussis un exercice à 60 % ou plus en battant ton record pour en gagner. 200 XP = 1 niveau.",
    level: "Ton rang de héros : tu montes d'un niveau tous les 200 XP.",
    streak: "Tes jours d'entraînement d'affilée. Reviens chaque jour pour la garder en vie !",
    coins: "Ta monnaie. Gagne-les en réussissant des exercices, dépense-les à la boutique.",
    heroClass: "Ta classe de héros : elle évolue à mesure que tu montes en niveau.",
    questResultXp:
      "XP = récompense de l'exercice × ton score. Conditions : ≥ 60 %, au moins 4 s par question, et battre ton meilleur score sur cet exercice.",
    questResultScore: "{correct} bonnes réponses sur {total}.",
    lockedChapter: "Réussis d'abord le quiz de compréhension à 80 % pour débloquer ce chapitre.",
  },
  ai: {
    sectionTitle: "Mode IA",
    sectionDesc:
      "Branchez votre propre clé d'API : l'application ne vous facture rien, vous payez directement votre fournisseur.",
    unavailable: "Le mode IA n'est pas disponible pour le moment.",
    stateNone: "Aucune clé enregistrée — le mode IA est éteint.",
    stateActive: "Clé active",
    stateInvalid: "Clé refusée par le fournisseur",
    stateUnverified: "Clé non vérifiée",
    keyMasked: "Clé : {last4}",
    lastVerified: "Vérifiée le {date}",
    attach: "Activer le mode IA",
    replace: "Remplacer la clé",
    revoke: "Révoquer",
    revokeTitle: "Révoquer cette clé ?",
    revokeBody:
      "Nous supprimons immédiatement notre copie chiffrée, et le mode IA s'éteint pour tous les élèves que vous aviez activés.",
    revokeAtProvider:
      "Important : pensez aussi à révoquer la clé chez votre fournisseur. Nous ne pouvons pas le faire à votre place.",
    revokeCancel: "Annuler",
    revokeConfirm: "Révoquer la clé",
    revoked: "Clé révoquée.",
    provider: "Fournisseur",
    providerAnthropic: "Anthropic",
    providerHint:
      "Ces raccourcis remplissent l'adresse et les modèles — vous pouvez tout modifier ensuite. « Autre » accepte n'importe quelle API compatible OpenAI (Mistral, OpenRouter, Groq, Qwen…), pourvu qu'elle soit en https et joignable publiquement.",
    providerCompatible: "Compatible OpenAI",
    baseUrl: "Adresse de l'API",
    baseUrlHint: "Doit être en https, sur le port 443, et joignable publiquement.",
    localModelWarning:
      "Un modèle qui tourne sur votre machine ne fonctionnera pas : nos serveurs n'appellent jamais une adresse privée.",
    modelFast: "Modèle rapide",
    modelRich: "Modèle avancé",
    modelCurated: "Modèles recommandés",
    modelFree: "…ou saisissez l'identifiant de votre choix",
    keyLabel: "Clé d'API",
    keyHint:
      "Elle est chiffrée avant d'être stockée, et ne réapparaît jamais — même pour vous, même pour nous.",
    dailyBudget: "Plafond par jour ($)",
    monthlyBudget: "Plafond par mois ($)",
    budgetHint: "Au-delà, aucun appel n'est émis et l'élève retrouve l'application sans IA.",
    dailyReference: "Repère par jour ($)",
    spendRefShort: "repère",
    monthlyReference: "Repère par mois ($)",
    budgetReferenceHint:
      "Ces montants ne coupent rien : ils servent de repère aux alertes. Armez les plafonds ci-dessus pour qu'ils interrompent les appels.",
    limitsTitle: "Plafonds de consommation",
    limitsOnHint:
      "Au-delà du plafond, plus aucun appel n'est émis — ni pour l'argent, ni pour le nombre de quiz par jour.",
    limitsOffHint:
      "Aucune coupure automatique. La dépense reste comptée et vous êtes alerté (50 / 80 / 100 % du repère, et dès qu'une journée dépasse 3× l'ordinaire), mais rien ne s'arrête tout seul — c'est votre facture chez le fournisseur.",
    estimateNotice:
      "Estimation — la facture qui fait foi est celle de votre fournisseur. Nous ne percevons rien.",
    pricesAsOf: "Grille de prix relevée le {date}.",
    doubleSolve: "Vérifier chaque question générée",
    doubleSolveHint:
      "Chaque question forgée est résolue une seconde fois avant de vous être proposée.",
    doubleSolveWarning:
      "Sans cette vérification, une correction fausse ne se voit pas : elle s'apprend. Une question sur cinq reste vérifiée quand même.",
    consentTitle: "Avant de continuer",
    consentSent:
      "Ce qui part chez votre fournisseur : la question travaillée, la réponse donnée, la matière, la classe et la tranche d'âge.",
    consentNotSent: "Ce qui ne part jamais : le nom, l'adresse e-mail et l'identifiant de l'élève.",
    consentShared:
      "Les explications produites peuvent servir à d'autres élèves : elles dépendent de la question, jamais de qui l'a posée.",
    consentProvider:
      "Votre fournisseur décide seul de ce qu'il conserve. Certaines offres gratuites entraînent leurs modèles sur ce qu'on leur envoie : préférez une offre payante.",
    consentMoney: "Cette clé engage de l'argent réel, sur un compte qui n'est pas celui de l'app.",
    consentAccept: "J'ai lu et j'accepte",
    adultTitle: "Un adulte doit être présent",
    adultConfirm: "Je confirme qu'un adulte responsable est avec moi",
    save: "Vérifier et enregistrer",
    saving: "Vérification…",
    saved: "Clé vérifiée et enregistrée.",
    cancel: "Annuler",
    prefsSaved: "Réglages enregistrés.",
    studentsTitle: "Pour qui l'IA est allumée",
    studentsDesc: "Rien n'est allumé par défaut. Choisissez l'élève, puis ce à quoi il a droit.",
    studentSelf: "Moi",
    studentNone: "Aucun élève lié pour l'instant.",
    studentEnabled: "Mode IA allumé",
    energyMax: "Énergie par jour",
    energyToday: "{spent} / {max} utilisée aujourd'hui",
    featExplain: "Explications",
    featReformulate: "Reformulations",
    featChat: "Chat",
    featCheck: "Vérification de compréhension",
    featForge: "La Forge (quiz sur mesure)",
    featExerciseGen: "Exercices ciblés",
    featDigestStudent: "Bilan de l'élève",
    featDigestParent: "Bilan parent",
    errNotLinked: "Cet élève n'est pas lié à votre compte.",
    errNoCredential: "Enregistrez d'abord une clé.",
    errEnergyCap: "L'énergie quotidienne ne peut pas dépasser le plafond.",
    forgeTitle: "La Forge",
    forgeDesc:
      "Demande un quiz sur mesure. Il ne rapporte ni XP ni pièces — c'est un entraînement, pas une quête.",
    forgeSize: "Nombre de questions",
    forgeDifficulty: "Difficulté",
    forgeStart: "Forger le quiz",
    forgeWorking: "La Forge travaille…",
    forgeQuota: "{left} quiz restants aujourd'hui",
    forgeNoQuiz: "Aucun quiz forgé pour l'instant.",
    forgePlay: "Jouer",
    forgeExpires: "Disparaît le {date}",
    forgeUnverified: "Non vérifié",
    forgeUnverifiedWhy:
      "La double vérification est désactivée sur cette clé : cette réponse peut être fausse. Signale-la si elle l'est.",
    forgeNoReward: "Écrit par l'IA de ta famille. Aucun XP, aucune pièce.",
    forgeQuestion: "Question {n} / {total}",
    forgeValidate: "Valider",
    forgeNext: "Suivante",
    forgeFinish: "Terminer",
    forgeScore: "{correct} bonnes réponses sur {total}",
    forgeReplay: "Rejouer",
    forgeBack: "Retour",
    errForgeQuota: "Tu as déjà forgé tes quiz du jour. Reviens demain.",
    errForgeNoQuorum:
      "La Forge n'a pas réussi à écrire un quiz correct — réessaie ou change de chapitre.",
    errForgeNotFound: "Ce quiz n'existe pas.",
    errForgeExpired: "Ce quiz a expiré.",
    errForgeNoContext: "Ce chapitre n'a pas assez de matière pour forger un quiz.",
    spendTitle: "Votre dépense",
    spendToday: "Aujourd'hui",
    spendMonth: "Ce mois-ci",
    spendCalls: "{n} appels ce mois-ci",
    spendByFeature: "Par usage",
    spendByStudent: "Par élève",
    spendByModel: "Par modèle",
    spendRecent: "Derniers appels",
    spendNone: "Aucune dépense pour l'instant.",
    adviceTitle: "Ce modèle échoue trop souvent",
    adviceBody:
      "{model} : plus d'une question générée sur deux est rejetée depuis une semaine. Vous payez pour du travail jeté.",
    adviceSuggest: "Ceux-ci passent la barre : {models}",
    feedbackUp: "Utile",
    feedbackDown: "Signaler une erreur",
    feedbackSent: "Merci — c'est noté.",
    adminTitle: "Mode IA",
    adminDesc: "Adoption, fournisseurs et qualité par modèle. Aucun montant nominatif.",
    adminGlobal: "Mode IA global",
    adminFamilies: "Familles avec une clé",
    adminSuspended: "Familles coupées",
    adminStudents: "Élèves activés",
    adminCalls: "Appels sur 30 jours",
    adminQuality: "Qualité par modèle (30 j)",
    adminPlatform: "Clé plateforme",
    adminDenied: "Accès réservé aux administrateurs.",
    errKeyInvalid: "Votre fournisseur a refusé cette clé.",
    errModelUnknown: "Ce modèle n'existe pas chez ce fournisseur.",
    errCreditExhausted: "Le compte de votre fournisseur n'a plus de crédit.",
    errRateLimited: "Trop d'appels d'un coup — réessayez dans un instant.",
    errProviderDown: "Votre fournisseur ne répond pas.",
    errHostNotAllowed: "Cette adresse ne peut pas être appelée depuis nos serveurs.",
    errBudgetReached: "Votre plafond est atteint.",
    errModeOff: "Le mode IA n'est pas disponible.",
    errGeneric: "L'enregistrement a échoué.",
  },
  public: frPublic,
  quotes: [
    { text: "Le guerrier qui s'entraîne chaque jour devient invincible.", author: "Sensei XP" },
    { text: "Chaque bonne réponse est un coup porté à l'ignorance.", author: "L'Académie" },
    { text: "Le talent sans travail n'est qu'une flamme sans bois.", author: "Proverbe shinobi" },
    { text: "Ta progression ne ment jamais. Continue.", author: "Le Système" },
    {
      text: "Un S-Rank n'est qu'un Candidat Civil qui n'a jamais abandonné.",
      author: "Légende",
    },
    { text: "La connaissance est la plus puissante des armes.", author: "L'Oracle" },
    { text: "1% de mieux chaque jour = 37x plus fort en un an.", author: "Les Maths" },
  ],
};
