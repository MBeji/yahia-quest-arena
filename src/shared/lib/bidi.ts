 ...
 *
 * The fix is to make each formula a **contiguous LTR run**. CSS already does
 ...
 * math signal (a digit or a math symbol) in Unicode directional **isolates**
 * (U+2066 LRI … U+2069 PDI). Isolates work in both React text nodes and raw
 * HTML strings, need no CSS, and don't introduce new DOM nodes.
 */
...
/**
 * A non-Arabic run is worth isolating only when it actually carries notation —
 * a digit or a math/scientific symbol. This leaves plain whitespace, lone
 * variable letters and punctuation between Arabic words untouched (those render
 * fine on their own and don't need an isolated run). Both raw operators and
 * their HTML-escaped forms (`&lt;`, `&gt;`) count, so the helper is safe to run
 * before or after HTML escaping.
 */
const MATH_SIGNAL = /[0-9+−×÷/=≠<>≤≥±√π∞∈∉⊂⊃∪∩∥⟹⟺→·%°²³¹⁰⁴⁵⁶⁷⁸⁹₀₁₂₃₄₅₆₇₈₉]|&lt;|&gt;|&le;|&ge;/u;

/**
 * Wrap every math-bearing, non-Arabic run in `text` with LTR isolates so it
 * renders left-to-right even inside RTL prose. Text with no Arabic at all is
 * returned unchanged — LTR content (French/English) has no reordering bug, so
...
export function isolateLtrRuns(text: string): string {
  ...
  return text.replace(SEGMENT_RE, (segment) => {
    ...
    return MATH_SIGNAL.test(segment) ? `${LRI}${segment}${PDI}` : segment;
  });
}
