...
s'appuyant sur les règles canoniques de `AGENTS.md` et `ARCHITECTURE.md`.

> **Principe directeur.** `AGENTS.md` est la source de vérité. Toute violation
> d'une règle de `AGENTS.md` est, par défaut, **bloquante**. En cas de doute
> entre deux interprétations, c'est `AGENTS.md` puis `ARCHITECTURE.md` qui
...
| ----------------------- | --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| 🔴 **Bloquant**         | Doit être corrigé avant merge           | Faille de sécurité, RLS/grant manquant, gate rouge, régression, violation `AGENTS.md`, ordre migration↔deploy non respecté |
