...
>    you back that package out of the lot. (AGENTS.md Definition of Done.)
> 2. **Never merge, never push to `main`.** You only push `Codex/upgrade-*`
...
- **GitHub Actions.** Check the pinned action majors (`actions/checkout@v5`,
  ...
  `actions/github-script@v7`, `supabase/setup-cli@v1`, `anthropics/Codex-action@v1`).
...
- Branch `Codex/upgrade-patch-minor-$(date +%F)` off `main`.
...
- Branch `Codex/upgrade-major-<pkg>-v<to>-$(date +%F)`.
...
```json
{
  ...
  "patch_minor_branch": "Codex/upgrade-patch-minor-2026-06-16",
  "patch_minor_pr": 130,
