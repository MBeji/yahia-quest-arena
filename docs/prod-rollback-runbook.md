...
| « le job est vert »     | l'effet est obtenu      | un job réussit aussi quand ses étapes ont été **sautées** (`if:` non satisfait), ou quand une étape avale son propre échec. Lire le résumé du run, pas la pastille                                                                                         |
| « le gel est posé »     | la chaîne est bloquée   | **vérifier la variable, pas le run** : `gh variable list` doit montrer `MERGE_FREEZE`. Le 2026-07-20, 5 runs verts d'affilée n'avaient rien posé (403 sur le PAT). Depuis, le pas `Verdict` fait échouer le job — mais le réflexe reste de regarder l'état |
...
| `GH_AUTOMATION_PAT` | Secrets → Actions   | doit porter la permission **`Variables`** (« Manage Actions repository variables ») en **Read and write** — sinon pas de gel. ⚠️ **Ne pas confondre avec « Agent variables »** (agents Copilot), qui ne donne AUCUN accès aux variables Actions : l'erreur coûte un `HTTP 403: Resource not accessible by personal access token` (constaté 2026-07-20) | ✅ existe (permission à vérifier) |
