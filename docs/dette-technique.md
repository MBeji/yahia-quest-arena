...
| **Un appel IA tué par la plateforme débite l'énergie et n'est jamais comptabilisé** — si la fonction SSR meurt entre `reserve_ai_spend` et `settle_ai_spend`, aucun code ne tourne plus : l'événement d'usage n'est pas écrit. L'**argent** s'auto-répare (`sweep_ai_reservations()` libère la réservation après 5 min, §3.7 l'avait prévu) ; l'**énergie de l'élève**, non — elle est débitée pour rien. Mesuré en prod le 2026-08-25 : deux générations parties chez le fournisseur, énergie passée de 0/18 à **6/18**, console de dépense figée sur « 1 appel · 0,00 $ ». Le porteur voit zéro pendant que sa facture fournisseur monte, ce que RISK-2 promet précisément d'éviter. Le passage de `maxDuration` à 300 s rend le cas **rare, pas impossible** — coupure réseau, arrêt d'instance | `src/features/ai/ai-call.server.ts`  | M      | 📝 Qualité |
