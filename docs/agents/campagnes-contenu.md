...
```bash
npm run content:manuel:fetch -- 102905 --render --pages 18-24   # → $TMPDIR/yqa-manuels/102905P00.pdf + PNG
```

L'URL se dérive du code, jamais saisie ; rien n'entre dans git. En session cloud, l'environnement
doit autoriser `www.cnp.com.tn` (lot 0 de l'étude) — sinon la commande le **dit** : « refusé par la
politique réseau de l'environnement », pas « le CNP est tombé ». Le hook de session exporte
`NODE_USE_ENV_PROXY=1`, sans quoi le `fetch` de Node ignore le proxy de la plateforme et échoue
sur tout hôte. Le wrapper `YahiaAcademy/` du poste n'est plus une condition ; ce qui n'est pas au
