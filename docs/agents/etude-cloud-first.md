...
| 15  | Rester « de garde » jusqu'au merge réel (DoD §8)                                                 | Une session ouverte sur le poste, ou un `until` fragile                                                                | Deux mécanismes dans la session : l'**abonnement aux événements de la PR** (`subscribe_pr_activity`, exige l'app GitHub « Claude » sur le dépôt — **constaté sur cette PR** : abonnement accepté, événements livrés) et le **réveil différé** (`send_later`). Une VM récupérée pour inactivité est **re-provisionnée à la reprise**, conversation restaurée (doc) | ✅ À documenter dans passation.md §7 (lot 5)                                 |
...
l'app GitHub « Claude » soit installée sur les **trois** dépôts (constaté sur celui-ci : l'abonnement
de cette PR a été accepté ; le privé et `ScribeKit` restent à vérifier — elle conditionne l'auto-fix
et les routines déclenchées par GitHub, pas le travail courant) ; que `gh` soit absent
...
5. **github.com → Settings → Applications → Claude** : vérifier que l'app couvre les trois
   dépôts (auto-fix des PR, routines sur événement GitHub). **Constaté sur celui-ci** : l'abonnement
   de cette PR (#976) a été accepté et les événements sont livrés — reste à vérifier pour les deux
   autres.
