-- =========================================================
-- Ouverture du parcours Baccalauréat — section Sciences techniques (seuil R-8).
-- Sur une DB fraîche (toutes les migrations appliquées, dont
-- 20260817150200_open_bac_techniques_parcours), le parcours
-- `concours-bac-techniques` doit être 'available' ET entièrement gratuit.
--
...
--
-- Le CONTENU qui justifie l'ouverture (`french-bac-techniques` 7/7 et
-- `english-bac-techniques`) n'est pas vérifiable ici : depuis l'étude 24 le corpus
...
SELECT is(
  (SELECT status FROM public.parcours WHERE id = 'concours-bac-techniques'),
  'available',
  'le parcours concours-bac-techniques est ouvert (status = available)'
);

-- 2) Il reste un parcours de type concours : seule la barrière est en pause.
SELECT is(
  (SELECT kind FROM public.parcours WHERE id = 'concours-bac-techniques'),
  'concours',
  'concours-bac-techniques reste un parcours concours'
);

-- 3) Il est gratuit — sinon l'ouverture n'exposerait que l'aperçu.
SELECT ok(
  (SELECT NOT is_premium FROM public.parcours WHERE id = 'concours-bac-techniques'),
  'concours-bac-techniques est gratuit (is_premium = false)'
);

-- 4) Et sans restriction d'aperçu, cohérent avec les autres parcours gratuits.
SELECT is(
  (SELECT preview_policy FROM public.parcours WHERE id = 'concours-bac-techniques'),
  'full',
  'concours-bac-techniques expose tout le contenu (preview_policy = full)'
);
