...
--              speed bonus of 20260816140000 applies to mode='boss' ONLY, and
--              every exercise here is mode='practice'; see 50_boss_speed_xp_bonus)
...
VALUES ('7e570401-0000-0000-0000-000000000001', 'score-subj', 'Scoring Chapter');

INSERT INTO public.exercises (id, chapter_id, subject_id, title, xp_reward, reward_coins, mode)
VALUES ('7e570402-0000-0000-0000-000000000001',
        '7e570401-0000-0000-0000-000000000001', 'score-subj',
...
SELECT
  ...
  '7e570402-0000-0000-0000-000000000001',
  'Q' || g,
...
SELECT is(
  (SELECT count(*)::int FROM public.spaced_repetition_schedule
     WHERE user_id = 'f3333333-3333-3333-3333-333333333333'
       AND exercise_id = '7e570402-0000-0000-0000-000000000001'
       AND status = 'pending'),
  3,
