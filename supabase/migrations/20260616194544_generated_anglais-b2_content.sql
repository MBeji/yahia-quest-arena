...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('b4196f97-2382-59e6-a67a-db4123dc878c', '3e7018e2-a2b0-5412-8bae-18da5b189a0f', 'Direct: "I was waiting for the bus when I saw the accident." → He told me he ___ for the bus when he ___ the accident.', '[{"id":"a","text":"was waiting / saw"},{"id":"b","text":"had been waiting / had seen"},{"id":"c","text":"was waiting / had seen"},{"id":"d","text":"had been waiting / saw"}]'::jsonb, 'd', 'Past continuous ''was waiting'' backshifts to past perfect continuous ''had been waiting''; past simple ''saw'' backshifts to past perfect ''had seen''. Option A applies no backshift to either verb. Option B correctly backshifts both but the past perfect continuous and past perfect form a grammatically odd pair here — the second event (seeing) should be past perfect ''had seen'' while the ongoing background action (waiting) becomes past perfect continuous. Option C only backshifts the second verb. Option D correctly applies ''had been waiting'' (duration) and ''had seen'' (completed event).', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
