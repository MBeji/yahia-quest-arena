...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('00d33855-0299-5047-9006-955b4c2eaf92', '1757de03-9447-575a-b3f6-12239af5e49d', 'Find the INCORRECT sentence.', '[{"id":"a","text":"She has gone to the market; she came back an hour ago."},{"id":"b","text":"They haven''t finished the project yet."},{"id":"c","text":"He has spoken to the manager this morning."},{"id":"d","text":"I''ve known him for years."}]'::jsonb, 'a', 'The error is in the third option: "has gone" means she is still away, but "came back an hour ago" shows she has returned. The correct verb is has been (visited and returned). The other three use the present perfect correctly with this morning, yet, and for.', 4)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
