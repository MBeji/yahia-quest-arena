...
- **Quick test**: happening _at_ a future time → continuous; _finished by_ a future time → perfect; _how long_ by a future time → perfect continuous.', 6)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- ⚠️ A correct inference is _supported by_ the text even if not stated; a wrong option is directly stated (not an inference), directly denied, or a baseless guess.', 7)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
