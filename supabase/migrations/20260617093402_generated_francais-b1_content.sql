...
- **Pièges :** ne pas confondre avec le passé composé (auxiliaire au présent) ; ne pas oublier l''accord avec _être_.', 1)
ON CONFLICT (id) DO UPDATE SET
...
- **Doubles pronoms** — ordre fixe : _me/te/se/nous/vous < le/la/les < lui/leur < y < en_ (_Je **le lui** donne_).', 2)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- ⚠️ Pièges — _qui_ (verbe après) ≠ _que_ (sujet après) ; _où_ pour le lieu/temps ≠ _que_ (COD) ; _dont_ uniquement avec **de**, jamais avec à/sur/pour.', 3)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- ⚠️ **Indicatif** après l''opinion affirmée et l''espoir : _je pense que, j''espère que, il est certain que_ + indicatif ; mais _je ne pense pas que_ + subjonctif.', 4)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- ⚠️ **Règle d''or** — après « si », **jamais** de futur ni de conditionnel : on dit _Si j''**étais**…_ (pas ~~si je serais~~) et _Si j''**avais**…_ (pas ~~si j''aurais~~).', 5)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- **Pièges** : confondre si/que et ce que/ce qui, oublier le changement de pronom, mauvaise concordance, marqueur de temps non transformé.', 6)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- **Pièges à retenir** : licencier ≠ démissionner ; gaspiller ≠ économiser ; une information ≠ une publicité ; respecter les prépositions (d''accord avec, s''inquiéter de, besoin de).', 7)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- **Élimination** : barre ce qui contredit, exagère (_toujours, jamais_) ou déforme le texte.', 8)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('0ecc993b-4157-5d33-8c67-80ace6f51cf2', '5efe148d-7a31-5022-a12b-01d2e884157f', 'Trouve la phrase INCORRECTE.', '[{"id":"a","text":"Il avait vu ce film."},{"id":"b","text":"Elle était arrivée en retard."},{"id":"c","text":"Nous avions compris la leçon."},{"id":"d","text":"J''avais parti très tôt."}]'::jsonb, 'd', '« J''avais parti » est incorrect : « partir » se conjugue avec être, donc « j''étais parti(e) ». Les phrases a, b et d sont correctes : « voir » et « comprendre » prennent avoir, « arriver » prend être avec accord.', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
