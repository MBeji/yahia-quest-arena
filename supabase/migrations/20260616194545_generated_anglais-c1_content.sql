...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order) VALUES
  ('5599a1c1-26d7-5737-b0e5-b8b5df4db6e8', '36944d0b-25df-5542-a207-9cc4eeea8f80', 'anglais-c1', '⭐⭐ Drill: Inversion for Emphasis', 2, 75, 15, 'practice', 'admin', 5)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('01bd4472-a006-5f68-bde6-cc3e7041523a', '5599a1c1-26d7-5737-b0e5-b8b5df4db6e8', 'Complete: "Scarcely ___ the ship left port when the storm struck."', '[{"id":"a","text":"did"},{"id":"b","text":"has"},{"id":"c","text":"had"},{"id":"d","text":"was"}]'::jsonb, 'c', 'Scarcely triggers inversion with the past perfect: Scarcely had the ship left port when… The main event (the storm) is also past, so the past perfect auxiliary had is correct. "Did" would need a base verb (leave, not left). "Has" is present perfect — wrong for a completed past narrative. "Was" does not collocate with the following verb left in this structure.', 1)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('47cfe7ec-aa54-50d6-877c-4e7a79e0d1f3', '5599a1c1-26d7-5737-b0e5-b8b5df4db6e8', 'Complete: "So ___ the conditions that the race was called off."', '[{"id":"a","text":"terrible were"},{"id":"b","text":"were terrible"},{"id":"c","text":"terrible they were"},{"id":"d","text":"were the terrible"}]'::jsonb, 'a', 'The "So + adjective + inversion" pattern requires: So + adjective (terrible) + inverted verb (were) + subject (the conditions) + that… So the correct answer is "terrible were". "Were terrible" places the verb before the adjective — wrong order. "Terrible they were" inserts the subject pronoun between adjective and verb, which is not used in this inverted structure. "Were the terrible" is not grammatical.', 2)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('22a8db17-fe40-5000-ac81-95f3ef7b762d', '5599a1c1-26d7-5737-b0e5-b8b5df4db6e8', 'Which sentence uses ''Only when'' correctly?', '[{"id":"a","text":"Only when did the fire brigade arrive the building was saved."},{"id":"b","text":"Only when the fire brigade arrived, the building was saved."},{"id":"c","text":"Only when the fire brigade arrived was the building saved."},{"id":"d","text":"Only when was the fire brigade arriving the building saved."}]'::jsonb, 'c', 'With "Only when", inversion applies to the MAIN clause, not the when-clause: Only when [the fire brigade arrived] was the building saved (auxiliary before subject). Option (a) wrongly inverts inside the when-clause ("did… arrive"). Option (b) has normal order in the main clause — no inversion, which is wrong after the fronted only when. Option (d) inverts both clauses, which is never correct.', 3)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('0d004f7e-576b-50c3-b7c3-4eb967f9097f', '5599a1c1-26d7-5737-b0e5-b8b5df4db6e8', 'Complete: "Not only ___ the experiment, but she also published her findings within a month."', '[{"id":"a","text":"she completed"},{"id":"b","text":"she did complete"},{"id":"c","text":"completed she"},{"id":"d","text":"did she complete"}]'::jsonb, 'd', 'After "Not only" the first clause must invert: auxiliary (did) + subject (she) + base verb (complete). This gives: Not only did she complete the experiment… Option (a) keeps normal subject-first order — no inversion. Option (b) also keeps subject before auxiliary — still no inversion. Option (c) inverts without the do-auxiliary — this is not grammatical in modern English for action verbs.', 4)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('d952a674-e9c0-5f0a-86cb-82598fec3842', '5599a1c1-26d7-5737-b0e5-b8b5df4db6e8', 'Complete: "Such ___ his influence that the whole industry changed direction."', '[{"id":"a","text":"is"},{"id":"b","text":"was"},{"id":"c","text":"had been"},{"id":"d","text":"would be"}]'::jsonb, 'b', '"Such was…" is the inverted past form: Such was his influence that… The verb must be in the simple past to match the past narrative (the industry changed direction — also past simple). "Is" makes it present, breaking the narrative tense. "Had been" signals past-before-past (past perfect), implying a prior event not expressed here. "Would be" introduces a conditional that has no supporting if-clause.', 5)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('a6a85e40-8fd7-5b03-8efc-9d02a7907f2c', '5599a1c1-26d7-5737-b0e5-b8b5df4db6e8', 'A safety notice reads: "___ tampering with the emergency exits. Doing so may endanger lives." Which opening is most appropriate AND correctly formed?', '[{"id":"a","text":"Not only should you refrain from"},{"id":"b","text":"On no account should passengers engage in"},{"id":"c","text":"Never you should be involved in"},{"id":"d","text":"Rarely do passengers consider"}]'::jsonb, 'b', '"On no account should passengers engage in tampering with the emergency exits" is correctly formed ("On no account" + modal inversion: should + subject) and idiomatically appropriate for a formal safety notice expressing absolute prohibition. Option (a) produces "Not only should you refrain from tampering…" — grammatically inverted but the "Not only" structure requires a second clause ("but also…") which is absent. Option (c) puts the subject before the modal ("Never you should") — no inversion. Option (d) "Rarely" softens the prohibition to a habit observation — wrong for a safety notice demanding total compliance.', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order) VALUES
  ('1f5ed18c-5cc9-51c3-920b-0c0f0543d9dc', '109d0c1b-7491-540a-8c61-771d4801dc19', 'anglais-c1', '⭐⭐ Drill: Cleft Sentences', 2, 75, 15, 'practice', 'admin', 5)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('85fc46fe-1e2c-56ed-9fd6-45091969eab3', '1f5ed18c-5cc9-51c3-920b-0c0f0543d9dc', 'Identify the cleft type: "It was the funding gap that ultimately delayed the project."', '[{"id":"a","text":"Wh-cleft (pseudo-cleft)"},{"id":"b","text":"All-cleft"},{"id":"c","text":"It-cleft"},{"id":"d","text":"Reverse Wh-cleft"}]'::jsonb, 'c', 'An It-cleft has the structure It + be + [focused element] + that/who + rest of clause. Here: It was + the funding gap (focus) + that + ultimately delayed the project. A Wh-cleft would begin with What…. An All-cleft would begin with All…. A reverse Wh-cleft would put the focused element first followed by is/was + what-clause.', 1)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('e065e52f-9fda-59f1-a1c0-f9ef1b803482', '1f5ed18c-5cc9-51c3-920b-0c0f0543d9dc', 'Complete the Wh-cleft: "___ the delegates achieved was a preliminary framework, not a binding treaty."', '[{"id":"a","text":"That"},{"id":"b","text":"What"},{"id":"c","text":"It"},{"id":"d","text":"All what"}]'::jsonb, 'b', 'A Wh-cleft begins with a wh-clause as subject: What the delegates achieved was a preliminary framework. "That" would form a different clause type, not a Wh-cleft. "It" starts an It-cleft, which requires a following that/who. "All what" is never correct — the correct limiting cleft uses simply "All" without "what".', 2)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('490101df-b319-5048-93d7-f18608532ba8', '1f5ed18c-5cc9-51c3-920b-0c0f0543d9dc', 'Choose the sentence that correctly uses a cleft to focus on the PLACE.', '[{"id":"a","text":"What the treaty was signed was in Geneva."},{"id":"b","text":"All they signed was in Geneva."},{"id":"c","text":"It was in Geneva that the treaty was signed."},{"id":"d","text":"Geneva is what signed the treaty."}]'::jsonb, 'c', 'To focus on the place, use an It-cleft with the prepositional phrase as the focused element: It was in Geneva that the treaty was signed. The preposition in is retained with the place name. Option (a) forms a Wh-cleft but the word order is wrong (what + place is unusual). Option (b) uses All-cleft inappropriately — it focuses on action, not location. Option (d) inverts meaning (Geneva signed the treaty) and is not a cleft.', 3)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('cb446c86-4d52-5080-b4ab-bf33cd7adc3c', '1f5ed18c-5cc9-51c3-920b-0c0f0543d9dc', 'Find the INCORRECT cleft sentence.', '[{"id":"a","text":"What she objected to was the lack of transparency."},{"id":"b","text":"It was because of poor planning that the launch failed."},{"id":"c","text":"It is the budget that needs reviewing."},{"id":"d","text":"What I did was to consider all the options."}]'::jsonb, 'd', 'Option (d) is wrong: after "All/What [subject] did was", the verb takes the bare infinitive, not the to-infinitive. Correct: What I did was consider all the options. The use of "to consider" (to-infinitive) after "did was" is the classic learner error for this cleft type. Options (a), (b), and (c) are all correctly formed — a Wh-cleft, an It-cleft with a because-clause, and an It-cleft in the present tense.', 4)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('04ab2366-0595-507f-8b11-bb4816dc5e2c', '1f5ed18c-5cc9-51c3-920b-0c0f0543d9dc', 'Rewrite using an All-cleft to capture "She didn''t argue or protest — she simply resigned." Which version is correct?', '[{"id":"a","text":"All what she did was resigning."},{"id":"b","text":"All she did was resign."},{"id":"c","text":"All she did was to resign."},{"id":"d","text":"All did she was resign."}]'::jsonb, 'b', 'The All-cleft structure is: All + subject + did + was + bare infinitive. So: All she did was resign. "All what she did" inserts an incorrect "what" — it should be just "All". "To resign" (to-infinitive) after "did was" is the common error — the bare infinitive is required. "All did she was" wrongly applies inversion inside the cleft — no inversion occurs in an All-cleft.', 5)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('09379b08-c3fd-57e8-a107-a0b2e6b0bbe4', '1f5ed18c-5cc9-51c3-920b-0c0f0543d9dc', 'Complete the formal academic sentence to identify the cause: "Many observers blamed mismanagement. However, ___ that derailed the project was the sudden withdrawal of government funding."', '[{"id":"a","text":"it was what"},{"id":"b","text":"all what"},{"id":"c","text":"what"},{"id":"d","text":"it"}]'::jsonb, 'c', 'The sentence is a Wh-cleft: What + [relative clause] + was + [focused element]: What derailed the project was the sudden withdrawal of government funding. Only "what" correctly heads this structure. "It was what" would create a double-frame error (It-cleft + Wh-cleft merged). "All what" is never standard. "It" would require the It-cleft structure with that/who: It was [element] that derailed the project — but the sentence as given continues with "that derailed", so the slot before "that" needs the focused element, not just "it".', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order) VALUES
  ('4ede1f42-ad21-553c-8ee6-2d945f0447db', 'c6fee95d-7870-566a-842f-bbc5b2d0b46c', 'anglais-c1', '⭐⭐ Drill: Modals of Deduction', 2, 75, 15, 'practice', 'admin', 5)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('289ce2cf-9dea-5b69-85c4-3571c744ac47', '4ede1f42-ad21-553c-8ee6-2d945f0447db', 'The conference room is booked all day and a dozen delegates arrived this morning. Complete: "There ___ an important summit today."', '[{"id":"a","text":"can''t be"},{"id":"b","text":"must be"},{"id":"c","text":"should be"},{"id":"d","text":"would be"}]'::jsonb, 'b', 'Two strong pieces of evidence — a fully booked room and arriving delegates — point to a near-certain positive conclusion: There must be an important summit today. "Can''t be" signals near-certain negation, contradicting the evidence. "Should be" expresses expectation from a schedule or plan, not a deduction drawn from observed evidence. "Would be" introduces a conditional meaning not supported by an if-clause.', 1)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('3208d7e8-e62e-550e-80b8-7df59c701022', '4ede1f42-ad21-553c-8ee6-2d945f0447db', 'The package is sealed and undamaged. Complete: "Someone ___ tampered with it."', '[{"id":"a","text":"must have"},{"id":"b","text":"should have"},{"id":"c","text":"couldn''t have"},{"id":"d","text":"might have"}]'::jsonb, 'c', 'A sealed, undamaged package provides strong evidence that no tampering occurred: couldn''t have tampered (near-certain negative past deduction). "Must have" is near-certain positive — the opposite conclusion. "Should have" expresses advice or expectation about the past, not a deduction from evidence. "Might have" expresses mere possibility — too weak and in the wrong direction given the evidence.', 2)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('79458d41-0c73-5b9f-8155-8ba4a5b2617e', '4ede1f42-ad21-553c-8ee6-2d945f0447db', 'Which sentence correctly expresses a near-certain POSITIVE past deduction?', '[{"id":"a","text":"The lights were still on at midnight — they must have worked late."},{"id":"b","text":"The lights were still on at midnight — they can''t have worked late."},{"id":"c","text":"The lights were still on at midnight — they had to have worked late."},{"id":"d","text":"The lights were still on at midnight — they should have worked late."}]'::jsonb, 'a', 'Lights on at midnight is concrete evidence of late work: they must have worked late — near-certain positive past deduction (must have + past participle). Option (b) "can''t have" expresses near-certain negation, contradicting the evidence. Option (c) "had to have" is not standard English — the correct obligation form is "had to"; the correct past deduction form is "must have". Option (d) "should have" expresses advisability about the past, not a deduction from evidence.', 3)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('34bc6351-b76b-5923-bc48-5db8decec90a', '4ede1f42-ad21-553c-8ee6-2d945f0447db', 'Complete: "She spent three years studying marine biology. She ___ know a great deal about coral reef ecosystems."', '[{"id":"a","text":"can''t"},{"id":"b","text":"must"},{"id":"c","text":"might not"},{"id":"d","text":"should"}]'::jsonb, 'b', 'Three years of specialist study is compelling evidence for a near-certain positive deduction about current knowledge: She must know a great deal. "Can''t" is near-certain negative — the opposite of what the evidence suggests. "Might not" expresses uncertain negation, which also contradicts three years of focused study. "Should" expresses expectation or advisability, not evidence-based deduction.', 4)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('309fe555-758d-5354-b0bc-04b186e4ed8d', '4ede1f42-ad21-553c-8ee6-2d945f0447db', 'Find the INCORRECT sentence.', '[{"id":"a","text":"The roads are flooded — it must have rained heavily last night."},{"id":"b","text":"No one picked up the parcel — it might still be at the depot."},{"id":"c","text":"She left her passport at home — she mustn''t have boarded the flight."},{"id":"d","text":"The engine is ice-cold — the car can''t have been driven today."}]'::jsonb, 'c', 'Option (c) is wrong: "mustn''t have" is not used for past negative deduction. The standard form for near-certain negative past deduction is "can''t have" or "couldn''t have": She can''t have boarded the flight. "Mustn''t" expresses prohibition. Options (a), (b), and (d) correctly use must have (near-certain positive past), might (possibility), and can''t have (near-certain negative past).', 5)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('12b56487-a0a4-5320-81c8-7c32fa52d2e2', '4ede1f42-ad21-553c-8ee6-2d945f0447db', 'A formal investigation report states: "Access to the server room was logged at 03:14. The chief technician clocked out at 22:00. Given that only two people hold access credentials, ___ responsible."', '[{"id":"a","text":"the chief technician must have been"},{"id":"b","text":"the chief technician might have been"},{"id":"c","text":"the other credential holder may have been"},{"id":"d","text":"neither person can have been"}]'::jsonb, 'c', 'If only two people hold credentials and the chief technician left at 22:00, the 03:14 access must belong to the other credential holder. However, a formal investigation report hedges conclusions pending further verification: the other credential holder may have been responsible (formal possibility — may rather than might is preferred in written formal contexts). Option (a) leaps to near-certainty about the chief technician — but the chief technician left at 22:00, making this illogical. Option (b) also points to the wrong person. Option (d) is a near-certain negative that contradicts the evidence (someone accessed the server room).', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order) VALUES
  ('ee2a3bb6-3b54-5794-ad11-569e61615b16', '205e7d4e-3de0-5dc9-be5e-e542f8f6c73e', 'anglais-c1', '⭐⭐ Drill: Participle Clauses', 2, 75, 15, 'practice', 'admin', 5)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('3a9c10c9-4371-545f-8d04-7f19dd74bbdc', 'ee2a3bb6-3b54-5794-ad11-569e61615b16', 'Reduce the relative clause: "The letter which was sent by the director arrived this morning."', '[{"id":"a","text":"The letter sent by the director arrived this morning."},{"id":"b","text":"The letter having sent by the director arrived this morning."},{"id":"c","text":"The letter sending by the director arrived this morning."},{"id":"d","text":"The letter to send by the director arrived this morning."}]'::jsonb, 'a', '"Which was sent" is a passive relative clause — it reduces to a past participle: sent by the director. The past participle preserves the passive meaning (the letter was sent by the director). "Sending" is the active present participle — wrong, since the letter doesn''t send itself. "Having sent" is the active perfect participle — also wrong for a passive clause. "To send" is a to-infinitive, which changes meaning and is not a reduced relative here.', 1)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('fd1fcc37-fcd7-514b-9934-6090e4a45702', 'ee2a3bb6-3b54-5794-ad11-569e61615b16', 'Identify the type and function: "Turning the corner, the team spotted the finish line."', '[{"id":"a","text":"Past participle clause — passive meaning"},{"id":"b","text":"Perfect participle clause — action prior to the main clause"},{"id":"c","text":"Reduced passive relative clause — modifying a noun"},{"id":"d","text":"Present participle clause — simultaneous action"}]'::jsonb, 'd', '"Turning the corner" is a present participle clause (-ing form) expressing an action happening at the same time as the main clause (spotting the finish line). Both actions are simultaneous. It is not past participle (no passive meaning — the team turned the corner, not the corner was turned). It is not perfect participle (no "having"). It is not a reduced relative clause — it modifies the whole event, not a noun.', 2)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('8dc3607f-4b7d-5699-be72-fae4c089a4ca', 'ee2a3bb6-3b54-5794-ad11-569e61615b16', 'Find the DANGLING participle.', '[{"id":"a","text":"Having reviewed the contract, the lawyer signed it."},{"id":"b","text":"Inspired by the speech, the volunteers signed up immediately."},{"id":"c","text":"Crossing the finish line, the crowd erupted in applause."},{"id":"d","text":"Not understanding the task, she asked her supervisor for help."}]'::jsonb, 'c', 'Option (c) is a dangling participle: "Crossing the finish line" implies the crowd was crossing it, but the crowd was watching, not running. The subject of the main clause (the crowd) cannot cross the finish line. A corrected version would be: Crossing the finish line, the runner heard the crowd erupt in applause. Options (a), (b), and (d) correctly share the participle subject with the main clause subject (the lawyer, the volunteers, she).', 3)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('54ee9d81-5921-5ce3-ba64-7bde7b49079d', 'ee2a3bb6-3b54-5794-ad11-569e61615b16', 'Complete to show the first action: "___ all available evidence, the investigators published their final report."', '[{"id":"a","text":"Being examined"},{"id":"b","text":"Examining"},{"id":"c","text":"Having examined"},{"id":"d","text":"To examine"}]'::jsonb, 'c', 'The investigators examined the evidence FIRST, then published the report — two sequential actions. Use having + past participle to mark the prior completed action: Having examined all available evidence, the investigators published their final report. "Examining" (present participle) would imply both actions were simultaneous. "Being examined" is passive — the investigators did not examine themselves. "To examine" is a to-infinitive expressing purpose, not sequence.', 4)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('adf722d6-12e8-5c73-8e93-a12777d4d4f5', 'ee2a3bb6-3b54-5794-ad11-569e61615b16', 'Complete: "___ in multiple languages, the instruction manual reached a global audience."', '[{"id":"a","text":"Writing"},{"id":"b","text":"Written"},{"id":"c","text":"Having written"},{"id":"d","text":"To write"}]'::jsonb, 'b', '"Written in multiple languages" is a past participle clause expressing an adverbial meaning (because it was written in multiple languages). The past participle carries the passive sense — the manual was written by someone, not that it wrote something. "Writing" (active present participle) would imply the manual is actively writing — impossible. "Having written" implies the manual wrote something in the past. "To write" changes the meaning to purpose (in order to write).', 5)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('18db6ba5-34c5-55c9-a5b1-b99eb677c5e0', 'ee2a3bb6-3b54-5794-ad11-569e61615b16', 'Which sentence most effectively reduces this pair for a formal report? "The board reviewed the proposals carefully. It then selected three for further development."', '[{"id":"a","text":"Reviewed carefully, the board selected three proposals for further development."},{"id":"b","text":"Having carefully reviewed the proposals, the board selected three for further development."},{"id":"c","text":"Reviewing the proposals carefully, three were selected for further development."},{"id":"d","text":"The board, reviewing the proposals carefully, had selected three for further development."}]'::jsonb, 'b', 'The review came FIRST; the selection followed. Use having + past participle for the prior active action with consistent subject (the board): Having carefully reviewed the proposals, the board selected three for further development. Option (a) uses a bare past participle ("Reviewed") which is passive — implies the board was reviewed, not that it reviewed. Option (c) is a dangling participle — "three" cannot review proposals. Option (d) uses "had selected" (past perfect) which implies an even earlier past event and shifts the emphasis incorrectly.', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order) VALUES
  ('65777174-18b1-5175-87dd-57fac5b69845', '6d77d1ef-a1fa-5f50-98d6-c50dcb5685a1', 'anglais-c1', '⭐⭐ Drill: Discourse Markers', 2, 75, 15, 'practice', 'admin', 5)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('2bf09ddc-4d76-5a3f-a6f2-028e2d4e21f7', '65777174-18b1-5175-87dd-57fac5b69845', 'Complete: "The solar panels were installed last spring. ___, the building''s energy bills fell by 30%."', '[{"id":"a","text":"Nevertheless"},{"id":"b","text":"Whereas"},{"id":"c","text":"Consequently"},{"id":"d","text":"On the contrary"}]'::jsonb, 'c', 'The solar panels caused the bills to fall — a direct result. "Consequently" is the correct result marker here. "Nevertheless" introduces a concession (something happening despite an obstacle) — no obstacle is mentioned. "Whereas" joins two contrasting clauses in a single sentence and cannot open a new sentence. "On the contrary" denies the previous statement and asserts the opposite — inappropriate when the second sentence is a natural consequence.', 1)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('f8744680-02d4-52d4-9abf-316174a9e26f', '65777174-18b1-5175-87dd-57fac5b69845', 'Find the INCORRECT sentence.', '[{"id":"a","text":"Despite the heavy traffic, she arrived on time."},{"id":"b","text":"Despite the fact that traffic was heavy, she arrived on time."},{"id":"c","text":"Despite traffic being heavy, she arrived on time."},{"id":"d","text":"Despite traffic was heavy, she arrived on time."}]'::jsonb, 'd', 'Option (d) is wrong: "Despite" cannot be followed directly by a subject + finite verb (traffic was heavy). After "Despite" use a noun phrase (option a), "the fact that" + clause (option b), or a gerund phrase (option c — "traffic being heavy"). All three correct options use grammatically valid structures after "despite"; option (d) does not.', 2)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('3e46e2ac-252d-5562-b9db-cf24818aa900', '65777174-18b1-5175-87dd-57fac5b69845', 'Complete: "The documentary was long and slow-paced. ___, it offered a profound meditation on loss."', '[{"id":"a","text":"Furthermore"},{"id":"b","text":"Therefore"},{"id":"c","text":"Namely"},{"id":"d","text":"Nonetheless"}]'::jsonb, 'd', 'The second sentence concedes that despite the film''s weaknesses (long and slow-paced), it had genuine value — a classic concessive relationship. "Nonetheless" (meaning even so / despite that) is correct. "Furthermore" adds a point in the same direction — wrong here, as the two sentences pull in opposite directions. "Therefore" signals a result — a slow documentary does not logically result in profundity. "Namely" introduces a specification or list — not applicable here.', 3)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('4428105e-f8e2-5227-bb64-c9c464645411', '65777174-18b1-5175-87dd-57fac5b69845', 'Complete: "The scheme was costly, ___ effective in reducing reoffending rates."', '[{"id":"a","text":"although"},{"id":"b","text":"albeit"},{"id":"c","text":"nevertheless"},{"id":"d","text":"however"}]'::jsonb, 'b', '"Albeit" + adjective/adverb/noun phrase is the correct mid-clause concessive structure: costly, albeit effective. It acknowledges the cost while asserting the effectiveness within a single clause. "Although" requires a full finite clause (subject + verb) after it — it cannot precede a bare adjective like "effective". "Nevertheless" and "however" are sentence connectors that must introduce a new clause, not modify within the same clause.', 4)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('fbd35ddb-bc56-5e92-82ee-e9b293751591', '65777174-18b1-5175-87dd-57fac5b69845', 'Which sentence correctly uses ''thereby''?', '[{"id":"a","text":"The committee voted to extend the deadline, thereby the project was saved."},{"id":"b","text":"The committee voted to extend the deadline; thereby, they saved the project."},{"id":"c","text":"The committee voted to extend the deadline, thereby saving the project."},{"id":"d","text":"The committee voted to extend the deadline. Thereby the project was saved."}]'::jsonb, 'c', '"Thereby" must be followed by a present participle (-ing form) to embed a result in the same clause: thereby saving the project. Option (a) follows "thereby" with a full clause (the project was saved) — this is the classic error. Option (b) uses a semicolon and treats "thereby" like "therefore" — incorrect, as "thereby" does not introduce an independent clause. Option (d) starts a new sentence with "Thereby" — it cannot stand as a sentence connector the way "therefore" or "consequently" can.', 5)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('59ccbb34-e757-50bb-b62c-c9c0d0165609', '65777174-18b1-5175-87dd-57fac5b69845', 'Read: "Critics praised the novel''s structure. ___, the characterisation was seen as underdeveloped. ___ the author was shortlisted for three awards."  Choose the pair that best fits both gaps.', '[{"id":"a","text":"Moreover … Consequently"},{"id":"b","text":"However … Nevertheless"},{"id":"c","text":"Therefore … Furthermore"},{"id":"d","text":"On the contrary … Nonetheless"}]'::jsonb, 'b', 'Gap 1: the second sentence introduces a weakness contrasting with the praise — "However" (contrast) is correct. Gap 2: despite mixed reviews, the author was still shortlisted — a concession: "Nevertheless" (even so) is correct. Option (a) uses "Moreover" (addition in the same direction) for a contrast — wrong. Option (c) makes the characterisation criticism a result of praise — illogical. Option (d): "On the contrary" would deny the praise entirely, but the critics did praise the structure — the issue is only with characterisation, not a full denial.', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order) VALUES
  ('551f7488-b570-5365-9034-910c6790a596', 'd52b54fe-baf1-5b5b-b19c-17dd73549161', 'anglais-c1', '⭐⭐ Drill: Word Formation', 2, 75, 15, 'practice', 'admin', 5)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('fab31593-a318-5fd1-b932-88985e2b91e5', '551f7488-b570-5365-9034-910c6790a596', 'Complete: "The ___ of the new bridge took three years." (base word: CONSTRUCT)', '[{"id":"a","text":"constructive"},{"id":"b","text":"constructing"},{"id":"c","text":"constructed"},{"id":"d","text":"construction"}]'::jsonb, 'd', 'The gap follows the article "The" and needs a noun: construction (-tion suffix). "Constructive" is an adjective meaning helpful or productive. "Constructing" is a present participle or gerund — it could head a noun phrase ("The constructing of…") but is far less natural than "construction" here. "Constructed" is a past participle/adjective and cannot serve as the head noun after "The".', 1)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('f8ad97e0-54e4-567b-8497-0ae0fa31ccdb', '551f7488-b570-5365-9034-910c6790a596', 'Which negative prefix correctly forms the opposite of ''logical''?', '[{"id":"a","text":"unlogical"},{"id":"b","text":"mislogical"},{"id":"c","text":"illogical"},{"id":"d","text":"inlogical"}]'::jsonb, 'c', 'The prefix il- attaches to roots beginning with l: il- + logical = illogical. "Unlogical" uses the wrong prefix — un- is for general adjectives (unhappy, unreliable) and does not collocate with this Latin-origin adjective. "Mislogical" and "inlogical" are not words in standard English.', 2)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('ee524853-6f22-5e56-add6-dee496e038e3', '551f7488-b570-5365-9034-910c6790a596', 'Complete: "Her ___ handling of the crisis impressed the board." (base word: SKILL — adjective form)', '[{"id":"a","text":"skill"},{"id":"b","text":"skilfully"},{"id":"c","text":"skilful"},{"id":"d","text":"skillness"}]'::jsonb, 'c', 'The gap precedes a noun ("handling") and follows a possessive ("Her"), so an adjective is needed: skilful (-ful suffix). "Skill" is a noun and cannot directly modify another noun in this slot without forming a compound ("skill handling" is not natural). "Skilfully" is an adverb and cannot modify a noun. "Skillness" is not a word — the correct noun form is "skill" or "skilfulness".', 3)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('9b9c6d39-bfd5-5820-aa11-47e2547ab3fb', '551f7488-b570-5365-9034-910c6790a596', 'Find the sentence with a word-formation ERROR.', '[{"id":"a","text":"The committee''s inability to reach consensus caused delays."},{"id":"b","text":"Her remarkably precise analysis won widespread admiration."},{"id":"c","text":"The govern of the region introduced sweeping reforms."},{"id":"d","text":"Irregular working hours affected their productivity."}]'::jsonb, 'c', 'Option (c) uses "govern" (a verb) where a noun is required: The government (or governor) of the region. Options (a), (b), and (d) all use correctly formed words: inability (in- + able + -ity), remarkably (remark + -able + -ly as an adverb), and irregular (ir- + regular) and productivity (produce + -ivity).', 4)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('e8ffec00-be28-540e-9825-587ff768e95f', '551f7488-b570-5365-9034-910c6790a596', 'Complete: "The ___ of the witnesses made the trial far more complex." (base word: CONTRADICT — use the NOUN form)', '[{"id":"a","text":"contradictory"},{"id":"b","text":"contradictions"},{"id":"c","text":"contradicting"},{"id":"d","text":"contradictively"}]'::jsonb, 'b', 'The gap follows "The" and needs a noun (plural, as there are multiple witnesses): contradictions (-tion + plural -s). "Contradictory" is an adjective. "Contradicting" is a present participle/gerund — it could head a gerund phrase but is less precise here as a count noun. "Contradictively" is not a standard word — the adverb form is "contradictorily".', 5)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('63f8477c-6a47-52e5-8841-ab0ef849a731', '551f7488-b570-5365-9034-910c6790a596', 'Complete both gaps: "The scheme was praised for its ___ design and its ___ impact on local communities." (gap 1: base INNOVATE — adjective; gap 2: base TRANSFORM — adjective)', '[{"id":"a","text":"innovation … transforming"},{"id":"b","text":"innovative … transformative"},{"id":"c","text":"innovatively … transformed"},{"id":"d","text":"innovating … transformational"}]'::jsonb, 'b', 'Both gaps precede nouns ("design", "impact") and need adjectives. Gap 1: innovative (-ive suffix from innovate). Gap 2: transformative (-ative suffix, meaning having the power to transform). Option (a): "innovation" is a noun and cannot modify "design" attributively; "transforming" is a present participle — possible but less precise than "transformative" at C1. Option (c): both are adverb and past participle — neither can precede a noun as an attributive adjective here. Option (d): "innovating" is a present participle — awkward before a noun; "transformational" is a valid adjective but "transformative" is the standard C1 collocation for this context.', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order) VALUES
  ('2a3f5d7f-35ff-5a70-a4f6-af5294f3e820', '5e2ad3bf-2fd2-5e53-ab55-41479a2ff162', 'anglais-c1', '⭐⭐ Drill: Inference & Implication Mastery', 2, 75, 15, 'practice', 'admin', 5)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('bb513912-1811-573e-9149-83ed8a38b00a', '2a3f5d7f-35ff-5a70-a4f6-af5294f3e820', 'Read: "She glanced at her watch for the third time and shifted in her seat."

What can you INFER about her state of mind?', '[{"id":"a","text":"She is impatient or anxious about the time."},{"id":"b","text":"She is admiring her new watch."},{"id":"c","text":"She is calm and perfectly comfortable."},{"id":"d","text":"She has nowhere she needs to be."}]'::jsonb, 'a', 'Inference reads the implied meaning behind described behaviour: repeatedly checking the time and shifting in the seat are signs of impatience or anxiety. Admiring the watch ignores the repetition and the restlessness; ''calm'' and ''nowhere to be'' directly contradict the nervous actions.', 1)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('8f9a58e8-d8d1-540c-bf01-05cea8cd4b73', '2a3f5d7f-35ff-5a70-a4f6-af5294f3e820', 'Read: "The negotiations were protracted, dragging on for months without resolution."

Using the context, ''protracted'' most nearly means:', '[{"id":"a","text":"brief and efficient"},{"id":"b","text":"drawn out over a long time"},{"id":"c","text":"immediately successful"},{"id":"d","text":"conducted in secret"}]'::jsonb, 'b', 'The context clue ''dragging on for months'' defines the word: protracted means extended or drawn out over a long time. ''Brief and efficient'' is the opposite; ''successful'' is contradicted by ''without resolution''; ''secret'' is unrelated to the duration the sentence describes.', 2)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('34c6baba-483e-56a5-a540-67d23a76872e', '2a3f5d7f-35ff-5a70-a4f6-af5294f3e820', 'Read: "The minister''s plan is, shall we say, ambitious."

What does the hedge ''shall we say'' most likely signal about the writer''s stance?', '[{"id":"a","text":"The writer sincerely admires the plan as bold."},{"id":"b","text":"The writer has no opinion about the plan."},{"id":"c","text":"The writer is being ironic, implying the plan is unrealistic."},{"id":"d","text":"The writer thinks the plan is far too modest."}]'::jsonb, 'c', 'A self-conscious hedge like ''shall we say'' before a positive-sounding word flags irony: the writer means the opposite, hinting the plan is unrealistic. Sincere admiration would not need the hedge; ''no opinion'' ignores the loaded phrasing; ''too modest'' reverses the implied criticism of over-ambition.', 3)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('1376325d-7056-5bb6-b9b8-a079fbd4c4cf', '2a3f5d7f-35ff-5a70-a4f6-af5294f3e820', 'Read: "When the bridge collapsed a week after opening, the engineers described it as ''a minor setback''."

The phrase ''a minor setback'' is an example of:', '[{"id":"a","text":"hyperbole — deliberate exaggeration"},{"id":"b","text":"a literal, accurate description"},{"id":"c","text":"an extended metaphor"},{"id":"d","text":"understatement — deliberately downplaying"}]'::jsonb, 'd', 'Calling a total collapse ''a minor setback'' deliberately downplays a serious event — that is understatement. Hyperbole would exaggerate, not minimise; it is not ''literal'' because the event was major, not minor; and there is no metaphor (no implied comparison) at work.', 4)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('0005801c-45ce-5e6d-873f-931e347221f0', '2a3f5d7f-35ff-5a70-a4f6-af5294f3e820', 'Read: "The first trial failed. The second, however, exceeded every expectation. This success transformed the company''s prospects."

What does ''This success'' refer back to?', '[{"id":"a","text":"the outcome of the second trial"},{"id":"b","text":"the failure of the first trial"},{"id":"c","text":"both trials taken together"},{"id":"d","text":"the company''s prospects"}]'::jsonb, 'a', 'Cohesion: ''This success'' points back to the nearest matching idea — the second trial, which ''exceeded every expectation''. The first trial ''failed'' (not a success); ''both trials'' wrongly includes the failure; and the company''s prospects are what the success changed, not the success itself.', 5)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('ad65c1a1-db06-5089-854d-ad543019eb44', '2a3f5d7f-35ff-5a70-a4f6-af5294f3e820', 'Read: "Critics praised the film''s visuals. Of its plot, they said little."

What does the writer imply by noting that critics ''said little'' about the plot?', '[{"id":"a","text":"The plot was the film''s strongest feature."},{"id":"b","text":"The plot was probably weak, so critics avoided praising it."},{"id":"c","text":"The critics had not finished watching the film."},{"id":"d","text":"The film had no plot whatsoever."}]'::jsonb, 'b', 'Implication by contrast: praising the visuals but saying little about the plot hints, without stating it, that the plot was weak. If the plot were the strongest feature, critics would have praised it; ''had not finished'' and ''no plot whatsoever'' overstate what the silence can support.', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
