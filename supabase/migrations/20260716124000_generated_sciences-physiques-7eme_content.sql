...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('6c0db2e4-d2d6-5e9d-ac82-6b2ac1890010', '446982dc-36b0-57df-be46-1ec522a7e4e1', 'بالون منفوخ بالهواء أثقل من بالون فارغ من الهواء (بنفس الحجم تقريبًا) عند وزنهما بميزان حسّاس. ماذا يثبت هذا؟', '[{"id":"a","text":"أنّ الهواء مادّة لها كتلة فعلية"},{"id":"b","text":"أنّ الهواء ليس له أيّ كتلة"},{"id":"c","text":"أنّ الهواء سائل لا غاز"},{"id":"d","text":"أنّ البالون المنفوخ أخفّ من الفارغ"}]'::jsonb, 'a', 'زيادة كتلة البالون بعد نفخه بالهواء تثبت أنّ الهواء المضاف له كتلة فعلية، فالهواء مادّة حقيقية كباقي الموادّ.', 1, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('90ae290f-bed3-5d7a-8de2-aeeaad3cb027', '1b3f79ca-085b-58b7-a651-d2364307d25b', 'دارة كهربائية بسيطة متّصلة بالكامل من المولّد إلى المصباح وعودةً إلى المولّد دون أيّ انقطاع. ماذا يحدث للمصباح؟', '[{"id":"a","text":"يبقى مطفأً رغم اتّصال الدارة"},{"id":"b","text":"ينفصل عن الدارة تلقائيًا"},{"id":"c","text":"يتحوّل إلى محرّك كهربائي"},{"id":"d","text":"يضيء، لأنّ التيّار يجري في الدارة المغلقة"}]'::jsonb, 'd', 'عندما تكون الدارة مغلقة بالكامل دون أيّ انقطاع، يجري التيّار الكهربائي في الحلقة فيضيء المصباح.', 3, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('39e32a36-d634-50cb-8b5e-ebfed0d02b9a', '1b3f79ca-085b-58b7-a651-d2364307d25b', 'انقطع سلك واحد فقط في دارة كهربائية بسيطة تحتوي على مصباح واحد. ماذا يحدث للمصباح؟', '[{"id":"a","text":"ينطفئ تمامًا، لأنّ الدارة أصبحت مفتوحة"},{"id":"b","text":"يضيء بشكل طبيعي رغم الانقطاع"},{"id":"c","text":"يضيء بشكل أضعف فقط"},{"id":"d","text":"يتغيّر لونه"}]'::jsonb, 'a', 'انقطاع سلك واحد فقط يكفي لجعل الدارة مفتوحة، فيتوقّف التيّار الكهربائي عن الجريان تمامًا وينطفئ المصباح بالكامل.', 5, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
