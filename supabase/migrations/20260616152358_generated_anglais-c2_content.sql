...
- ⚠️ _get away with_, not _get away from_ — the preposition is fixed and not interchangeable.', 6)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- ⚠️ "The author implies" ≠ "the author states" — implied meaning is suggested without being stated; examination questions distinguish these deliberately.', 7)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
