...
INSERT INTO public.subjects (id, name_fr, description, attribute, color_token, icon, display_order, content_language, is_premium, theme_id, grade_id, manuel_refs) VALUES
  ('physique-1ere-sec', 'Sciences Physiques', 'L''électricité, la matière et la mécanique selon le programme officiel de la 1ère année de l''enseignement secondaire (tronc commun) : le phénomène d''électrisation, le circuit électrique, l''intensité et la tension électriques, le dipôle résistor et la loi d''Ohm ; les états physiques de la matière, ses propriétés (dilatation, température, conductivité thermique), la masse (masse volumique, densité) et les changements d''état d''un corps pur ; puis la mécanique : le mouvement, les actions mécaniques et le poids, forces et équilibre', 'Observation', 'subject-svt', 'Atom', 2, 'fr', false, 'ecole-tn', (SELECT id FROM public.grades WHERE theme_id = 'ecole-tn' AND slug = '1ere-sec'), NULL)
