-- =========================================================
-- Mode boss : le chronomètre note aussi les XP
-- ---------------------------------------------------------
-- Suite de la PR #742, qui a remplacé le compte à rebours de 20 s (il
-- répondait à la place de l'élève, puis enchaînait) par un chronomètre OUVERT
-- pilotant les dégâts infligés au boss. Ce lot fait porter le même chronomètre
-- sur la récompense : une manche de boss menée sous le temps de référence vaut
-- jusqu'à +50 % d'XP.
--
-- Réémet submit_exercise_attempt à l'identique de 20260727120000, à trois
-- greffes près (prime déclarée, calculée, renvoyée). Signature inchangée, donc
-- privilèges conservés ; réaffirmés quand même pour que la migration se
-- suffise à elle-même. Idempotent.
--
-- Garde-fous, parce que ce terrain a déjà été brûlé une fois :
--   * mode = 'boss' UNIQUEMENT — practice / quiz / challenge / rappel intacts ;
--   * borné à [1, 1.5] : c'est une prime, il n'existe aucun cas où jouer
--     lentement rapporte MOINS qu'aujourd'hui ;
--   * calculé après v_eligible, donc les trois portes d'effort (>= 4 s par
--     question, >= 60 %, amélioration du record) s'appliquent d'abord ;
--   * la durée est celle du SERVEUR (clock_timestamp() - session.started_at) —
--     p_duration_seconds envoyé par le client n'a jamais été lu, et ne l'est
--     toujours pas.
-- =========================================================
