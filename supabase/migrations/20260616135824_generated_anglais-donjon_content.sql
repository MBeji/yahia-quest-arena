...
- 🏆 Versatility wins: a hero ready in every tense, voice, and structure has no weak spot.', 4)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- 🏆 Precision and versatility win: a hero who commands both the form and the effect has no weak room.', 5)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- 🏆 Mastery wins: a speaker with no seam, no redundancy, and no false note commands every room.', 6)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('66fd19f4-8376-5dfa-bad1-992fa439fc40', '7fa9d06c-0246-534e-bfaf-cc643eefad4e', 'Grammar — Reported speech: "He said, ''I can swim very well.'' " What did he say?', '[{"id":"a","text":"He said he can swim very well."},{"id":"b","text":"He said he could swim very well."},{"id":"c","text":"He said he swam very well."},{"id":"d","text":"He said he will swim very well."}]'::jsonb, 'b', 'In reported speech, the modal can back-shifts to could: he said he could swim very well. (a) keeps can without back-shifting — only possible in informal immediate reporting. (c) uses the past simple swam, which changes the meaning. (d) uses will, which is not a back-shift of can.', 4)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('44b76531-fa9e-5dec-b8b7-559d1ab35c1c', '9236c621-0487-5754-a561-61b796380f43', 'Grammar — Reported speech: "The manager told us, ''The meeting will be rescheduled.'' " What did she tell us?', '[{"id":"a","text":"The manager told us the meeting will be rescheduled."},{"id":"b","text":"The manager told us the meeting would be rescheduled."},{"id":"c","text":"The manager told us the meeting would have been rescheduled."},{"id":"d","text":"The manager told us the meeting had been rescheduled."}]'::jsonb, 'b', 'Reported speech back-shifts will → would and keeps the passive (will be → would be): the meeting would be rescheduled. (a) keeps will — valid only in informal immediate speech (same day), but the standard back-shift is required here. (c) would have been rescheduled is the third-conditional result form — a different meaning (now impossible). (d) had been rescheduled implies it had already happened — no back-shift from a prediction.', 3)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('47dc7334-7b55-5945-bea2-476d1b9d7014', '91d20544-8a7f-5848-acff-4a219e376de0', 'Grammar — Modal of past deduction: "The exam paper was left on the desk all night. Anyone ___ the answers."', '[{"id":"a","text":"could have seen"},{"id":"b","text":"must see"},{"id":"c","text":"should have seen"},{"id":"d","text":"can''t have seen"}]'::jsonb, 'a', 'Anyone could have seen (past ability/possibility) — the exposure made it possible for someone to see the answers. "must see" is a present obligation (no past reference). "should have seen" implies someone was expected or obliged to see the answers. "can''t have seen" expresses certainty that nobody saw — but the paper being left out makes seeing possible, not impossible.', 3)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('44967475-3da8-5d2b-8733-1de3899c1155', '91d20544-8a7f-5848-acff-4a219e376de0', 'Usage — Discourse cohesion: "Choose the option in which all discourse markers are used correctly."', '[{"id":"a","text":"The study was small; moreover, its findings were surprising."},{"id":"b","text":"She trained hard. However she won the race."},{"id":"c","text":"The plan failed. Therefore, it had been well-funded."},{"id":"d","text":"Although the results were poor but the team didn''t give up."}]'::jsonb, 'a', '(a) is correct: moreover links a small size to surprising findings as two related noteworthy points (the smallness makes the findings more surprising — a compound idea). (b) needs a comma after However: However, she won. (c) makes a logical error: Therefore signals a result, but the plan failing and being well-funded are not in a cause-result relationship — Nevertheless would fit. (d) doubles the concession word: Although and but cannot both appear in the same clause — remove but (Although the results were poor, the team didn''t give up).', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('d96d53f9-fc41-5809-8626-f401eea9c716', '861145f7-4055-5f75-9520-dcf4b49d9411', 'Usage — Confusable words: "As a ___ observer with no financial stake in either company, she was the ideal person to mediate."', '[{"id":"a","text":"uninterested"},{"id":"b","text":"disinterested"},{"id":"c","text":"indifferent"},{"id":"d","text":"impartial"}]'::jsonb, 'b', 'disinterested means impartial — having no personal stake in the outcome, which makes someone ideal to mediate. This is tested at C2 because it is widely confused with uninterested. "uninterested" means not curious or caring — an uninterested mediator would do a terrible job. "indifferent" means having no preference or concern — too apathetic for a mediator. "impartial" is also correct in meaning, but the question specifically asks for the C2 near-synonym "disinterested" as the precise term that matches the "no financial stake" clue.', 3)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('2dbb5a31-b9ee-5c0f-970b-0733cc619386', '861145f7-4055-5f75-9520-dcf4b49d9411', 'Usage — Ellipsis: "Choose the sentence in which ellipsis is used CORRECTLY."', '[{"id":"a","text":"Some students passed the exam, and some did not."},{"id":"b","text":"She left early because tired."},{"id":"c","text":"He decided to leave, and so she."},{"id":"d","text":"The report was finished, but not presented."}]'::jsonb, 'a', '(a) is correct: …and some did not [pass the exam] — the omitted material (pass the exam) is fully recoverable. (b) is incorrect: because tired omits the subject and auxiliary (because she was tired) — this ellipsis is ungrammatical in standard English. (c) should be so did she (inversion required after so: so + auxiliary + subject). (d) omits was — the full form would be but [the report] was not presented — strictly, the subject is recoverable but the auxiliary was is also omitted; however this is actually a common and accepted gapping structure. (a) remains the cleanest, unambiguous correct example.', 4)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('ab024f8b-6c19-5c21-9962-d3512a4b2dda', '861145f7-4055-5f75-9520-dcf4b49d9411', 'Usage — Register: "Which option nominalises correctly and raises the register of ''They failed to respond to warnings''?"', '[{"id":"a","text":"The fact that they were failing to respond to warnings…"},{"id":"b","text":"They did not respond, failing with the warnings…"},{"id":"c","text":"The warnings were not responded to by them, failing…"},{"id":"d","text":"Their failure to respond to warnings…"}]'::jsonb, 'd', 'Their failure to respond to warnings nominalises the verb fail → failure and compresses the clause into a noun phrase, which is concise and formal. (b) uses a heavy that-clause and keeps the continuous aspect unnecessarily — not a clean nominalisation. (c) splits the structure awkwardly and is not more formal. (d) is a convoluted passive that adds words without adding clarity.', 6)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
