...
- **Homophones** — _on_ (= nous) ≠ _ont_ (avoir) ; _son_ (le sien) ≠ _sont_ (être).', 1)
ON CONFLICT (id) DO UPDATE SET
...
- **Usage** — habitudes, goûts et vérités générales : _J''habite à Tunis. Nous aimons le café._', 2)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- **Accord total** — un groupe nominal correct combine l''article, le genre et le nombre : _l''école, les animaux, une pomme_.', 3)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- ⚠️ Pièges : l''âge se dit avec **avoir** (_j''ai dix ans_) ; jours et mois en **minuscule** ; on écrit **quarante** et **cinquante**.', 4)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- ⚠️ Pièges : ordre des mots dans l''inversion, oubli du trait d''union, « ne » mal placé, « ne » non élidé devant voyelle, « quel » mal accordé.', 5)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
- ⚠️ **Piège** : la réponse doit venir du texte seul ; ne choisis pas un autre nombre ou un autre jour présent dans le texte.', 6)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order) VALUES
  ('fd9ee791-3e7f-5cc6-ae9a-d9434112b6a9', 'f3dd4dc6-9b35-544b-b30d-b5f35f1da030', 'Choisis les bonnes formes : « ___ sœurs ___ médecins. »', '[{"id":"a","text":"Ses … sont"},{"id":"b","text":"Ses … ont"},{"id":"c","text":"Ces … sont"},{"id":"d","text":"Ses … son"}]'::jsonb, 'a', 'Il faut le possessif pluriel « Ses » (les siennes) puis le verbe être « sont » : Ses sœurs sont médecins. « ont » est le verbe avoir, « son » est singulier, et « Ces » est un démonstratif, pas un possessif — le sens « ses sœurs à elle » demande « Ses ».', 5)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
