...
INSERT INTO public.chapters (id, subject_id, title, description, lesson_content, summary, display_order, manuel_ref, videos) VALUES
  ('3f2eb68a-bcd1-5538-b438-d78f8b67fa44', 'math-bac-math', 'Dérivabilité — la pente qui trahit la fonction', 'Nombre dérivé et tangente, approximation affine, dérivabilité à gauche et à droite, dérivabilité sur un intervalle, dérivées des fonctions usuelles et opérations, dérivée d''une fonction composée, dérivées successives, théorème de Rolle, théorème et inégalité des accroissements finis, sens de variation par le signe de la dérivée, extrema, point d''inflexion et centre de symétrie', '# ⚔️ Dérivabilité — lire la pente et dompter les variations
...
- **Point d''inflexion et centre de symétrie** : f″ s''annule en a en changeant de signe ⟹ inflexion en I(a, f(a)) (la courbe traverse sa tangente) ; O′(a, b) centre de symétrie ⟺ f(2a − x) = 2b − f(x).', 3, NULL, '[]'::jsonb)
ON CONFLICT (id) DO UPDATE SET
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
  description = EXCLUDED.description,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order, correction_video) VALUES
  ('59da127f-c905-5b05-8171-aae940fcfff7', '3f2eb68a-bcd1-5538-b438-d78f8b67fa44', 'math-bac-math', 'Test de compréhension ⭐ : Dérivabilité', 1, 20, 5, 'quiz', 'admin', 0, NULL)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('e97083e3-9d77-5d50-b865-73e0a5436536', '59da127f-c905-5b05-8171-aae940fcfff7', 'Soit f définie sur un intervalle ouvert I et a un réel de I. Par définition, f est dérivable en a lorsque l''un des rapports suivants admet une limite finie quand x tend vers a. Lequel ?', '[{"id":"a","text":"(f(x) − f(a))/x"},{"id":"b","text":"(f(x) − a)/(x − a)"},{"id":"c","text":"(f(x) − f(a))/(x − a)"},{"id":"d","text":"(f(x) + f(a))/(x − a)"}]'::jsonb, 'c', 'Le taux d''accroissement de f entre a et x est (f(x) − f(a))/(x − a) : c''est la pente de la corde, et sa limite quand x → a est le nombre dérivé f′(a) ✓. Diviser par x seul (option a) ne mesure aucune pente. L''option b soustrait a à f(x), donc mélange une image et une abscisse. L''option d additionne au lieu de soustraire : elle ne compare plus f(x) à f(a) et tend vers l''infini dès que f(a) ≠ 0.', 1, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('e5084261-1ae7-53c8-888c-23a0b8889caa', '59da127f-c905-5b05-8171-aae940fcfff7', 'Soit f(x) = x² − 3x. Quelle est l''équation de la tangente à C_f au point d''abscisse 2 ?', '[{"id":"a","text":"y = x − 4"},{"id":"b","text":"y = x − 2"},{"id":"c","text":"y = 4x − 10"},{"id":"d","text":"y = −2x + 5"}]'::jsonb, 'a', 'On a f(2) = 4 − 6 = −2 et f′(x) = 2x − 3, donc f′(2) = 1. La tangente est y = f′(a)(x − a) + f(a) = 1 × (x − 2) − 2 = x − 4 ✓. L''option b applique la formule fautive y = f′(a)x + f(a) (on oublie de translater de a). L''option c oublie de dériver le terme −3x : elle utilise f′(2) = 4. L''option d échange les rôles de f(a) et f′(a) : y = f(a)(x − a) + f′(a) = −2(x − 2) + 1.', 2, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('245aed6d-1d94-5a84-8e4e-eeff47f79729', '59da127f-c905-5b05-8171-aae940fcfff7', 'Soit f(x) = x³. En utilisant l''approximation affine f(a) + f′(a)h de f(a + h) avec a = 1 et h = 0,0002, quelle valeur approchée obtient-on pour (1,0002)³ ?', '[{"id":"a","text":"1,0002"},{"id":"b","text":"1,0006"},{"id":"c","text":"1,0020"},{"id":"d","text":"3,0002"}]'::jsonb, 'b', 'f(1) = 1 et f′(x) = 3x², donc f′(1) = 3. L''approximation affine donne f(1 + h) ≈ 1 + 3 × 0,0002 = 1,0006 ✓. L''option a oublie de multiplier h par f′(1) : elle donne 1 + h. L''option c se trompe d''une décimale en écrivant 3 × 0,0002 = 0,002. L''option d confond f(1) et f′(1) : elle calcule 3 + h.', 3, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('b90ba29c-3e23-5720-9cf2-2d5ef45d2a4f', '59da127f-c905-5b05-8171-aae940fcfff7', 'Soit f définie sur ℝ par f(x) = x² si x ≤ 1 et f(x) = 3x − 2 si x > 1. La fonction f est-elle dérivable en 1 ?', '[{"id":"a","text":"Oui, car f est continue en 1"},{"id":"b","text":"Oui, car f′_g(1) = f′_d(1) = 3"},{"id":"c","text":"Non, car f n''est pas continue en 1"},{"id":"d","text":"Non, car f′_g(1) = 2 et f′_d(1) = 3"}]'::jsonb, 'd', 'À gauche f(x) = x² donne f′_g(1) = 2 ; à droite f(x) = 3x − 2 donne f′_d(1) = 3. Comme 2 ≠ 3, f n''est pas dérivable en 1 ✓ : la courbe y présente un point anguleux. L''option a confond continuité et dérivabilité : f est bien continue en 1 (1² = 3 × 1 − 2 = 1), mais x ↦ |x| rappelle qu''une fonction continue peut ne pas être dérivable. L''option b dérive les deux morceaux avec la même règle. L''option c est fausse : la continuité en 1 est justement vérifiée.', 4, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('b049b1d7-42ae-5824-848b-cd8819c52893', '59da127f-c905-5b05-8171-aae940fcfff7', 'Soit f(x) = (x² + 1)(2x − 3) sur ℝ. Quelle est la dérivée f′(x) ?', '[{"id":"a","text":"4x"},{"id":"b","text":"6x² + 6x + 2"},{"id":"c","text":"6x² − 6x + 2"},{"id":"d","text":"2x² − 6x − 2"}]'::jsonb, 'c', 'Avec (f × g)′ = f′g + g′f : f′(x) = 2x(2x − 3) + 2(x² + 1) = 4x² − 6x + 2x² + 2 = 6x² − 6x + 2 ✓. L''option a applique la règle inventée « dérivée du produit = produit des dérivées » : 2x × 2 = 4x. L''option b se trompe de signe en développant 2x(2x − 3). L''option d utilise la règle du quotient (une soustraction) au lieu de celle du produit : 2x(2x − 3) − 2(x² + 1).', 5, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('aefb928a-efeb-542b-ac2c-918ee04ce81c', '59da127f-c905-5b05-8171-aae940fcfff7', 'Soit h(x) = (3x² + 1)⁴ sur ℝ. Quelle est la dérivée h′(x) ?', '[{"id":"a","text":"4(3x² + 1)³"},{"id":"b","text":"24x(3x² + 1)³"},{"id":"c","text":"6x(3x² + 1)³"},{"id":"d","text":"24x(3x² + 1)⁴"}]'::jsonb, 'b', 'On pose u(x) = 3x² + 1, donc u′(x) = 6x, et on applique (uⁿ)′ = nu′uⁿ⁻¹ : h′(x) = 4 × 6x × (3x² + 1)³ = 24x(3x² + 1)³ ✓. L''option a oublie la dérivée intérieure u′ = 6x : c''est l''erreur classique sur les composées. L''option c oublie le facteur n = 4. L''option d n''abaisse pas l''exposant de 4 à 3.', 6, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('982b3e29-ea69-5466-84fb-1004a2e7c9c8', '59da127f-c905-5b05-8171-aae940fcfff7', 'Soit f(x) = x⁴ − 2x³ + 5x sur ℝ. Quelle est la dérivée seconde f″(x) ?', '[{"id":"a","text":"4x³ − 6x² + 5"},{"id":"b","text":"12x² − 12x + 5"},{"id":"c","text":"24x − 12"},{"id":"d","text":"12x² − 12x"}]'::jsonb, 'd', 'On dérive deux fois : f′(x) = 4x³ − 6x² + 5, puis f″(x) = 12x² − 12x ✓. L''option a s''arrête à la dérivée première. L''option b garde la constante 5 alors que la dérivée d''une constante est nulle. L''option c va trop loin : 24x − 12 est la dérivée troisième f⁽³⁾(x).', 7, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('da4e7672-6a8c-5783-8285-ab92a9a8ed8c', '59da127f-c905-5b05-8171-aae940fcfff7', 'Soit f continue sur [a, b] (a < b) et dérivable sur ]a, b[. Quelle hypothèse supplémentaire le théorème de Rolle exige-t-il pour garantir l''existence d''un réel c de ]a, b[ tel que f′(c) = 0 ?', '[{"id":"a","text":"f(a) = f(b)"},{"id":"b","text":"f′(a) = f′(b)"},{"id":"c","text":"f est strictement monotone sur [a, b]"},{"id":"d","text":"f(a) × f(b) < 0"}]'::jsonb, 'a', 'Le théorème de Rolle s''énonce : f continue sur [a, b], dérivable sur ]a, b[ et f(a) = f(b) ⇒ il existe c de ]a, b[ tel que f′(c) = 0 ✓. Sans l''égalité des valeurs aux bornes la conclusion tombe : sur [0, 1], f(x) = x est continue et dérivable mais f′ ne s''annule jamais. L''option b invente une condition sur f′, qui n''est même pas supposée continue. L''option c est contradictoire : une fonction strictement monotone ne peut pas vérifier f(a) = f(b). L''option d est l''hypothèse du théorème des valeurs intermédiaires (existence d''une racine de f), pas celle de Rolle.', 8, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('43ec5197-c056-51d8-b8c3-34b217b51e07', '59da127f-c905-5b05-8171-aae940fcfff7', 'Soit f continue sur [1, 4] et dérivable sur ]1, 4[, avec 2 ≤ f′(x) ≤ 5 pour tout x de ]1, 4[. Quel encadrement de f(4) − f(1) l''inégalité des accroissements finis fournit-elle ?', '[{"id":"a","text":"2 ≤ f(4) − f(1) ≤ 5"},{"id":"b","text":"5 ≤ f(4) − f(1) ≤ 8"},{"id":"c","text":"6 ≤ f(4) − f(1) ≤ 15"},{"id":"d","text":"8 ≤ f(4) − f(1) ≤ 20"}]'::jsonb, 'c', 'Si m ≤ f′(x) ≤ M sur ]a, b[, alors m(b − a) ≤ f(b) − f(a) ≤ M(b − a). Ici b − a = 4 − 1 = 3, donc 2 × 3 ≤ f(4) − f(1) ≤ 5 × 3, c''est-à-dire 6 ≤ f(4) − f(1) ≤ 15 ✓. L''option a oublie complètement le facteur (b − a). L''option b ajoute 3 aux bornes au lieu de les multiplier. L''option d multiplie par b = 4 au lieu de b − a = 3.', 9, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('16675712-0717-5ea5-8d48-ab8800d8df97', '59da127f-c905-5b05-8171-aae940fcfff7', 'Soit f deux fois dérivable sur un intervalle ouvert contenant le réel a. On sait que f″ s''annule en a en changeant de signe. Que peut-on en déduire ?', '[{"id":"a","text":"f admet un extremum local en a"},{"id":"b","text":"Le point I(a, f(a)) est un point d''inflexion de C_f"},{"id":"c","text":"f′(a) = 0"},{"id":"d","text":"f est strictement croissante au voisinage de a"}]'::jsonb, 'b', 'C''est le théorème de la dérivée seconde : si f″ s''annule en a en changeant de signe, le point I(a, f(a)) est un point d''inflexion — la courbe y traverse sa tangente ✓. L''option a applique le critère au mauvais ordre de dérivation : c''est f′ (et non f″) qui, en s''annulant en changeant de signe, donne un extremum local. L''option c confond aussi f′ et f″ : rien n''impose ici que f′(a) soit nul (pour f(x) = x³ − 3x + 1, f″(0) = 0 alors que f′(0) = −3). L''option d parle du signe de f′, qui gouverne le sens de variation, et dont on ne sait rien ici.', 10, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order, correction_video) VALUES
  ('37e8d046-31af-5bf8-aa99-f25a884e8ee9', '3f2eb68a-bcd1-5538-b438-d78f8b67fa44', 'math-bac-math', '⭐ Exercice : Premiers pas avec la dérivée', 1, 50, 10, 'practice', 'admin', 1, NULL)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('98840e17-f7ad-5886-8986-2d4e9ac1839d', '37e8d046-31af-5bf8-aa99-f25a884e8ee9', 'Soit f(x) = 5x³ − 2x + 7 sur ℝ. Quelle est la dérivée f′(x) ?', '[{"id":"a","text":"3x² − 2"},{"id":"b","text":"15x² − 2"},{"id":"c","text":"15x² − 2x"},{"id":"d","text":"15x² − 2x + 7"}]'::jsonb, 'b', 'On dérive terme à terme : (5x³)′ = 15x², (−2x)′ = −2 et (7)′ = 0, donc f′(x) = 15x² − 2 ✓. L''option a oublie de multiplier par le coefficient 5. L''option c ne dérive pas −2x (elle le recopie). L''option d garde en plus la constante 7, dont la dérivée est pourtant nulle.', 1, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('30d161c4-269d-5980-bd56-efab42af495f', '37e8d046-31af-5bf8-aa99-f25a884e8ee9', 'Soit f(x) = x² + 4x sur ℝ. Quel est le nombre dérivé f′(3) ?', '[{"id":"a","text":"6"},{"id":"b","text":"21"},{"id":"c","text":"2x + 4"},{"id":"d","text":"10"}]'::jsonb, 'd', 'On calcule d''abord f′(x) = 2x + 4, puis on remplace x par 3 : f′(3) = 6 + 4 = 10 ✓. L''option a oublie de dériver le terme 4x et ne garde que 2 × 3. L''option b donne f(3) = 9 + 12 = 21, c''est-à-dire l''image et non le nombre dérivé. L''option c s''arrête à la fonction dérivée sans l''évaluer en 3.', 2, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('54c6da99-01c0-5e3d-80de-d04813afb43b', '37e8d046-31af-5bf8-aa99-f25a884e8ee9', 'Soit f(x) = x² − 2x sur ℝ. Quelle est l''équation de la tangente à C_f au point d''abscisse 1 ?', '[{"id":"a","text":"y = −1"},{"id":"b","text":"y = 0"},{"id":"c","text":"y = 2x − 3"},{"id":"d","text":"y = −x + 1"}]'::jsonb, 'a', 'On a f(1) = 1 − 2 = −1 et f′(x) = 2x − 2, donc f′(1) = 0. La tangente est y = f′(1)(x − 1) + f(1) = 0 × (x − 1) − 1 = −1 ✓ : elle est horizontale, à la hauteur −1. L''option b confond la pente f′(1) = 0 avec l''équation elle-même et oublie l''ordonnée f(1). L''option c oublie de dériver −2x, ce qui donne à tort f′(1) = 2. L''option d échange les rôles de f(1) et f′(1) : y = f(1)(x − 1) + f′(1).', 3, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('55b16647-8cd9-5a2e-9f44-f353eb54d56a', '37e8d046-31af-5bf8-aa99-f25a884e8ee9', 'Soit f(x) = sin(3x) sur ℝ. Quelle est la dérivée f′(x) ?', '[{"id":"a","text":"cos(3x)"},{"id":"b","text":"3sin(3x)"},{"id":"c","text":"3cos(3x)"},{"id":"d","text":"−3cos(3x)"}]'::jsonb, 'c', 'La table des dérivées usuelles donne (sin(ax + b))′ = a cos(ax + b) ; ici a = 3, donc f′(x) = 3cos(3x) ✓. L''option a oublie le facteur intérieur a = 3. L''option b ne change pas sin en cos. L''option d ajoute un signe moins, qui appartient à la dérivée du cosinus, pas à celle du sinus.', 4, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('75353a0a-b0a0-5e87-b370-5742993b965f', '37e8d046-31af-5bf8-aa99-f25a884e8ee9', 'Soit f(x) = x² − 6x + 5 sur ℝ. Quel est son sens de variation ?', '[{"id":"a","text":"Croissante sur ℝ"},{"id":"b","text":"Décroissante sur ℝ"},{"id":"c","text":"Croissante sur ]−∞, 3] puis décroissante sur [3, +∞["},{"id":"d","text":"Décroissante sur ]−∞, 3] puis croissante sur [3, +∞["}]'::jsonb, 'd', 'On a f′(x) = 2x − 6 = 2(x − 3) : f′ est strictement négative pour x < 3 et strictement positive pour x > 3. Donc f est strictement décroissante sur ]−∞, 3] puis strictement croissante sur [3, +∞[ ✓ (elle admet un minimum en 3). L''option c lit le signe de f′ à l''envers. Les options a et b supposent un signe constant, alors que f′ change de signe en 3.', 5, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('abd3eefb-dfe2-5784-be46-d0f71addfcdb', '37e8d046-31af-5bf8-aa99-f25a884e8ee9', 'Soit f(x) = (2x + 1)(x − 3) sur ℝ. Quelle est la dérivée f′(x) ?', '[{"id":"a","text":"4x − 5"},{"id":"b","text":"2"},{"id":"c","text":"4x + 5"},{"id":"d","text":"4x − 7"}]'::jsonb, 'a', 'Avec (f × g)′ = f′g + g′f : f′(x) = 2(x − 3) + 1 × (2x + 1) = 2x − 6 + 2x + 1 = 4x − 5 ✓. On peut vérifier en développant d''abord : f(x) = 2x² − 5x − 3, donc f′(x) = 4x − 5. L''option b multiplie les dérivées entre elles (2 × 1), ce qui n''est pas la règle du produit. L''option c se trompe de signe sur 2 × (−3). L''option d soustrait 1 au lieu de l''ajouter en développant 2x − 6 + 2x + 1.', 6, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order, correction_video) VALUES
  ('11344969-e225-5a3f-b391-99373cf44222', '3f2eb68a-bcd1-5538-b438-d78f8b67fa44', 'math-bac-math', '⚔️ Boss du chapitre ⭐⭐⭐ : Le lecteur de pentes', 3, 120, 30, 'boss', 'admin', 2, NULL)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('42a27a4b-2ec2-50fb-beea-171de3ea61fe', '11344969-e225-5a3f-b391-99373cf44222', 'Déterminer la dérivée sur ℝ de la fonction f : x ↦ cos(πx²).', '[{"id":"a","text":"f′(x) = 2πx sin(πx²)"},{"id":"b","text":"f′(x) = −sin(πx²)"},{"id":"c","text":"f′(x) = −2πx sin(πx²)"},{"id":"d","text":"f′(x) = −2πx cos(πx²)"}]'::jsonb, 'c', 'On pose u(x) = πx², donc u′(x) = 2πx, et f = cos∘u. La règle de la composée donne f′(x) = u′(x) × (−sin(u(x))) = −2πx sin(πx²) ✓ (contrôle : en x = 0, f′(0) = 0, cohérent avec le fait que f atteint son maximum 1 en 0, donc il y a une tangente horizontale ✓). Le piège courant : dériver la composée sans le facteur intérieur u′ donne −sin(πx²) ; oublier le signe « − » de la dérivée de cos donne 2πx sin(πx²) ; et remplacer sin par cos dans la dérivée conserve la fonction de départ au lieu de la transformer.', 1, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('92849556-2053-580a-bd6f-99249cee2a9c', '11344969-e225-5a3f-b391-99373cf44222', 'Soit f(x) = (x − 1)²(x − 3). Déterminer une équation de la tangente à C_f au point d''abscisse 2.', '[{"id":"a","text":"y = 2x − 5"},{"id":"b","text":"y = −x + 1"},{"id":"c","text":"y = −x + 2"},{"id":"d","text":"y = −x − 1"}]'::jsonb, 'b', 'On développe : f(x) = x³ − 5x² + 7x − 3, donc f′(x) = 3x² − 10x + 7. En a = 2 : f(2) = 8 − 20 + 14 − 3 = −1 et f′(2) = 12 − 20 + 7 = −1. La tangente est y = f′(2)(x − 2) + f(2) = −(x − 2) − 1 = −x + 1 ✓ (contrôle par le produit : f′ = 2(x − 1)(x − 3) + (x − 1)² = (x − 1)(3x − 7), donc f′(2) = 1 × (−1) = −1 ✓). Le piège courant : appliquer (uv)′ = u′v′ avec u = (x − 1)² et v = x − 3 donne f′ = 2(x − 1), soit f′(2) = 2 et la droite y = 2x − 5 ; oublier le terme f(a) donne y = −x + 2 ; et écrire la formule sous la forme fausse y = f′(a)x + f(a) fabrique y = −x − 1.', 2, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('66fd4bd9-d9d8-547d-818b-9eac0fe81a47', '11344969-e225-5a3f-b391-99373cf44222', 'Soit f définie sur ℝ par f(x) = |x² + x| si x < 0 et f(x) = x√x si x ≥ 0. Étudier la dérivabilité de f en 0.', '[{"id":"a","text":"f n''est pas dérivable en 0 : f′_g(0) = −1 et f′_d(0) = 0"},{"id":"b","text":"f est dérivable en 0 et f′(0) = 0"},{"id":"c","text":"f n''est pas dérivable en 0 : f′_g(0) = 1 et f′_d(0) = 0"},{"id":"d","text":"f n''est pas dérivable en 0 : la dérivée à droite est infinie (tangente verticale)"}]'::jsonb, 'a', 'D''abord f(0) = 0. À gauche : pour −1 < x < 0, x² + x = x(x + 1) est négatif, donc |x² + x| = −(x² + x) et le taux vaut (−(x² + x))/x = −(x + 1) → −1 quand x → 0⁻, d''où f′_g(0) = −1. À droite : le taux vaut (x√x)/x = √x → 0, d''où f′_d(0) = 0. Comme −1 ≠ 0, f n''est pas dérivable en 0 : C_f y présente un point anguleux ✓. Le piège courant : dériver sous la valeur absolue sans étudier le signe de x² + x sur ]−1 ; 0[ donne le taux x + 1 → 1 et la réponse f′_g(0) = 1 ; répondre « dérivable, f′(0) = 0 » ne regarde que la branche de droite ; et la tangente verticale est le comportement de √x en 0, pas celui de x√x, dont le taux √x tend bien vers 0.', 3, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('da07e6ac-f9b3-55e8-a6ca-d7ad3bd80e7d', '11344969-e225-5a3f-b391-99373cf44222', 'Soit h(x) = √((x − 1)/(x + 1)) sur ]1, +∞[. Déterminer h′(x).', '[{"id":"a","text":"h′(x) = 2/((x + 1)√(x² − 1))"},{"id":"b","text":"h′(x) = 1/(2√((x − 1)/(x + 1)))"},{"id":"c","text":"h′(x) = 2/(x + 1)²"},{"id":"d","text":"h′(x) = 1/((x + 1)√(x² − 1))"}]'::jsonb, 'd', 'On pose u(x) = (x − 1)/(x + 1), strictement positive sur ]1, +∞[. La formule du quotient donne u′(x) = ((x + 1) − (x − 1))/(x + 1)² = 2/(x + 1)². Puis (√u)′ = u′/(2√u) = [2/(x + 1)²] / (2√((x − 1)/(x + 1))) = 1/((x + 1)² × √(x − 1)/√(x + 1)) = 1/((x + 1)√((x − 1)(x + 1))) = 1/((x + 1)√(x² − 1)) ✓ (contrôle numérique en x = 3 : h(3) = √(1/2) ≈ 0,70711 et h(3,001) ≈ 0,70720, soit un taux ≈ 0,0884 ; la formule donne 1/(4√8) ≈ 0,0884 ✓). Le piège courant : oublier le facteur 2 du dénominateur de (√u)′ = u′/(2√u) double le résultat et fabrique 2/((x + 1)√(x² − 1)) ; dériver la composée sans le facteur intérieur u′ donne 1/(2√u) ; et 2/(x + 1)² n''est que u′, la dérivée de l''intérieur, confondue avec celle de h.', 4, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('e70e77d1-c163-5ca6-9fec-8227da19231b', '11344969-e225-5a3f-b391-99373cf44222', 'La courbe représentative de la fonction f : x ↦ (1/20)x⁵ − 4x² admet un point d''inflexion. Quelle est son abscisse ?', '[{"id":"a","text":"x = 0"},{"id":"b","text":"x = −2"},{"id":"c","text":"x = 2"},{"id":"d","text":"Aucune : f″ ne s''annule jamais"}]'::jsonb, 'c', 'On calcule f′(x) = (1/4)x⁴ − 8x, puis f″(x) = x³ − 8. Or x³ − 8 = 0 ⟺ x³ = 8 ⟺ x = 2, et f″ est strictement croissante : elle est négative pour x < 2 et positive pour x > 2, donc elle s''annule en 2 EN CHANGEANT DE SIGNE. Le point d''abscisse 2 est donc un point d''inflexion ✓. Le piège courant : confondre les racines de f′ et celles de f″ — f′(0) = 0 (tangente horizontale en 0), mais f″(0) = −8 ≠ 0, donc 0 n''est pas un point d''inflexion ; résoudre x³ = 8 en x = −2 inverse le signe (une racine cubique conserve le signe : (−2)³ = −8) ; et affirmer que f″ ne s''annule jamais oublie qu''un polynôme de degré 3 admet toujours au moins une racine réelle.', 5, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('5ae77f6d-557b-5c09-81bd-a0d68b4d81d4', '11344969-e225-5a3f-b391-99373cf44222', 'Une seule des affirmations suivantes est vraie. Laquelle ?', '[{"id":"a","text":"Si f est continue en a, alors f est dérivable en a"},{"id":"b","text":"Si f′(a) = 0, alors f admet un extremum local en a"},{"id":"c","text":"Si le produit de deux fonctions est dérivable en a, alors chacune des deux fonctions est dérivable en a"},{"id":"d","text":"Si f est dérivable sur [2 ; 5] et |f′(t)| ≤ 2 pour tout t de [2 ; 5], alors |f(5) − f(2)| ≤ 6"}]'::jsonb, 'd', 'Seule d est vraie ✓ : c''est le corollaire de l''inégalité des accroissements finis, |f(b) − f(a)| ≤ M|b − a| avec M = 2 et |5 − 2| = 3, d''où |f(5) − f(2)| ≤ 2 × 3 = 6. a est fausse : x ↦ |x| est continue en 0 mais n''y est pas dérivable (point anguleux) — l''implication ne marche que dans l''autre sens, dérivable ⟹ continue. b est fausse : pour f(x) = x³, f′(0) = 0 alors que f est strictement croissante et n''a aucun extremum ; f′(a) = 0 est nécessaire, jamais suffisant — il faut un changement de signe de f′. c est fausse : avec f(x) = g(x) = |x|, le produit fg(x) = x² est dérivable en 0 alors qu''aucun des deux facteurs ne l''est. Le piège courant : retenir ces énoncés « dans les deux sens » alors que chacun n''a qu''un sens de validité.', 6, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order, correction_video) VALUES
  ('c622662f-59a9-5f18-a4e3-e1ffb2262bec', '3f2eb68a-bcd1-5538-b438-d78f8b67fa44', 'math-bac-math', '⭐⭐ Révision (type examen) : Dérivabilité', 2, 75, 15, 'practice', 'admin', 3, NULL)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('6db1531d-8f1c-537b-ac03-b56132710219', 'c622662f-59a9-5f18-a4e3-e1ffb2262bec', 'Soit f(x) = (x + 1)/(x − 2) sur ]2, +∞[. Quelle est la dérivée f′(x) ?', '[{"id":"a","text":"−3/(x − 2)²"},{"id":"b","text":"3/(x − 2)²"},{"id":"c","text":"1"},{"id":"d","text":"(2x − 1)/(x − 2)²"}]'::jsonb, 'a', 'Avec (f/g)′ = (f′g − g′f)/g², où f′ = 1 et g′ = 1 : f′(x) = (1 × (x − 2) − 1 × (x + 1))/(x − 2)² = (x − 2 − x − 1)/(x − 2)² = −3/(x − 2)² ✓. La dérivée est donc strictement négative : f est décroissante sur ]2, +∞[. L''option b oublie le signe moins du numérateur. L''option c divise les dérivées entre elles (1/1), ce qui n''est pas la règle du quotient. L''option d additionne au lieu de soustraire au numérateur : (x − 2) + (x + 1) = 2x − 1.', 1, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('602908e7-e145-50ad-94d9-e183f5d1651d', 'c622662f-59a9-5f18-a4e3-e1ffb2262bec', 'Soit h(x) = √(x² + 3) sur ℝ. Quelle est la dérivée h′(x) ?', '[{"id":"a","text":"1/(2√(x² + 3))"},{"id":"b","text":"2x/√(x² + 3)"},{"id":"c","text":"x/(2√(x² + 3))"},{"id":"d","text":"x/√(x² + 3)"}]'::jsonb, 'd', 'On pose u(x) = x² + 3 (strictement positive), donc u′(x) = 2x, et (√u)′ = u′/(2√u) : h′(x) = 2x/(2√(x² + 3)) = x/√(x² + 3) ✓. L''option a oublie la dérivée intérieure u′ = 2x. L''option b oublie le 2 du dénominateur de la formule. L''option c simplifie mal : elle divise le numérateur par 2 sans supprimer le 2 du dénominateur.', 2, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('b87dc142-231b-57d7-b415-c8f783c1057b', 'c622662f-59a9-5f18-a4e3-e1ffb2262bec', 'Soit f(x) = x⁴ − 6x² + 1 sur ℝ. Quelles sont les abscisses des points d''inflexion de C_f ?', '[{"id":"a","text":"−1 et 1"},{"id":"b","text":"0 seulement"},{"id":"c","text":"−√6 et √6"},{"id":"d","text":"Aucune : une fonction du quatrième degré n''a pas de point d''inflexion"}]'::jsonb, 'a', 'On calcule f′(x) = 4x³ − 12x puis f″(x) = 12x² − 12 = 12(x² − 1) = 12(x − 1)(x + 1). La dérivée seconde s''annule en −1 et en 1, et elle y change de signe : f″ est positive sur ]−∞, −1[, négative sur ]−1, 1[, puis de nouveau positive sur ]1, +∞[. La courbe change donc de courbure en chacun de ces deux points : les abscisses des points d''inflexion sont −1 et 1 ✓. L''option « 0 seulement » vient de f′(x) = 4x(x² − 3), qui s''annule en 0 : c''est une racine de la dérivée PREMIÈRE (elle donne un extremum, pas une inflexion) — la confusion classique entre f′ et f″. L''option −√6 et √6 lit le coefficient 6 de l''énoncé et résout x⁴ − 6x² = 0, c''est-à-dire x²(x² − 6) = 0 : ce sont les racines de f privée de son terme constant, pas celles de f″. L''option « aucune » croit qu''une quartique garde une courbure constante ; ici f″ est un polynôme du second degré à deux racines simples, donc elle change bien de signe deux fois.', 3, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('f76589f6-8ca6-5b72-a02b-67ec6656695f', 'c622662f-59a9-5f18-a4e3-e1ffb2262bec', 'Soit f(x) = x² sur [0, 3]. Le théorème des accroissements finis garantit l''existence d''un réel c de ]0, 3[ tel que f(3) − f(0) = f′(c)(3 − 0). Que vaut c ?', '[{"id":"a","text":"3"},{"id":"b","text":"9/2"},{"id":"c","text":"3/2"},{"id":"d","text":"9"}]'::jsonb, 'c', 'On a f(3) − f(0) = 9 − 0 = 9 et f′(x) = 2x, donc l''égalité s''écrit 9 = 2c × 3 = 6c, d''où c = 3/2 ✓, qui appartient bien à ]0, 3[. L''option a donne la pente de la corde, c''est-à-dire f′(c) = 9/3 = 3, et non c lui-même. L''option b oublie le facteur (b − a) et résout 9 = 2c. L''option d confond c avec l''accroissement f(3) − f(0) = 9. On peut d''ailleurs vérifier que 9/2 et 9 sortent de l''intervalle ]0, 3[.', 4, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('409cab35-3ef2-5662-acca-2b58d6871827', 'c622662f-59a9-5f18-a4e3-e1ffb2262bec', 'Soit f(x) = x³ − 3x sur ℝ. Quels sont ses extrema locaux ?', '[{"id":"a","text":"Un minimum local égal à −2 en −1 et un maximum local égal à 2 en 1"},{"id":"b","text":"Un maximum local égal à 2 en −1 et un minimum local égal à −2 en 1"},{"id":"c","text":"Un minimum local égal à 0 en 0"},{"id":"d","text":"Aucun extremum : f est strictement croissante sur ℝ"}]'::jsonb, 'b', 'On a f′(x) = 3x² − 3 = 3(x − 1)(x + 1). Donc f′ > 0 sur ]−∞, −1[, f′ < 0 sur ]−1, 1[ et f′ > 0 sur ]1, +∞[. En −1, f′ passe du positif au négatif : maximum local f(−1) = −1 + 3 = 2. En 1, f′ passe du négatif au positif : minimum local f(1) = 1 − 3 = −2 ✓. L''option a lit le signe de f′ à l''envers et échange donc maximum et minimum. L''option c applique le raisonnement de x ↦ x², alors qu''ici f′(0) = −3 ≠ 0. L''option d est le comportement de x ↦ x³, dont la dérivée 3x² ne change jamais de signe ; ce n''est pas le cas ici.', 5, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('fc045e04-7117-5415-a40e-17e19291c475', 'c622662f-59a9-5f18-a4e3-e1ffb2262bec', 'Soit f(x) = √x sur [4, 9], de dérivée f′(x) = 1/(2√x), qui vérifie 1/6 ≤ f′(x) ≤ 1/4 sur ]4, 9[. Quel encadrement de √9 − √4 l''inégalité des accroissements finis fournit-elle ?', '[{"id":"a","text":"1/6 ≤ √9 − √4 ≤ 1/4"},{"id":"b","text":"4/6 ≤ √9 − √4 ≤ 1"},{"id":"c","text":"1/30 ≤ √9 − √4 ≤ 1/20"},{"id":"d","text":"5/6 ≤ √9 − √4 ≤ 5/4"}]'::jsonb, 'd', 'L''inégalité m(b − a) ≤ f(b) − f(a) ≤ M(b − a) s''applique avec m = 1/6, M = 1/4 et b − a = 9 − 4 = 5 : (1/6) × 5 ≤ √9 − √4 ≤ (1/4) × 5, soit 5/6 ≤ √9 − √4 ≤ 5/4 ✓. La valeur exacte est 3 − 2 = 1, bien comprise entre 5/6 ≈ 0,833 et 5/4 = 1,25. L''option a oublie de multiplier par (b − a). L''option b multiplie par a = 4 au lieu de b − a = 5. L''option c divise par 5 au lieu de multiplier.', 6, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order, correction_video) VALUES
  ('b469cc6b-f731-5016-9807-c571895eee07', '3f2eb68a-bcd1-5538-b438-d78f8b67fa44', 'math-bac-math', '👑 Défi élite ⭐⭐⭐⭐ : Le verrou des accroissements finis', 4, 300, 60, 'challenge', 'admin', 4, NULL)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('475d5279-f815-5bc3-9a64-2b2bf120bec7', 'b469cc6b-f731-5016-9807-c571895eee07', 'Soit f(x) = 1/√(1 + x) sur [0 ; 1]. On a établi que |f′(x)| ≤ 1/2, puis que 1 − x/2 ≤ f(x) ≤ 1 sur [0 ; 1]. En déduire un encadrement d''amplitude 10⁻¹⁰ de f(0,0000000002).', '[{"id":"a","text":"1 − 10⁻¹⁰ ≤ f(0,0000000002) ≤ 1"},{"id":"b","text":"1 − 2 × 10⁻¹⁰ ≤ f(0,0000000002) ≤ 1"},{"id":"c","text":"1 ≤ f(0,0000000002) ≤ 1 + 10⁻¹⁰"},{"id":"d","text":"1 − 10⁻¹⁰ ≤ f(0,0000000002) ≤ 1 + 10⁻¹⁰"}]'::jsonb, 'a', 'Ici x = 0,0000000002 = 2 × 10⁻¹⁰, qui appartient bien à [0 ; 1]. L''encadrement donne 1 − x/2 ≤ f(x) ≤ 1, soit 1 − (2 × 10⁻¹⁰)/2 = 1 − 10⁻¹⁰ ≤ f(x) ≤ 1 ✓. Son amplitude est exactement 1 − (1 − 10⁻¹⁰) = 10⁻¹⁰, conforme à la consigne. Le piège courant : reporter x tel quel sans le diviser par 2 donne la borne 1 − 2 × 10⁻¹⁰ (amplitude 2 × 10⁻¹⁰, deux fois trop large) ; inverser le sens oublie que f est strictement décroissante (f′ < 0), donc que f(x) ≤ f(0) = 1 et jamais l''inverse ; et symétriser en 1 ± 10⁻¹⁰ produit une amplitude 2 × 10⁻¹⁰ qui ne respecte pas la précision demandée.', 1, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('96b09db7-954b-58d9-ae5b-8305f708e34d', 'b469cc6b-f731-5016-9807-c571895eee07', 'Soit f(x) = (x² − 1)³(x⁴ − 16)² sur ℝ. Combien de racines de f′ distinctes de −2, −1, 1 et 2 le théorème de Rolle garantit-il, et par quel découpage ?', '[{"id":"a","text":"4 racines : une par racine de f"},{"id":"b","text":"2 racines : f ne s''annule qu''en −1 et 1"},{"id":"c","text":"3 racines : Rolle sur ]−2 ; −1[, ]−1 ; 1[ et ]1 ; 2["},{"id":"d","text":"1 seule racine : Rolle s''applique une fois, sur ]−2 ; 2["}]'::jsonb, 'c', 'f est polynomiale, donc continue et dérivable sur ℝ. Ses racines réelles : x² − 1 = 0 donne x = −1 et x = 1 ; x⁴ − 16 = 0 donne x⁴ = 16, soit x² = 4 (x² = −4 est impossible), d''où x = −2 et x = 2. f s''annule donc en quatre réels distincts −2 < −1 < 1 < 2. Sur chacun des trois intervalles consécutifs [−2 ; −1], [−1 ; 1] et [1 ; 2], f est continue, dérivable à l''intérieur et prend la même valeur 0 aux deux bornes : le théorème de Rolle fournit un c dans chaque intervalle OUVERT avec f′(c) = 0. Ces trois c sont distincts (les intervalles ouverts sont disjoints) et différents de −2, −1, 1, 2 ✓. Règle générale : n racines distinctes de f donnent n − 1 racines de f′ ; ici 4 − 1 = 3. Le piège courant : compter une racine de f′ par racine de f (4) confond les zéros de f et ceux de f′ ; ne retenir que ±1 oublie que x⁴ − 16 se factorise en (x² − 4)(x² + 4) et possède bien deux racines réelles ; et n''appliquer Rolle que sur l''intervalle extérieur ]−2 ; 2[ gaspille les zéros intermédiaires et ne garantit qu''une seule racine.', 2, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('b30033c5-c2ae-51b6-b184-06ec33f04388', 'b469cc6b-f731-5016-9807-c571895eee07', 'Soit f(x) = (1/3)x³ − x² sur ℝ, et A, B les points de C_f d''abscisses 1 et 3. Déterminer les abscisses c de ]1 ; 3[ où la tangente à C_f est parallèle à la droite (AB).', '[{"id":"a","text":"c = 2"},{"id":"b","text":"Deux valeurs conviennent : c = 1 + 2√3/3 et c = 1 − 2√3/3"},{"id":"c","text":"Aucune : f(1) ≠ f(3), donc le théorème de Rolle ne s''applique pas"},{"id":"d","text":"c = 1 + 2√3/3"}]'::jsonb, 'd', 'f(1) = 1/3 − 1 = −2/3 et f(3) = 9 − 9 = 0, donc le coefficient directeur de (AB) vaut (f(3) − f(1))/(3 − 1) = (2/3)/2 = 1/3. On cherche c avec f′(c) = 1/3, où f′(x) = x² − 2x : c² − 2c = 1/3, soit 3c² − 6c − 1 = 0. Le discriminant vaut 36 + 12 = 48, d''où c = (6 ± √48)/6 = 1 ± 2√3/3. Numériquement 2√3/3 ≈ 1,155, donc les racines sont ≈ 2,155 et ≈ −0,155 : seule la première appartient à ]1 ; 3[, donc c = 1 + 2√3/3 ✓ (contrôle : f′(2,155) = 2,155² − 2 × 2,155 ≈ 4,644 − 4,310 = 0,334 ≈ 1/3 ✓). Le piège courant : garder les deux racines sans vérifier l''appartenance à l''intervalle — le théorème des accroissements finis ne conclut que dans ]a ; b[ ; répondre c = 2 résout f′(c) = 0, c''est-à-dire le cas de Rolle (tangente HORIZONTALE), alors qu''ici la corde n''est pas horizontale ; et conclure « aucune » confond Rolle et le TAF : c''est justement parce que f(1) ≠ f(3) qu''on utilise le TAF, qui lui n''exige aucune égalité des valeurs aux bornes.', 3, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('1ad63e0c-ac04-5fe5-b865-5792f8489c33', 'b469cc6b-f731-5016-9807-c571895eee07', 'Soit x et y de [π/6 ; π/4] avec x < y. En appliquant l''inégalité des accroissements finis à la fonction sinus, on obtient un encadrement de sin y − sin x. En le spécialisant à x = π/6 et y = π/4, quel encadrement de (√2 − 1)/π obtient-on ?', '[{"id":"a","text":"√3/12 ≤ (√2 − 1)/π ≤ √2/12"},{"id":"b","text":"√2/12 ≤ (√2 − 1)/π ≤ √3/12"},{"id":"c","text":"1/12 ≤ (√2 − 1)/π ≤ √3/12"},{"id":"d","text":"√2/24 ≤ (√2 − 1)/π ≤ √3/24"}]'::jsonb, 'b', 'La dérivée de sinus est cosinus, décroissante sur [π/6 ; π/4] : pour tout t de cet intervalle, cos(π/4) ≤ cos t ≤ cos(π/6), soit √2/2 ≤ cos t ≤ √3/2. L''inégalité des accroissements finis donne alors (√2/2)(y − x) ≤ sin y − sin x ≤ (√3/2)(y − x). Avec x = π/6 et y = π/4 : y − x = π/12 et sin(π/4) − sin(π/6) = √2/2 − 1/2 = (√2 − 1)/2. On obtient (√2/2)(π/12) ≤ (√2 − 1)/2 ≤ (√3/2)(π/12) ; en multipliant les trois membres par 2/π (positif, le sens est conservé) : √2/12 ≤ (√2 − 1)/π ≤ √3/12 ✓ (contrôle : (√2 − 1)/π ≈ 0,1319, encadré par √2/12 ≈ 0,1179 et √3/12 ≈ 0,1443 ✓). Le piège courant : lire cosinus comme croissant sur [π/6 ; π/4] échange m et M et retourne l''encadrement ; prendre les valeurs de SINUS aux bornes (1/2 et √2/2) au lieu de celles de la DÉRIVÉE fabrique la borne 1/12 ; et multiplier par 1/π au lieu de 2/π divise tout par 2 et donne des bornes en /24.', 4, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('25eeba31-9438-566c-b61f-e892f64f821b', 'b469cc6b-f731-5016-9807-c571895eee07', 'Soit f dérivable sur [0 ; 1] avec f(0) = f′(0) = 0 et f(1) = 0. On veut prouver qu''il existe c de ]0 ; 1[ tel que cf′(c) − f(c) = 0. À quelle fonction auxiliaire faut-il appliquer le théorème de Rolle ?', '[{"id":"a","text":"À g définie par g(x) = f(x)/x si x ≠ 0 et g(0) = 0"},{"id":"b","text":"À f elle-même sur [0 ; 1], puisque f(0) = f(1) = 0"},{"id":"c","text":"À la fonction x ↦ xf(x) sur [0 ; 1]"},{"id":"d","text":"À la fonction x ↦ f(x) − xf′(x) sur [0 ; 1]"}]'::jsonb, 'a', 'La quantité cf′(c) − f(c) est exactement le numérateur de la dérivée du QUOTIENT f(x)/x : pour x ≠ 0, g′(x) = (xf′(x) − f(x))/x². Il faut donc annuler g′, d''où le choix de g ✓. Les hypothèses valident Rolle : g est continue sur [0 ; 1] — en 0, g(x) = f(x)/x = (f(x) − f(0))/(x − 0) → f′(0) = 0 = g(0) — dérivable sur ]0 ; 1[ comme quotient de fonctions dérivables à dénominateur non nul, et g(0) = 0 = f(1)/1 = g(1). Rolle fournit c de ]0 ; 1[ avec g′(c) = 0, donc cf′(c) − f(c) = 0 ✓. Le piège courant : appliquer Rolle à f directement donne f′(c) = 0, un résultat vrai mais qui n''est PAS la relation demandée (il faudrait de plus f(c) = 0) ; choisir le produit x ↦ xf(x) exécute la mauvaise formule, sa dérivée valant f(x) + xf′(x) — signe « + », donc jamais l''expression voulue ; et x ↦ f(x) − xf′(x) est l''expression cherchée elle-même, dont on ignore la dérivabilité (f′ n''est pas supposée dérivable) : Rolle s''applique à une fonction dont on annule la DÉRIVÉE, pas à la conclusion.', 5, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('fbba2a1a-67d6-5516-a940-ecf2e6cc4817', 'b469cc6b-f731-5016-9807-c571895eee07', 'Soit 𝒫 la parabole d''équation y = x² + px + q, et A, B deux points distincts de 𝒫 d''abscisses α et β. En quelle abscisse c la tangente à 𝒫 est-elle parallèle à (AB) ?', '[{"id":"a","text":"c = (α + β − p)/2"},{"id":"b","text":"c = √(αβ)"},{"id":"c","text":"c = (α + β)/2"},{"id":"d","text":"Le TAF garantit l''existence de c, mais sa valeur dépend de p et de q"}]'::jsonb, 'c', 'Posons f(x) = x² + px + q, continue et dérivable sur ℝ. Le coefficient directeur de (AB) vaut (f(β) − f(α))/(β − α) = ((β² − α²) + p(β − α))/(β − α) = (α + β) + p (on simplifie par β − α ≠ 0). Comme f′(x) = 2x + p, on résout 2c + p = α + β + p, d''où 2c = α + β et c = (α + β)/2 : c''est toujours le MILIEU des abscisses, quels que soient p et q ✓ (contrôle avec p = q = 0, α = 1, β = 3 : pente de (AB) = (9 − 1)/2 = 4 et f′(2) = 4 ✓, et 2 = (1 + 3)/2 ✓). Le piège courant : oublier de simplifier le p présent des DEUX côtés de l''équation 2c + p = α + β + p fabrique c = (α + β − p)/2 ; la moyenne géométrique √(αβ) n''a aucun sens ici (et n''existe même pas si αβ < 0) ; et croire que c dépend des paramètres confond l''existence donnée par le TAF avec le calcul explicite, qui montre justement que les termes en p s''éliminent et que q, constante, disparaît à la dérivation.', 6, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order, correction_video) VALUES
  ('33a349d3-3fbb-5c74-89d8-4707cac43b93', '3f2eb68a-bcd1-5538-b438-d78f8b67fa44', 'math-bac-math', '🧩 Mission interactive ⭐⭐ : L''atelier des pentes', 2, 75, 15, 'practice', 'admin', 5, NULL)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('67dda20a-5de1-55bc-9048-47a65670f4a2', '33a349d3-3fbb-5c74-89d8-4707cac43b93', 'Texte à trous — soit h(x) = √(4x − x²), définie sur ]0 ; 4[. On pose u(x) = 4x − x², donc u′(x) = 4 − 2x, et u > 0 sur ]0 ; 4[. En appliquant la formule de dérivation d''une composée (√u)′ = u′/(2√u), on obtient, après simplification, h′(x) = ____ pour tout x de ]0 ; 4[.', '[{"id":"a","text":"(2 − x)/√(4x − x²)"},{"id":"b","text":"(4 − 2x)/√(4x − x²)"},{"id":"c","text":"1/(2√(4x − x²))"},{"id":"d","text":"(2 − x)/(2√(4x − x²))"}]'::jsonb, 'a', 'On applique (√u)′ = u′/(2√u) avec u = 4x − x² et u′ = 4 − 2x : h′(x) = (4 − 2x)/(2√(4x − x²)). Le numérateur 4 − 2x vaut 2(2 − x), et ce facteur 2 se simplifie avec le 2 du dénominateur, d''où h′(x) = (2 − x)/√(4x − x²) ✓. La réponse (4 − 2x)/√(4x − x²) garde bien u′ au numérateur mais oublie purement et simplement la division par 2 imposée par la formule : elle vaut le double du résultat. La réponse 1/(2√(4x − x²)) applique la dérivée de la racine seule, (√x)′ = 1/(2√x), sans multiplier par u′ : c''est l''oubli du facteur intérieur, l''erreur reine de la composée. Et (2 − x)/(2√(4x − x²)) divise deux fois par 2 : une fois en simplifiant 4 − 2x en 2 − x, une seconde fois en conservant le 2 du dénominateur — elle vaut la moitié du résultat.', 1, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('a3d1eeae-78f0-5913-9cca-096328e2d2f5', '33a349d3-3fbb-5c74-89d8-4707cac43b93', 'Chasse à l''erreur — un élève dérive f(x) = (x² − 3x)⁵ sur ℝ :
① On pose u(x) = x² − 3x, de sorte que f = u⁵.
② On calcule u′(x) = 2x − 3.
③ La formule (uⁿ)′ = nu′uⁿ⁻¹ donne f′(x) = 5(2x − 3)(x² − 3x)⁵.
④ Conclusion : f′(x) = 5(2x − 3)(x² − 3x)⁵ pour tout réel x.
Une seule étape contient une erreur : laquelle ?', '[{"id":"a","text":"L''étape ①"},{"id":"b","text":"L''étape ②"},{"id":"c","text":"L''étape ③"},{"id":"d","text":"L''étape ④"}]'::jsonb, 'c', 'L''erreur est à l''étape ③ : la formule (uⁿ)′ = nu′uⁿ⁻¹ abaisse l''exposant d''une unité, donc avec n = 5 il faut écrire u⁴ et non u⁵. Étape corrigée : f′(x) = 5(2x − 3)(x² − 3x)⁴ ✓ — et le résultat final annoncé en ④ est donc faux lui aussi, puisqu''il recopie ③. Le choix de u en ① est correct (f est bien la puissance cinquième de u), et u′(x) = 2x − 3 en ② est exact (dérivée de x² − 3x). Le seul défaut est l''exposant non abaissé : n multiplie devant, n − 1 reste en exposant.', 2, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('9bd6f3b9-56ae-5534-9909-e9a7f1ed6b8b', '33a349d3-3fbb-5c74-89d8-4707cac43b93', 'Appariement — associe chaque fonction (①②③), définie là où elle a un sens, à sa dérivée (a, b, c, d) :
① f(x) = cos(2x)
② f(x) = sin(2x)
③ f(x) = tan(2x)
a. 2cos(2x)
...
c. 2(1 + tan²(2x))
d. −2cos(2x)
Quel appariement est entièrement correct ?', '[{"id":"a","text":"①-a · ②-b · ③-c"},{"id":"b","text":"①-b · ②-a · ③-c"},{"id":"c","text":"①-d · ②-a · ③-c"},{"id":"d","text":"①-b · ②-d · ③-c"}]'::jsonb, 'b', 'La table du cours donne (sin(ax + b))′ = a cos(ax + b), (cos(ax + b))′ = −a sin(ax + b) et (tan(ax + b))′ = a(1 + tan²(ax + b)). Avec a = 2 : cos(2x) a pour dérivée −2sin(2x), sin(2x) a pour dérivée 2cos(2x), et tan(2x) a pour dérivée 2(1 + tan²(2x)). Le bon appariement est donc ①-b · ②-a · ③-c ✓. Apparier ①-a · ②-b échange purement et simplement les deux dérivées : c''est la confusion classique « la dérivée de cos est cos, celle de sin est sin ». Apparier ①-d garde bien le signe − mais conserve le cosinus (−2cos(2x)) : dériver un cosinus donne un SINUS, changé de signe. Apparier ②-d ajoute au contraire un signe − parasite à la dérivée de sin(2x), qui est positive. La dérivée d est ainsi la fausse piste des deux erreurs de signe.', 3, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('cc8bc873-270d-5963-8440-60c0bf1035f2', '33a349d3-3fbb-5c74-89d8-4707cac43b93', 'Remise en ordre — on veut étudier les variations et les extrema de f(x) = x³ − 3x + 1 sur ℝ. Quatre étapes sont données dans le désordre :
① Dresser le tableau de variations : f est croissante sur ]−∞, −1], décroissante sur [−1, 1], croissante sur [1, +∞[.
② Calculer f′(x) = 3x² − 3 = 3(x − 1)(x + 1).
③ Conclure que f admet un maximum local en −1, égal à 3, et un minimum local en 1, égal à −1.
④ Étudier le signe de f′(x) : positif sur ]−∞, −1[, négatif sur ]−1, 1[, positif sur ]1, +∞[.
Quel est l''ordre correct des étapes ?', '[{"id":"a","text":"② → ① → ④ → ③"},{"id":"b","text":"④ → ② → ① → ③"},{"id":"c","text":"③ → ② → ④ → ①"},{"id":"d","text":"② → ④ → ① → ③"}]'::jsonb, 'd', 'L''ordre correct est ② → ④ → ① → ③ ✓ : on calcule d''abord la dérivée et on la factorise (②), ce qui permet d''en lire le SIGNE (④) ; le signe de f′ donne le sens de variation, donc le tableau (①), et le tableau met en évidence les extrema, là où f′ s''annule en changeant de signe (③). L''ordre ② → ① → ④ → ③ dresse le tableau avant d''avoir étudié le signe : or c''est exactement le signe de f′ qui remplit le tableau. L''ordre ④ → ② → … étudie le signe d''une dérivée pas encore calculée. Et ③ → … annonce les extrema en tête, avant tout calcul : on ne peut affirmer un maximum en −1 qu''après avoir vu f′ changer de signe en ce point.', 4, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('b106c744-2752-50c4-a5f4-e9dc1f4dd6c4', '33a349d3-3fbb-5c74-89d8-4707cac43b93', 'Vrai ou faux ? On considère l''affirmation : « Soit f dérivable sur un intervalle ouvert I et a un réel de I. Si f′(a) = 0, alors f admet un extremum local en a. » Choisis le verdict accompagné de la justification correcte.', '[{"id":"a","text":"Vrai : f′(a) = 0 signifie que la tangente en a est horizontale, ce qui caractérise un extremum local"},{"id":"b","text":"Faux : il faudrait en plus supposer que f″(a) = 0 pour pouvoir conclure à un extremum en a"},{"id":"c","text":"Faux : f′(a) = 0 est une condition nécessaire mais pas suffisante — pour f(x) = x³ on a f′(0) = 0 sans extremum ; il faut que f′ CHANGE de signe en a"},{"id":"d","text":"Vrai : c''est la condition d''extremum du cours, qui affirme l''équivalence entre f′(a) = 0 et extremum local en a"}]'::jsonb, 'c', 'L''affirmation est FAUSSE, et la bonne raison est le contre-exemple du cours : pour f(x) = x³, f′(x) = 3x² s''annule en 0, pourtant f est strictement croissante sur ℝ et n''admet aucun extremum en 0 — car f′ = 3x² reste positive de part et d''autre, elle ne change pas de signe ✓. Le cours énonce deux implications distinctes : « extremum local en a ⟹ f′(a) = 0 » (nécessaire) et « f′ s''annule en a EN CHANGEANT DE SIGNE ⟹ extremum local en a » (suffisant). Les deux verdicts « Vrai » lisent la première implication comme une équivalence : une tangente horizontale peut aussi être celle d''un point d''inflexion. Et exiger f″(a) = 0 ne corrige rien : pour f(x) = x², qui admet bien un minimum en 0, on a f″(0) = 2 ≠ 0 — cette condition écarterait justement le cas le plus simple.', 5, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('135f03eb-c78b-56ba-880f-50442e7be64d', '33a349d3-3fbb-5c74-89d8-4707cac43b93', 'QCM visuel — dans le repère ci-dessous, la courbe C_f est tracée en vert et sa tangente au point A d''abscisse 1 en orange. Lis sur le quadrillage l''ordonnée de A et la pente de la tangente, puis donne l''équation de cette tangente. <svg viewBox="0 0 300 200"><g stroke="#cbd5e1" stroke-width="1"><line x1="60" y1="10" x2="60" y2="170"/><line x1="100" y1="10" x2="100" y2="170"/><line x1="140" y1="10" x2="140" y2="170"/><line x1="180" y1="10" x2="180" y2="170"/><line x1="220" y1="10" x2="220" y2="170"/><line x1="60" y1="170" x2="220" y2="170"/><line x1="60" y1="130" x2="220" y2="130"/><line x1="60" y1="90" x2="220" y2="90"/><line x1="60" y1="50" x2="220" y2="50"/><line x1="60" y1="10" x2="220" y2="10"/></g><path d="M35 170 H250" fill="none" stroke="#0f172a" stroke-width="1.6"/><path d="M250 170 l-7 -3.5 l0 7 z" fill="#0f172a"/><path d="M60 190 V8" fill="none" stroke="#0f172a" stroke-width="1.6"/><path d="M60 8 l-3.5 7 l7 0 z" fill="#0f172a"/><polyline points="60,170 80,100 100,50 120,20 140,10 160,20 180,50 200,100 220,170" fill="none" stroke="#0f6e56" stroke-width="2.6" stroke-linejoin="round" stroke-linecap="round"/><line x1="40" y1="170" x2="120" y2="10" stroke="#b45309" stroke-width="2.2"/><circle cx="100" cy="50" r="4" fill="#0f172a"/><text x="108" y="46" font-size="12" font-weight="700" fill="#0f172a">A</text><text x="100" y="186" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">1</text><text x="140" y="186" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">2</text><text x="180" y="186" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">3</text><text x="220" y="186" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">4</text><text x="52" y="134" text-anchor="end" font-size="11" font-weight="700" fill="#0f172a">1</text><text x="52" y="94" text-anchor="end" font-size="11" font-weight="700" fill="#0f172a">2</text><text x="52" y="54" text-anchor="end" font-size="11" font-weight="700" fill="#0f172a">3</text><text x="52" y="14" text-anchor="end" font-size="11" font-weight="700" fill="#0f172a">4</text><text x="52" y="184" text-anchor="end" font-size="11" font-weight="700" fill="#0f172a">0</text></svg>', '[{"id":"a","text":"y = 2x + 1"},{"id":"b","text":"y = 2x + 3"},{"id":"c","text":"y = 3x + 2"},{"id":"d","text":"y = 2(x − 3) + 1"}]'::jsonb, 'a', 'Le point A est sur la courbe à l''abscisse 1 et à l''ordonnée 3 : f(1) = 3. La tangente orange passe par les nœuds du quadrillage (0 ; 1) et (1 ; 3) : en avançant de 1 vers la droite on monte de 2, sa pente vaut donc f′(1) = 2. L''équation de la tangente au point d''abscisse a est y = f′(a)(x − a) + f(a), soit y = 2(x − 1) + 3 = 2x − 2 + 3 = 2x + 1 ✓. Écrire y = 2x + 3 utilise la forme fautive y = f′(a)x + f(a) : on oublie le décalage (x − a), qui n''est nul que si a = 0. Écrire y = 3x + 2 échange la pente et l''ordonnée : 3 est la valeur de f(1), pas celle de f′(1). Et y = 2(x − 3) + 1 place l''abscisse et l''ordonnée de A à l''envers dans la formule.', 6, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('09f66223-7040-59b3-bcb5-7b1fe60e3c77', '33a349d3-3fbb-5c74-89d8-4707cac43b93', 'Lecture de document — voici le tableau de variations d''une fonction f, dérivable sur l''intervalle [−2 ; 4]. Que peut-on affirmer ? <svg viewBox="0 0 320 165"><g stroke="#0f172a" stroke-width="1.4" fill="none"><line x1="20" y1="20" x2="300" y2="20"/><line x1="20" y1="50" x2="300" y2="50"/><line x1="20" y1="80" x2="300" y2="80"/><line x1="20" y1="155" x2="300" y2="155"/><line x1="20" y1="20" x2="20" y2="155"/><line x1="70" y1="20" x2="70" y2="155"/><line x1="300" y1="20" x2="300" y2="155"/></g><text x="45" y="40" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">x</text><text x="45" y="70" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">f′(x)</text><text x="45" y="122" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">f(x)</text><text x="95" y="40" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">−2</text><text x="185" y="40" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">1</text><text x="275" y="40" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">4</text><text x="140" y="70" text-anchor="middle" font-size="13" font-weight="700" fill="#0f6e56">+</text><text x="185" y="70" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">0</text><text x="232" y="70" text-anchor="middle" font-size="13" font-weight="700" fill="#b45309">−</text><g stroke="#0f172a" stroke-width="2" fill="none"><path d="M105 142 L172 100"/><path d="M198 100 L265 142"/></g><path d="M172 100 l-9 1.5 l4 5.5 z" fill="#0f172a"/><path d="M265 142 l-4 -8 l-4.5 5 z" fill="#0f172a"/><text x="100" y="152" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">−3</text><text x="185" y="96" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">5</text><text x="272" y="152" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">1</text></svg>', '[{"id":"a","text":"f admet un minimum local en 1, égal à 5, car f′ s''y annule"},{"id":"b","text":"f′ s''annule en 1 en changeant de signe (de + à −) : f admet en 1 un maximum, égal à 5"},{"id":"c","text":"Le point d''abscisse 1 est un point d''inflexion de C_f, puisque la dérivée s''y annule"},{"id":"d","text":"f est croissante sur [−2 ; 4] tout entier, car f′ ne s''annule qu''en un seul point"}]'::jsonb, 'b', 'Le tableau se lit ainsi : f′ est positive sur ]−2 ; 1[ (f croît de −3 à 5), s''annule en 1, puis est négative sur ]1 ; 4[ (f décroît de 5 à 1). La dérivée s''annule en 1 EN CHANGEANT DE SIGNE, du positif vers le négatif : f admet donc en 1 un maximum, de valeur 5 ✓. Parler de minimum en 1 lit le sens des flèches à l''envers : monter puis descendre dessine un sommet, pas un creux. Un point d''inflexion se détecte avec la dérivée SECONDE f″ qui change de signe, pas avec l''annulation de f′ — que la tangente soit horizontale ne dit rien de la courbure. Enfin, le théorème « f′ ≥ 0 s''annulant en des points isolés ⟹ f strictement croissante » exige que f′ garde un signe constant : ici f′ devient franchement négative après 1, donc f décroît sur [1 ; 4].', 7, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order, correction_video) VALUES
  ('a093e497-76a7-5dc4-90e2-94b61e1c646f', '3f2eb68a-bcd1-5538-b438-d78f8b67fa44', 'math-bac-math', '🗿 Histoire ⭐⭐⭐ : Le Sanctuaire de la Courbe', 3, 120, 30, 'boss', 'admin', 6, NULL)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('bf87c289-97f2-5ab4-be37-99865a9b60d3', 'a093e497-76a7-5dc4-90e2-94b61e1c646f', 'Épisode 1 — Héros de la section Mathématiques, tu franchis le portail du Sanctuaire de la Courbe. Gravée dans la pierre, une unique fonction règne sur les lieux : f(x) = x³ − 3x² + 4, définie sur ℝ. La première épreuve est celle du Rayon de Pente : « Trace la tangente à C_f au point A d''abscisse 3, et je te laisserai passer. » Sachant que f′(x) = 3x² − 6x, quelle est l''équation de cette tangente ?', '[{"id":"a","text":"y = 9x − 23"},{"id":"b","text":"y = 9x + 4"},{"id":"c","text":"y = 4(x − 3) + 9"},{"id":"d","text":"y = 9x − 27"}]'::jsonb, 'a', 'On calcule les deux ingrédients : f(3) = 27 − 27 + 4 = 4 et f′(3) = 3 × 9 − 6 × 3 = 27 − 18 = 9. La tangente au point d''abscisse a a pour équation y = f′(a)(x − a) + f(a), soit ici y = 9(x − 3) + 4 = 9x − 27 + 4 = 9x − 23 ✓. Écrire y = 9x + 4 utilise la forme fautive y = f′(a)x + f(a), qui oublie le décalage (x − a) : elle n''est correcte que lorsque a = 0. L''équation y = 4(x − 3) + 9 échange les rôles de f(3) et f′(3) : la PENTE est le nombre dérivé, jamais l''image. Et y = 9x − 27 développe 9(x − 3) mais oublie d''ajouter f(3) = 4 : la droite obtenue a la bonne pente, mais elle ne passe pas par A.', 1, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('54da0878-98ca-5af4-b3b9-db20628a35ec', 'a093e497-76a7-5dc4-90e2-94b61e1c646f', 'Épisode 2 — Au bout du couloir, la Forge des Dérivées s''illumine. La même fonction f(x) = x³ − 3x² + 4 (sur ℝ) est projetée sur l''enclume, et le forgeron gronde : « Donne-moi sa dérivée, et surtout sa forme factorisée — sans elle, tu ne liras jamais son signe. » Quelle réponse est exacte ?', '[{"id":"a","text":"f′(x) = 3x² − 6x + 4"},{"id":"b","text":"f′(x) = 3x² − 3x = 3x(x − 1)"},{"id":"c","text":"f′(x) = 3x² − 6x = 3x(x − 2)"},{"id":"d","text":"f′(x) = 3x² − 6x = 3x(x + 2)"}]'::jsonb, 'c', 'On dérive terme à terme : (x³)′ = 3x², (−3x²)′ = −6x et la dérivée de la constante 4 est nulle. Donc f′(x) = 3x² − 6x, que l''on factorise en mettant 3x en facteur : 3x² − 6x = 3x(x − 2) ✓. Garder « + 4 » dérive la constante en elle-même : une constante disparaît toujours à la dérivation. Écrire 3x² − 3x oublie de multiplier −3 par l''exposant 2 : la dérivée de −3x² est −3 × 2x = −6x, et non −3x. Enfin, la factorisation 3x(x + 2) donne, en redéveloppant, 3x² + 6x : le signe du second terme est faux — un contrôle de sécurité consiste toujours à redévelopper sa factorisation.', 2, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('38a2b932-9d71-52dd-84d0-8ac75521ac0f', 'a093e497-76a7-5dc4-90e2-94b61e1c646f', 'Épisode 3 — La Salle des Signes s''ouvre : au sol, une immense droite des réels attend que tu y places les zones lumineuses (f′ > 0) et les zones d''ombre (f′ < 0). Rappel des données gravées : f(x) = x³ − 3x² + 4 sur ℝ, de dérivée f′(x) = 3x² − 6x = 3x(x − 2). Quel relevé du signe de f′ est correct ?', '[{"id":"a","text":"f′(x) < 0 sur ]−∞ ; 0[ et sur ]2 ; +∞[, et f′(x) > 0 sur ]0 ; 2["},{"id":"b","text":"f′(x) > 0 sur ]−∞ ; 0[ et sur ]2 ; +∞[, et f′(x) < 0 sur ]0 ; 2["},{"id":"c","text":"f′(x) > 0 sur ℝ tout entier, car le coefficient dominant de 3x² − 6x est positif"},{"id":"d","text":"f′(x) ≥ 0 sur ℝ, en ne s''annulant qu''en 0 et en 2 : f est donc strictement croissante sur ℝ"}]'::jsonb, 'b', 'f′(x) = 3x(x − 2) est un trinôme de racines 0 et 2, de coefficient dominant 3 > 0 : il est donc positif À L''EXTÉRIEUR de ses racines et négatif entre elles. D''où f′(x) > 0 sur ]−∞ ; 0[ et sur ]2 ; +∞[, et f′(x) < 0 sur ]0 ; 2[ ✓ (vérification directe : f′(1) = 3 × 1 × (1 − 2) = −3 < 0, et f′(3) = 3 × 3 × 1 = 9 > 0). Le relevé inversé applique la règle des trinômes à coefficient dominant NÉGATIF. Affirmer f′ > 0 partout confond le signe du coefficient dominant avec le signe du trinôme : un trinôme à racines réelles distinctes change forcément de signe deux fois. Enfin, prétendre f′ ≥ 0 partout est faux dès x = 1 ; le théorème « f′ ≥ 0 s''annulant en des points isolés ⟹ f strictement croissante » ne s''applique que si f′ garde bien un signe constant, ce qui n''est pas le cas ici.', 3, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('fbd6ecfd-7dfb-5345-a738-8a72a9182bca', 'a093e497-76a7-5dc4-90e2-94b61e1c646f', 'Épisode 4 — Le Tableau des Variations, immense grille de cristal, exige d''être rempli avant que la voûte ne s''ouvre. Données rappelées : f(x) = x³ − 3x² + 4, f′(x) = 3x(x − 2), avec f′ > 0 sur ]−∞ ; 0[ et ]2 ; +∞[, et f′ < 0 sur ]0 ; 2[. Quelle conclusion sur les extrema de f est correcte ?', '[{"id":"a","text":"f admet un maximum local en 0, égal à 4, et un maximum local en 2, égal à 0"},{"id":"b","text":"f admet un minimum local en 0, égal à 4, et un maximum local en 2, égal à 0"},{"id":"c","text":"f admet un maximum local en 0, égal à 0, et un minimum local en 2, égal à 4"},{"id":"d","text":"f admet un maximum local en 0, égal à 4, et un minimum local en 2, égal à 0"}]'::jsonb, 'd', 'Le signe de f′ donne les variations : f croît sur ]−∞ ; 0], décroît sur [0 ; 2], puis croît sur [2 ; +∞[. En 0, f′ passe du + au − : c''est un MAXIMUM local, de valeur f(0) = 4. En 2, f′ passe du − au + : c''est un MINIMUM local, de valeur f(2) = 8 − 12 + 4 = 0 ✓. Annoncer deux maxima ignore que le second changement de signe se fait en sens inverse du premier — entre deux sommets, la courbe doit bien remonter par un creux. Le couple « minimum en 0, maximum en 2 » lit les deux changements de signe à l''envers. Et intervertir les valeurs (0 en 0, 4 en 2) confond les images f(0) = 4 et f(2) = 0. Attention enfin : ces extrema sont bien LOCAUX et non globaux, puisque f(x) tend vers −∞ en −∞ et vers +∞ en +∞.', 4, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('a11b7ff6-1504-559b-a25f-f352de2c5f11', 'a093e497-76a7-5dc4-90e2-94b61e1c646f', 'Épisode 5 — La voûte s''ouvre sur le Pont des Deux Piliers. Les piliers, d''abscisses −1 et 2, sont exactement à la même hauteur : f(−1) = −1 − 3 + 4 = 0 et f(2) = 8 − 12 + 4 = 0, avec toujours f(x) = x³ − 3x² + 4 et f′(x) = 3x(x − 2). Le gardien du pont exige : « Prouve qu''entre les deux piliers, la courbe possède une tangente horizontale — et dis-moi où. » Quelle réponse est correcte ?', '[{"id":"a","text":"Théorème de Rolle : f est continue sur [−1 ; 2], dérivable sur ]−1 ; 2[ et f(−1) = f(2), donc il existe c ∈ ]−1 ; 2[ tel que f′(c) = 0 ; ici f′(x) = 3x(x − 2) s''annule en c = 0"},{"id":"b","text":"Théorème des accroissements finis : f(2) − f(−1) = f′(c) × 3 donne f′(c) = 0 avec c = 2"},{"id":"c","text":"Théorème des valeurs intermédiaires : comme f(−1) = f(2) = 0, la fonction f s''annule entre −1 et 2, ce qui fournit la tangente horizontale cherchée"},{"id":"d","text":"Aucun théorème ne s''applique ici, car f n''est pas dérivable sur l''intervalle fermé [−1 ; 2] tout entier"}]'::jsonb, 'a', 'C''est la situation exacte du théorème de Rolle : f, polynôme, est continue sur [−1 ; 2] et dérivable sur ]−1 ; 2[, et les deux valeurs aux bornes coïncident (f(−1) = f(2) = 0). Il existe donc c ∈ ]−1 ; 2[ tel que f′(c) = 0 ; en résolvant 3x(x − 2) = 0 on trouve x = 0 ou x = 2, et seul c = 0 appartient à l''intervalle OUVERT ]−1 ; 2[ ✓. Le raisonnement par le théorème des accroissements finis part juste — il généralise Rolle et donne bien f′(c) = 0 ici — mais il conclut c = 2, une BORNE de l''intervalle : les deux théorèmes fournissent un c strictement intérieur. Le théorème des valeurs intermédiaires, lui, parle de l''annulation de f, pas de celle de f′ : il ne dit rien des tangentes. Enfin, un polynôme est dérivable sur ℝ tout entier : la dérivabilité ne fait ici jamais défaut.', 5, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('0a8653ff-cdad-57be-addc-9bab0d4e33a1', 'a093e497-76a7-5dc4-90e2-94b61e1c646f', 'Épisode 6 — Dernière salle : le Miroir de Courbure. Pour que le miroir te rende ton reflet, tu dois lui désigner le point où C_f traverse sa propre tangente. On rappelle f(x) = x³ − 3x² + 4 sur ℝ et f′(x) = 3x² − 6x. Quelle affirmation est correcte ?', '[{"id":"a","text":"f″(x) = 6x − 6 s''annule en 1 sans changer de signe : C_f n''a aucun point d''inflexion"},{"id":"b","text":"f″(x) = 6x s''annule en 0 en changeant de signe : le point d''inflexion est J(0 ; 4)"},{"id":"c","text":"f″(x) = 6x − 6 s''annule en 1 en changeant de signe : le point I(1 ; 2) est un point d''inflexion de C_f"},{"id":"d","text":"f″(x) = 6x − 6 s''annule en 1 : le point d''inflexion est I(1 ; 0), car un point d''inflexion a toujours une ordonnée nulle"}]'::jsonb, 'c', 'On dérive f′(x) = 3x² − 6x pour obtenir la dérivée seconde f″(x) = 6x − 6 = 6(x − 1). Elle s''annule en 1 en changeant de signe (négative pour x < 1, positive pour x > 1), donc le point I d''abscisse 1 est un point d''inflexion, et son ordonnée est f(1) = 1 − 3 + 4 = 2 : c''est I(1 ; 2) ✓. Affirmer que 6(x − 1) ne change pas de signe est faux : une expression affine de coefficient non nul change toujours de signe en sa racine. Écrire f″(x) = 6x oublie de dériver le terme −6x de f′, dont la dérivée vaut −6. Et donner l''ordonnée 0 confond l''annulation de f″ avec celle de f : l''abscisse du point d''inflexion annule f″, mais son ordonnée reste f(1). Bonus du Sanctuaire : ce même point I(1 ; 2) est aussi centre de symétrie de C_f, car f(2 − x) = −x³ + 3x² = 2 × 2 − f(x) pour tout réel x.', 6, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.exercises (id, chapter_id, subject_id, title, difficulty, xp_reward, reward_coins, mode, source, display_order, correction_video) VALUES
  ('27d47fea-32f6-5840-9cbd-26601f44ddcc', '3f2eb68a-bcd1-5538-b438-d78f8b67fa44', 'math-bac-math', '🏛️ Annales Bac ⭐⭐⭐⭐ : Session principale — Étude complète d''une fonction rationnelle', 4, 300, 60, 'challenge', 'admin', 7, NULL)
ON CONFLICT (id) DO UPDATE SET
  chapter_id = EXCLUDED.chapter_id,
  subject_id = EXCLUDED.subject_id,
  title = EXCLUDED.title,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('6dc22130-2a49-554e-b63f-abe8b7d1ec33', '27d47fea-32f6-5840-9cbd-26601f44ddcc', 'Partie A. Soit g la fonction définie sur ℝ par g(x) = x³ + 3x + 2. Que peut-on affirmer sur l''équation g(x) = 0 ?', '[{"id":"a","text":"Elle admet trois solutions réelles, puisque g est de degré 3"},{"id":"b","text":"Elle admet une unique solution α, et α appartient à ]0 ; 1["},{"id":"c","text":"Elle admet une unique solution α, et α appartient à ]−1 ; 0["},{"id":"d","text":"Elle admet deux solutions, car g′ s''annule en −1"}]'::jsonb, 'c', 'g est polynomiale, donc continue et dérivable sur ℝ, et g′(x) = 3x² + 3 = 3(x² + 1) > 0 pour tout réel x : g est strictement croissante sur ℝ. Ses limites sont −∞ en −∞ et +∞ en +∞, donc g réalise une bijection de ℝ sur ℝ et l''équation g(x) = 0 admet une unique solution α. On localise : g(−1) = −1 − 3 + 2 = −2 < 0 et g(0) = 2 > 0, donc α ∈ ]−1 ; 0[ ✓. Le piège courant : croire qu''un polynôme de degré 3 a toujours trois racines réelles — il en a trois comptées dans ℂ, mais ici la stricte croissance en impose exactement une ; chercher α dans ]0 ; 1[ oublie que g(0) = 2 est déjà positif, donc que le changement de signe est à gauche de 0 ; et « g′ s''annule en −1 » résout mal 3x² + 3 = 0, équation qui n''a aucune solution réelle (x² = −1).', 1, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('a8308be5-b3e1-5039-9a64-a5daa7ca3b44', '27d47fea-32f6-5840-9cbd-26601f44ddcc', 'Toujours pour g(x) = x³ + 3x + 2, donner un encadrement de α d''amplitude 10⁻¹.', '[{"id":"a","text":"−0,6 < α < −0,5"},{"id":"b","text":"−0,5 < α < −0,4"},{"id":"c","text":"−0,7 < α < −0,6"},{"id":"d","text":"−0,8 < α < −0,7"}]'::jsonb, 'a', 'g étant strictement croissante, α est encadré par deux valeurs entre lesquelles g CHANGE de signe. On calcule : g(−0,8) = −0,512 − 2,4 + 2 = −0,912 ; g(−0,7) = −0,343 − 2,1 + 2 = −0,443 ; g(−0,6) = −0,216 − 1,8 + 2 = −0,016 ; g(−0,5) = −0,125 − 1,5 + 2 = 0,375 ; g(−0,4) = −0,064 − 1,2 + 2 = 0,736. Le seul changement de signe se produit entre −0,6 (négatif) et −0,5 (positif), donc −0,6 < α < −0,5 ✓ (contrôle : g(−0,6) ≈ −0,016 est très proche de 0, α est donc juste au-dessus de −0,6 ✓). Le piège courant : choisir un intervalle sur lequel g garde le MÊME signe — sur [−0,7 ; −0,6] les deux valeurs sont négatives, sur [−0,5 ; −0,4] les deux sont positives, et sur [−0,8 ; −0,7] les deux sont négatives : aucun de ces intervalles ne peut contenir α. C''est la vérification du changement de signe, et non la simple proximité de 0, qui localise la racine.', 2, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('d9b2c895-b592-5c4a-bce9-5a90b2c2ba96', '27d47fea-32f6-5840-9cbd-26601f44ddcc', 'Partie B. Soit f définie sur ℝ par f(x) = (x³ − 1)/(x² + 1). Exprimer f′(x) à l''aide de la fonction g de la partie A.', '[{"id":"a","text":"f′(x) = g(x)/(x² + 1)²"},{"id":"b","text":"f′(x) = x(5x³ + 3x − 2)/(x² + 1)²"},{"id":"c","text":"f′(x) = 3x²/(2x)"},{"id":"d","text":"f′(x) = x g(x)/(x² + 1)²"}]'::jsonb, 'd', 'Le dénominateur x² + 1 ne s''annule jamais, donc f est dérivable sur ℝ. Avec u(x) = x³ − 1 (u′ = 3x²) et v(x) = x² + 1 (v′ = 2x), la formule (u/v)′ = (u′v − v′u)/v² donne au numérateur 3x²(x² + 1) − 2x(x³ − 1) = 3x⁴ + 3x² − 2x⁴ + 2x = x⁴ + 3x² + 2x = x(x³ + 3x + 2) = x g(x). Donc f′(x) = x g(x)/(x² + 1)² ✓ (contrôle en x = 1 : f′(1) = 1 × g(1)/4 = 6/4 = 1,5 ; par le calcul direct, (3 × 2 − 2 × 0)/4 = 1,5 ✓). Le piège courant : oublier le facteur x mis en évidence donne g(x)/(x² + 1)², ce qui fausse tout le tableau de signe ; écrire la formule du quotient avec un « + » au numérateur donne 5x⁴ + 3x² − 2x = x(5x³ + 3x − 2), un polynôme qui n''est plus g ; et dériver numérateur et dénominateur séparément (3x²/(2x)) applique une règle qui n''existe pas.', 3, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('f730220a-8410-56d5-a46e-58b884d80bcf', '27d47fea-32f6-5840-9cbd-26601f44ddcc', 'Sachant que f′(x) = x g(x)/(x² + 1)², que g est strictement négative sur ]−∞ ; α[ et strictement positive sur ]α ; +∞[, avec −1 < α < 0, dresser le sens de variation de f.', '[{"id":"a","text":"f est croissante sur ]−∞ ; α] puis décroissante sur [α ; +∞["},{"id":"b","text":"f est croissante sur ]−∞ ; α], décroissante sur [α ; 0], croissante sur [0 ; +∞["},{"id":"c","text":"f est décroissante sur ]−∞ ; α], croissante sur [α ; 0], décroissante sur [0 ; +∞["},{"id":"d","text":"f est décroissante sur ]−∞ ; 0] puis croissante sur [0 ; +∞[, avec un unique minimum en α"}]'::jsonb, 'b', 'Le dénominateur (x² + 1)² est strictement positif, donc f′(x) a le signe du PRODUIT x g(x), qui change de signe en DEUX points : α et 0 (avec α < 0). Sur ]−∞ ; α[ : x < 0 et g(x) < 0, le produit est positif, f croît. Sur ]α ; 0[ : x < 0 et g(x) > 0, le produit est négatif, f décroît. Sur ]0 ; +∞[ : x > 0 et g(x) > 0, le produit est positif, f croît ✓ (contrôle numérique : f′(−2) = (−2)g(−2)/25 = (−2)(−12)/25 = 0,96 > 0 ✓ ; f′(−0,2) = (−0,2)(1,392)/1,0816 ≈ −0,257 < 0 ✓ ; f′(1) = 1,5 > 0 ✓). Le piège courant : ne retenir que le signe de g et oublier le facteur x ne produit qu''un seul changement de signe, en α (réponses a et d) ; et lire le tableau de signe à l''envers échange systématiquement croissance et décroissance (réponse c). Un produit de deux facteurs impose de croiser DEUX lignes de signe, pas une.', 4, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('164b7525-6c64-5a28-8d71-3051899ac882', '27d47fea-32f6-5840-9cbd-26601f44ddcc', 'Toujours pour f(x) = (x³ − 1)/(x² + 1). Que peut-on dire du point de C_f d''abscisse 0 ?', '[{"id":"a","text":"f y admet un maximum local égal à −1"},{"id":"b","text":"C''est un point d''inflexion, puisque f′(0) = 0"},{"id":"c","text":"f y admet un minimum local égal à −1"},{"id":"d","text":"Ce n''est ni un extremum ni un point d''inflexion"}]'::jsonb, 'c', 'On a f′(0) = 0 × g(0)/1 = 0, la tangente en ce point est donc horizontale. D''après le tableau de variation, f′ est strictement négative juste avant 0 (sur ]α ; 0[) et strictement positive juste après : f′ s''annule en 0 EN CHANGEANT DE SIGNE, de − à +, ce qui caractérise un MINIMUM local. Sa valeur est f(0) = (0 − 1)/(0 + 1) = −1 ✓ (contrôle : f(−0,2) ≈ (−1,008)/1,04 ≈ −0,969 > −1 et f(0,2) ≈ (−0,992)/1,04 ≈ −0,954 > −1 : les valeurs voisines sont bien supérieures ✓). Le piège courant : lire le changement de signe à l''envers (+ puis −) conclut à un maximum ; croire que f′(a) = 0 signale un point d''inflexion confond les deux critères — l''inflexion se lit sur le changement de signe de f″, jamais sur l''annulation de f′ ; et « ni l''un ni l''autre » serait le cas de x ↦ x³ en 0, où f′ NE change PAS de signe, ce qui n''est pas la situation ici.', 5, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('629e3c1f-6d4d-5ab9-b434-43ad84af068a', '27d47fea-32f6-5840-9cbd-26601f44ddcc', 'On note D la droite d''équation y = x. Étudier la nature de D pour C_f et la position relative de C_f et de D.', '[{"id":"a","text":"D est asymptote oblique à C_f en −∞ et en +∞ ; C_f est au-dessus de D sur ]−∞ ; −1[ et au-dessous sur ]−1 ; +∞["},{"id":"b","text":"D est asymptote oblique ; C_f est au-dessous de D sur ]−∞ ; −1[ et au-dessus sur ]−1 ; +∞["},{"id":"c","text":"C_f n''admet pas d''asymptote oblique : elle admet l''asymptote horizontale y = 0"},{"id":"d","text":"D est asymptote oblique et C_f reste toujours strictement au-dessous de D"}]'::jsonb, 'a', 'On calcule la différence : f(x) − x = (x³ − 1 − x(x² + 1))/(x² + 1) = (x³ − 1 − x³ − x)/(x² + 1) = −(x + 1)/(x² + 1). Son numérateur est de degré 1 et son dénominateur de degré 2, donc f(x) − x → 0 en −∞ comme en +∞ : D est bien asymptote oblique à C_f aux deux infinis ✓. Pour la position, le dénominateur x² + 1 est toujours positif, donc f(x) − x a le signe de −(x + 1) : positif pour x < −1 (C_f au-dessus de D) et négatif pour x > −1 (C_f au-dessous), les deux courbes se coupant au point d''abscisse −1, où f(−1) = −2/2 = −1 = (−1) ✓ (contrôle en x = −3 : f(−3) = −28/10 = −2,8 > −3 ✓ ; en x = 2 : f(2) = 7/5 = 1,4 < 2 ✓). Le piège courant : oublier le signe « − » devant (x + 1) retourne toute la position relative ; conclure à une asymptote horizontale y = 0 compare mal les degrés — le numérateur de f est de degré 3 et le dénominateur de degré 2, l''écart de 1 annonce précisément une asymptote OBLIQUE ; et affirmer que C_f reste d''un seul côté de D ignore que f(x) − x s''annule en −1 en changeant de signe : la courbe traverse son asymptote.', 6, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
