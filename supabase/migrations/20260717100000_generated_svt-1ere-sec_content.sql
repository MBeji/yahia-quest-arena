...
INSERT INTO public.questions (id, exercise_id, prompt, options, correct_option, explanation, display_order, question_type, answer_key, distractor_tags) VALUES
  ('cb6b615c-d0af-532b-b20c-08cdda3e006e', '6dccdf2a-b99f-5888-9622-c0201d5daec5', 'On met un échantillon de phosphate de 1 cm3 dans un tube à essai contenant 10 cm3 d''eau pure. Après agitation, on ne constate aucun changement. Qu''en déduit-on ?', '[{"id":"a","text":"La roche est insoluble dans l''eau"},{"id":"b","text":"La roche est non poreuse"},{"id":"c","text":"La roche est imperméable à l''eau"},{"id":"d","text":"La roche est compacte et résistante"}]'::jsonb, 'a', 'L''expérience ne teste qu''une chose : la roche ne se dissout pas, elle est donc insoluble ✓. Les trois autres propositions sont vraies de la roche phosphatée, mais aucune ne se déduit de CETTE expérience — et c''est tout le piège de l''item. La porosité se démontre en versant de l''eau dans un creux ménagé dans la roche : elle est rapidement absorbée, donc la roche est bien poreuse, et l''affirmation « non poreuse » est même carrément fausse. L''imperméabilité se démontre en versant encore de l''eau : elle ne traverse pas. Et l''aspect compact s''observe à l''œil. Retiens la règle : une conclusion ne vaut que ce que vaut le protocole qui la produit.', 3, 'mcq', NULL, NULL)
ON CONFLICT (id) DO UPDATE SET
  exercise_id = EXCLUDED.exercise_id,
  prompt = EXCLUDED.prompt,
  options = EXCLUDED.options,
