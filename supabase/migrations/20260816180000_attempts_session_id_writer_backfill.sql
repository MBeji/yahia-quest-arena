-- `submit_exercise_attempt` renseigne `attempts.session_id`, et l'historique est
-- rattaché en une passe.
--
-- Suite de 20260816170000, qui a posé la colonne, additive et sans écrivain. On
-- referme ici : la RPC écrit la clé, puis le backfill applique une DERNIÈRE fois
-- l'heuristique de proximité temporelle à l'historique — après quoi plus personne
-- n'a à deviner à quelle session appartiennent les réponses d'une tentative.
--
-- D-1 — LE CORPS EST RECOPIÉ À L'IDENTIQUE, UNE SEULE LIGNE CHANGE. C'est la
-- fonction la plus critique de l'application : scoring des trois variantes, garde
-- anti-précipitation, XP/coins, badges, potions et bouclier de reprise, clôture
-- SM-2, adaptation de difficulté, objectifs du jour et quêtes de la semaine. Elle
-- est aussi très disputée — quatre révisions depuis juillet (Rappel
-- #20260714130000, clôture SM-2 #20260720170000, short_answer #20260727120000,
-- bonus de vitesse du boss #20260816140000) — donc la seule règle qui tienne est :
-- repartir de la DERNIÈRE sur `main` au moment d'écrire, jamais d'une révision
-- retenue de mémoire. Le corps ci-dessous est celui de 20260816140000 ligne pour
...
-- resteront NULL : c'est la bonne réponse pour elles.
...
-- ---------------------------------------------------------------------------
-- Backfill — une passe, dans la même transaction que le nouvel écrivain, donc
-- sans fenêtre où une soumission écrirait pendant qu'on rattrape (D-2, D-3).
...
WITH paire AS (
  ...
  WHERE a.session_id IS NULL
    AND NOT EXISTS (
      SELECT 1
      FROM public.attempts deja
      WHERE deja.session_id = s.id
    )
)
...
-- décision. Le lecteur du détail parental garde une résolution par proximité
-- temporelle pour les lignes restées sans session ; savoir s'il peut partir
-- demande de connaître leur nombre EN PRODUCTION, ce qu'aucun poste de dev ne
-- peut lire. Ce NOTICE le fait apparaître dans le log du run
-- `db-migrate-prod.yml` qui applique cette migration. Zéro restant → le repli
-- peut être supprimé ; n restants → ce sont des tentatives dont la session est
-- introuvable ou ambiguë, et le repli a exactement là sa raison d'être.
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  v_sans_session BIGINT;
  v_total BIGINT;
BEGIN
  SELECT count(*) FILTER (WHERE session_id IS NULL), count(*)
    INTO v_sans_session, v_total
    FROM public.attempts;

  RAISE NOTICE '[attempts.session_id] % tentative(s) sur % restent sans session après backfill.',
    v_sans_session, v_total;
END;
$$;
